 ...
 **
 **
 ** $Header: distate.c, 32, 10/11/00 8:26:36 PM, Brent$
 ** $Log: 
...
GR_DIENTRY(grAlphaBlendFunction, void , (GrAlphaBlendFnc_t rgb_sf, GrAlphaBlendFnc_t rgb_df, GrAlphaBlendFnc_t alpha_sf, GrAlphaBlendFnc_t alpha_df) )
{
 #define FN_NAME "grAlphaBlendFunction"
  ...
  /* OPT 0.1.2 state-dedup: if the incoming blend state is identical to the
  ** values this setter already stored, skip the INVALIDATE/revalidation.
  ** Compared only against this setter's own arg shadow (grAlphaBlendFunction
  ** args), which every storing path keeps in sync.  On Napalm this entry
  ** also forces both blend ops to GR_BLEND_OP_ADD, so only skip when the
  ** stored ops are already ADD (grAlphaBlendFunctionExt may have set other
  ** ops).  assertDefaultState() force-invalidates these register groups at
  ** context open, so a first-call match can never skip the initial
  ** hardware validation. */
  if (gc->open &&
      (LOADARG(grAlphaBlendFunction, rgb_sf)   == rgb_sf) &&
      (LOADARG(grAlphaBlendFunction, rgb_df)   == rgb_df) &&
      (LOADARG(grAlphaBlendFunction, alpha_sf) == alpha_sf) &&
      (LOADARG(grAlphaBlendFunction, alpha_df) == alpha_df) &&
      (!IS_NAPALM(gc->bInfo->pciInfo.deviceID) ||
       ((LOADARG(grAlphaBlendFunction, rgb_op)   == GR_BLEND_OP_ADD) &&
        (LOADARG(grAlphaBlendFunction, alpha_op) == GR_BLEND_OP_ADD)))) {
    return;
  }
...
GR_DIENTRY(grAlphaTestFunction, void , (GrCmpFnc_t fnc) )
{
 #define FN_NAME "grAlphaTestFunction"
  ...
  /* OPT 0.1.2 state-dedup: identical alpha-test function -> no-op. */
  if (gc->open && (LOADARG(grAlphaTestFunction, fnc) == fnc)) {
    return;
  }
...
GR_DIENTRY(grAlphaTestReferenceValue, void , (GrAlpha_t value) )
{
 #define FN_NAME "grAlphaTestReferenceValue"
  ...
  /* OPT 0.1.2 state-dedup: identical alpha-test reference -> no-op. */
  if (gc->open && (LOADARG(grAlphaTestReferenceValue, value) == value)) {
    return;
  }
          ...
          GrCombineLocal_t local, GrCombineOther_t other, FxBool invert) )
{
 #define FN_NAME "grAlphaCombine"
  ...
  /* OPT 0.1.2 state-dedup: skip only when the legacy (non-ext) combine
  ** path is already active — this setter also clears STATE_USING_CA and
  ** disables GR_COMBINEEXT_MODE, so those must already be in the target
  ** state for the early-out to be a true no-op. */
  if (gc->open &&
      (gc->state.grEnableArgs.combine_ext_mode == GR_MODE_DISABLE) &&
      ((gc->state.combineExtsInUse & STATE_USING_CA) == 0) &&
      (LOADARG(grAlphaCombine, function) == function) &&
      (LOADARG(grAlphaCombine, factor)   == factor) &&
      (LOADARG(grAlphaCombine, local)    == local) &&
      (LOADARG(grAlphaCombine, other)    == other) &&
      (LOADARG(grAlphaCombine, invert)   == invert)) {
    return;
  }
...
#define FN_NAME "grColorCombine"
  ...
  /* OPT 0.1.2 state-dedup: skip only when the legacy (non-ext) combine
  ** path is already active — this setter also clears STATE_USING_CC and
  ** disables GR_COMBINEEXT_MODE, so those must already be in the target
  ** state for the early-out to be a true no-op. */
  if (gc->open &&
      (gc->state.grEnableArgs.combine_ext_mode == GR_MODE_DISABLE) &&
      ((gc->state.combineExtsInUse & STATE_USING_CC) == 0) &&
      (LOADARG(grColorCombine, function) == function) &&
      (LOADARG(grColorCombine, factor)   == factor) &&
      (LOADARG(grColorCombine, local)    == local) &&
      (LOADARG(grColorCombine, other)    == other) &&
      (LOADARG(grColorCombine, invert)   == invert)) {
    return;
  }
...
#define FN_NAME "grChromakeyMode"
  ...
  /* OPT 0.1.2 state-dedup: identical chromakey mode -> no-op. */
  if (gc->open && (LOADARG(grChromakeyMode, mode) == mode)) {
    return;
  }
...
#define FN_NAME "grChromakeyValue"
  ...
  /* OPT 0.1.2 state-dedup: identical chromakey color -> no-op.  Safe even
  ** though grChromaRange() also stores this arg: the comparison is against
  ** the shared stored field itself, so it is always current. */
  if (gc->open && (LOADARG(grChromakeyValue, color) == color)) {
    return;
  }
...
GR_DIENTRY(grDepthMask, void , (FxBool enable) )
{
 #define FN_NAME "grDepthMask"
  ...
  /* OPT 0.1.2 state-dedup: identical depth mask -> no-op. */
  if (gc->open && (LOADARG(grDepthMask, enable) == enable)) {
    return;
  }
...
GR_DIENTRY(grDepthBufferFunction, void , (GrCmpFnc_t fnc) )
{
 #define FN_NAME "grDepthBufferFunction"
  ...
  /* OPT 0.1.2 state-dedup: identical depth compare function -> no-op. */
  if (gc->open && (LOADARG(grDepthBufferFunction, fnc) == fnc)) {
    return;
  }
...
#define FN_NAME "grColorMask"
  ...
  /* OPT 0.1.2 state-dedup: compute the alpha value exactly as the store
  ** below would (it depends on the current grDepthMask arg) and skip when
  ** both stored values already match. */
  {
    const FxI32 newAlpha = ((LOADARG(grDepthMask, enable) && alpha)
                            ? -1
                            : (FxI32)alpha);
    if (gc->open &&
        (LOADARG(grColorMask, rgb)   == rgb) &&
        (LOADARG(grColorMask, alpha) == newAlpha)) {
      return;
    }
  }
...
#define FN_NAME "grConstantColorValue"
  ...
  /* OPT 0.1.2 state-dedup: identical constant color -> no-op. */
  if (gc->open && (LOADARG(grConstantColorValue, color) == color)) {
    return;
  }
  ...
  for(tmu = 0; tmu < gc->num_tmu; tmu++) {
#if Q3D_FC_PERFORMANCE_TRILINEAR
    FxU32   performanceTrilinearStateCheckTMU = tmu;
#endif
    tmuSource = tmu;
    ...
    if(tmuRequiredByDownstreamUnit && !doTMU0Passthrough || palletizedTexture) {
      ...
      if(NOTVALID_TMU(tmu, textureMode)) {
        ...
        REG_GROUP_END();
#if Q3D_FC_PERFORMANCE_TRILINEAR
        if(gc->state.per_tmu[tmu].performanceTrilinear)
            SetPerformanceTrilinearQ3D(tmu,gc->state.shadow.tmuState[tmu].tLOD);
#endif
        if(IS_NAPALM(gc->bInfo->pciInfo.deviceID)) {
      ...
      if ((IS_NAPALM(gc->bInfo->pciInfo.deviceID)) && (gc->do2ppc)) {
        if(tmu == 0 && doTMU0Passthrough) {
...
#if Q3D_FC_PERFORMANCE_TRILINEAR
          performanceTrilinearStateCheckTMU = 1;
#endif
          ...
          enable2PPC = FXTRUE;
#if Q3D_FC_PERFORMANCE_TRILINEAR
          performanceTrilinearStateCheckTMU = 0;
#endif
        } else {    
#if DEBUG_2PPC        
        ...
        REG_GROUP_END();
#if Q3D_FC_PERFORMANCE_TRILINEAR
        /* Use perfromanceTrilinearStateCheckTMU as index as this takes account of 2PPC mode */
        if(gc->state.per_tmu[performanceTrilinearStateCheckTMU].performanceTrilinear)
            SetPerformanceTrilinearQ3D(tmu,gc->state.shadow.tmuState[tmu].tLOD);
#endif
  ...
  if (NOTVALID(fogMode)) {
    ...
    if(gc->grPixelSample == 2) {
      ...
      for(chipIndex = 0; chipIndex < gc->chipCount; chipIndex++) {
		...
		/* The above code is broken for 4 and 8 chip boards in 4 or 8 SPAA respectively*/
        {
