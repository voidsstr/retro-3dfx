...
**
** $Header: minihwc.c, 91, 10/11/00 8:39:20 PM, Brent$
** $Log: 
...
/* crash).  Same crash-proof append-line logger as in GSST.C: open/seek-  */
/* end/write/flush/close per line so the log survives an AV on the very   */
/* next instruction.  wsprintfA/wvsprintfA only (user32; no floats).      */
/* Remove this block (and all _hwcLog call sites) when done debugging.    */
/* ====================================================================== */
#include <stdarg.h>
static void _hwcLog(const char* fmt, ...)
{
  char logBuf[600];
  int logLen;
  HANDLE logH;
...
#define FN_NAME "hwcGammaTable"
  ...
  if(GETENV("FX_GLIDE_USE_APP_GAMMA")) {    
    if(!atoi(GETENV("FX_GLIDE_USE_APP_GAMMA"))) {
       GDBG_INFO(80, "%s: Not using app. gamma settings.");
       return FXTRUE;      
    }
  }  
...
#define FN_NAME "hwcShareContextData"
  ...
  /* CRASH-TRACE */
  _hwcLog("hwcShareContextData ENTER bInfo=0x%x boardNum=%d osNT=%d hwcProtocol=%d hdc=0x%x",
          bInfo, bInfo->boardNum, bInfo->osNT,
          hInfo.boardInfo[bInfo->boardNum].hwcProtocol, bInfo->hdc);
  ...
  if( HWCEXT_PROTOCOL( bInfo->boardNum ) )
  {
    if (bInfo->osNT) {
      ...
      GDBG_INFO(80, FN_NAME ":  Calling ExtEscape(HWCEXT_CONTEXT_DWORD_NT)\n");
      {
        ...
        /* CANDIDATE FIX: pre-clear the result struct.  The display
         * driver's failure paths (hwcContextDWORDNT, HWCEXT.C) set
         * resStatus=0 but never write dwordOffset, which otherwise
         * leaves *data pointing at stack garbage. */
        ctxRes.resStatus = 0;
        ctxRes.optData.contextDwordNTRes.dwordOffset = 0;

        /* CRASH-TRACE */
        _hwcLog("hwcShareContextData NT: calling ExtEscape(HWCEXT_CONTEXT_DWORD_NT) escape=%d pid=0x%x cs=0x%x ds=0x%x",
                HWCEXT_ESCAPE(bInfo->boardNum),
        ...
        escRet = ExtEscape((HDC) bInfo->hdc, HWCEXT_ESCAPE(bInfo->boardNum), sizeof(ctxReq), (void *) &ctxReq,
                  sizeof(ctxRes), (void *) &ctxRes);

        /* CRASH-TRACE */
        _hwcLog("hwcShareContextData NT: ExtEscape ret=%d resStatus=0x%x dwordOffset=0x%x",
                escRet, ctxRes.resStatus,
                ctxRes.optData.contextDwordNTRes.dwordOffset);

        /* CANDIDATE FIX: original code blindly did
         *   *data = (FxU32*)ctxRes.optData.contextDwordNTRes.dwordOffset;
         * with no error check at all -- unlike the 9x branch below, which
         * falls back to &dummyContextDWORD on failure.  On our XP stack
         * the escape can fail (resStatus=0) or "succeed" with a NULL
         * mapping (h3MapKernelMemoryToCurrentProcessSpace returns NULL but
         * the IOCTL still completes NO_ERROR), and grSstWinOpenExt then
         * writes through the bogus pointer -> the grSstWinOpen+0xabc AV.
         * Success contract per HWCEXT.C: escRet>0, resStatus==1, offset!=0. */
        if (escRet <= 0 ||
            ctxRes.resStatus != 1 ||
            ctxRes.optData.contextDwordNTRes.dwordOffset == 0) {
          /* CRASH-TRACE */
          _hwcLog("hwcShareContextData NT: CANDIDATE FIX ENGAGED - escape failed/NULL, using dummyContextDWORD (lost-context detection disabled)");
  ...
  {
    /* CANDIDATE FIX: if the HWCEXT protocol handshake never happened
     * (hwcProtocol != -1) the original code skipped the whole body and
     * never wrote *data at all, leaving the caller's gc->lostContext
     * NULL -> AV on the very next write in grSstWinOpenExt. */
    /* CRASH-TRACE */
    _hwcLog("hwcShareContextData: HWCEXT_PROTOCOL false (hwcProtocol=%d) - CANDIDATE FIX ENGAGED, using dummyContextDWORD",
            hInfo.boardInfo[bInfo->boardNum].hwcProtocol);
    *data = &dummyContextDWORD;
  }
#endif
#if HWC_GDX_INIT
...
#endif
  /* CRASH-TRACE */
  _hwcLog("hwcShareContextData EXIT *data=0x%x (dummy=0x%x) retVal=%d",
          *data, &dummyContextDWORD, retVal);
  return retVal;
#undef FN_NAME
} /* hwcShareContextData */
