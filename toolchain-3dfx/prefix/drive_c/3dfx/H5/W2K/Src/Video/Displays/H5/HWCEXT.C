...
hwcExt(DWORD *lpInput, DWORD *lpOutput, PDEV *ppdev)
{
  ...
  /* ---- TEARDOWN TRACE: log only the WinOpen/WinClose lifecycle escapes    *
   * Glide's ExtEscape hits (exclusive grab/release, context alloc/release,  *
   * desktop-restore, ctx-dword, unmap).  We deliberately DO NOT log the     *
   * high-frequency EXECUTEFIFO/QUERYCONTEXT/GETLINEARADDR escapes so this    *
   * stays low-volume (mode transitions, not per-blt / per-frame).           */
  switch (req->which) {
    ...
    case HWCEXT_CONTEXT_DWORD_NT:
      if (req->which < (sizeof(extNames)/sizeof(extNames[0])))
        V5DLog("hwcExt escape which=0x%lx (%s) ppdev=%08lXh (Glide ExtEscape -> board/context lifecycle op on WinOpen/WinClose)\n",
               (unsigned long)req->which, extNames[req->which], (unsigned long)ppdev);
      else
        V5DLog("hwcExt escape which=0x%lx ppdev=%08lXh (Glide ExtEscape lifecycle op)\n",
               (unsigned long)req->which, (unsigned long)ppdev);
      break;
    default:
      break;
  }
