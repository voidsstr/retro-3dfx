...
__glSSTBegin(__GLcontext *gc, GLenum mode) {
    /* one-shot live draw-state sampler (CS green-world hunt): every 200th
    ** glBegin, log the state signature of what is about to be drawn. */
    ...
    if ( mode == GL_LINES || mode == GL_LINE_STRIP ) {
        ...
        gc->primState.vB = &gc->primState.vBuf[1];
    } else {
...
void __glSSTEndTStrip(__GLcontext *gc) {
    SSTPrimState *ps = &gc->primState;
    if (ps->vCounter > 2) {
        grDrawVertexArray(ps->stripMode, ps->vCounter - ps->countDelta, 
	  	          &ps->vList[ps->vReadPointer]);
    }
    gc->procs.endPrim = __glEndPrim;
}

void  RobFastCalcRGBColor(__GLcontext *gc, IVert *s, __GLvertex *vx);

void APIENTRY __glsstim_Vertex2fv_L(const GLfloat *v) {
    __GL_SETUP();
    ...
    /* Update new state */
    ps->vWritePointer = ps->nextWritePointer[ps->vWritePointer];
    ps->vA = ps->vB;
    ps->vB = ps->vC;
    ps->vC = &ps->vBuf[ps->vWritePointer];
    ps->vCounter++;
    ...
    if (!(f | savA->flags | savB->flags)) {
        if ((ps->vCounter-ps->countDelta) == ps->vBufSize) {
	    grDrawVertexArray(ps->stripMode, ps->vCounter-ps->countDelta, 
			      &ps->vList[ps->vReadPointer]);
            ps->vReadPointer = ps->vWritePointer;
            ps->vCounter = 2;
            ps->stripMode = GR_TRIANGLE_STRIP_CONTINUE;
            ps->countDelta = 2;
        }
    } else {
        if (ps->vCounter > 3) {
	    grDrawVertexArray(ps->stripMode, ps->vCounter - 1 - ps->countDelta,
                              &ps->vList[ps->vReadPointer]);
	}
        ps->stripMode = GR_TRIANGLE_STRIP;
        ps->vReadPointer = ps->nextReadPointer[ps->vWritePointer];
        ps->vCounter = 2;
        ps->countDelta = 0;
...
void APIENTRY __glsstim_Vertex4fv_L(const GLfloat *v) {
    __GL_SETUP();
    float     *m  = (float *)&gc->transform.modelView->mvp;
    __GLviewport     *vp = &gc->state.viewport;
    SSTPrimState *ps = &gc->primState;
...
void  RobFastCalcRGBColor(__GLcontext *gc, IVert *s, __GLvertex *vx)
{
    ...
    /* Compute the color */
    r = msm->sceneColor.r + lss->ambient.r * ms->ambient.r;
    g = msm->sceneColor.g + lss->ambient.g * ms->ambient.g;
    b = msm->sceneColor.b + lss->ambient.b * ms->ambient.b;
...
#define GL_JCLAMP(x) { \
    unsigned msk = ~((int)(GL_UINTCAST(x)-1)>>31); \
    GL_UINTCAST(x) -= (unsigned)0x437f0000; \
    GL_UINTCAST(x) &= GL_SIGNMASK(x); \
    GL_UINTCAST(x) += (unsigned)0x437f0000; \
    GL_UINTCAST(x) &= msk; \
			      }
...
#else
static float robzero = 0.0f;
static float rob255 = 255.0f;
__declspec(naked)
void  RobFastCalcRGBColor(__GLcontext *gc, IVert *s, __GLvertex *vx)
{
__asm {
...
#define lss  esi
	...
	push    ebx
        mov	lsm, __GLcontext.light.sources[_gc_]


    /* Compute the color */
    /* r = gc->light.front.sceneColor.r +
               lss->ambient.r * gc->state.light.front.ambient.r;
       g = gc->light.front.sceneColor.g +
               lss->ambient.g * gc->state.light.front.ambient.g;
       b = gc->light.front.sceneColor.b +
               lss->ambient.b * gc->state.light.front.ambient.b; */
        flds	__GLcontext.light.front.sceneColor.r[_gc_]
        mov     lss, __GLlightSourceMachine.state[lsm]
        flds	__GLcontext.light.front.sceneColor.g[_gc_]
        flds	__GLcontext.light.front.sceneColor.b[_gc_]	// r g b

	flds    __GLlightSourceState.ambient.r[lss]
        fmuls	__GLcontext.state.light.front.ambient.r[_gc_]
	flds    __GLlightSourceState.ambient.g[lss]
        fmuls	__GLcontext.state.light.front.ambient.g[_gc_]
	flds    __GLlightSourceState.ambient.b[lss]
        fmuls	__GLcontext.state.light.front.ambient.b[_gc_]	// r g b R G B

	fxch	st(2)						// r g b B G R
	faddp	st(5), st					// R g b B G
	faddp	st(3), st					// R G b B
	faddp	st(1), st					// R G B
...
*/
	...
	flds	__GLlightSourceMachine.unitVPpli.x[lsm]
	fmuls	__GLvertex.normal.x[ebx]
	flds	__GLlightSourceMachine.unitVPpli.y[lsm]
	fmuls	__GLvertex.normal.y[ebx]
	flds	__GLlightSourceMachine.unitVPpli.z[lsm]
	fmuls	__GLvertex.normal.z[ebx]			// R G B X Y Z

	fxch	st(2)
	faddp	st(1), st
	faddp	st(1), st					// R G B N1
...
//        r += n1 * gc->state.light.front.diffuse.r * lss->diffuse.r;
//        g += n1 * gc->state.light.front.diffuse.g * lss->diffuse.g;
//        b += n1 * gc->state.light.front.diffuse.b * lss->diffuse.b;

	flds	__GLcontext.state.light.front.diffuse.r[_gc_]
	fmuls	__GLlightSourceState.diffuse.r[lss]
	flds	__GLcontext.state.light.front.diffuse.g[_gc_]
	fmuls	__GLlightSourceState.diffuse.g[lss]
	flds	__GLcontext.state.light.front.diffuse.b[_gc_]
	fmuls	__GLlightSourceState.diffuse.b[lss]	// R G B N1 r g b

	fxch	st(3)					// R G B b r g N1
	fmul	st(2), st(0)
	fmul	st(1), st(0)
	fmulp	st(3), st(0)				// R G B b r g

	fxch	st(1)					// R G B b g r
	faddp	st(5), st				// R G B b g
	faddp	st(3), st				// R G B b
	faddp	st(1), st				// R G B
...
//        n2 -= gc->light.front.threshold;
	...
	flds	__GLlightSourceMachine.hHat.x[lsm]
	fmuls	__GLvertex.normal.x[ebx]
	flds	__GLlightSourceMachine.hHat.y[lsm]
	fmuls	__GLvertex.normal.y[ebx]
	flds	__GLlightSourceMachine.hHat.z[lsm]
	fmuls	__GLvertex.normal.z[ebx]		// R G B X Y Z
	flds	__GLcontext.light.front.threshold[_gc_] // R G B X Y Z th

	fxch	st(2)					// R G B X th Z Y
	faddp	st(3), st				// R G B XY th Z
	faddp	st(2), st				// R G B N2 th
	fsubp	st(1), st				// R G B N2
...
//            GLuint ix = (GLuint)(n2 * gc->light.front.scale + __glHalf);
//            if (ix < __GL_SPEC_LOOKUP_TABLE_SIZE) {
//                n2 = gc->light.front.specTable[ix];
//            } else {
//                n2 = __glOne;
//            }

	fmuls	__GLcontext.light.front.scale[_gc_]
	fadds	__GLcontext.constants.half[_gc_]
	fistp	DWORD PTR [esp-4]
	mov	eax, [esp-4]
	cmp	eax, 256
	jae	FullSpec

	mov	ecx, __GLcontext.light.front.specTable[_gc_]
	flds	[ecx+eax*4]
	jmp	AfterN2
FullSpec:
	flds	__GLcontext.constants.one

AfterN2:
//            r += n2 * gc->state.light.front.specular.r * lss->specular.r;
//            g += n2 * gc->state.light.front.specular.g * lss->specular.g;
//            b += n2 * gc->state.light.front.specular.b * lss->specular.b;
//        }
//    }
	flds	__GLcontext.state.light.front.specular.r[_gc_]
	fmuls	__GLlightSourceState.specular.r[lss]
	flds	__GLcontext.state.light.front.specular.g[_gc_]
	fmuls	__GLlightSourceState.specular.g[lss]
	flds	__GLcontext.state.light.front.specular.b[_gc_]
	fmuls	__GLlightSourceState.specular.b[lss]	// R G B N2 r g b

	fxch	st(3)					// R G B b r g N2
	fmul	st(2), st(0)
	fmul	st(1), st(0)
	fmul	st(3), st(0)				// R G B b r g N2

	fxch	st(2)					// R G B b N2 g r
	faddp	st(6), st				// R G B b N2 g
	faddp	st(4), st				// R G B b N2
	fxch	st(1)					// R G B N2 b
	faddp	st(2), st				// R G B N2

    /* Clamp the computed color */
//    s->r = r;
//    s->g = g;
//    s->b = b;
//    s->a = gc->light.front.alpha;

AfterCalc:
	fstp	st(0)
	mov	ecx, [esp+24]
	mov	ebx, __GLcontext.light.front.alpha[_gc_]
...
AfterClip:
   ...
   /* end new code        */
        ...
        push    vo
        push    _gc_
        call    RobFastCalcRGBColor
        add     esp, 192

    // ps->vWritePointer = ps->nextWritePointer[ps->vWritePointer];
        mov     eax, ps.nextWritePointer[_gc_]
        mov     ecx, ps.vWritePointer[_gc_]
        mov     edx, DWORD PTR[eax+ecx*4]
        mov     ps.vWritePointer[_gc_], edx
...
AfterViewport:
        ...
        mov     edx, ps.vCounter[_gc_]
        cmp     edx, 3
        jb      Exit

        mov     ebx, [esp+4]
        mov     eax, [esp]
	mov     ecx, IVert.flags[ebx]
        mov     eax, IVert.flags[eax]
        or      ecx, code
        or      ecx, eax
        jne     DrawClipped

        mov     ebx, ps.vBufSize[_gc_]
        sub     edx, ps.countDelta[_gc_]
        cmp     ebx, edx
        jne     Exit

        mov     eax, ps.vReadPointer[_gc_]
        mov     ecx, ps.vList[_gc_]
        mov     ebx, ps.stripMode[_gc_]
        lea     eax, DWORD PTR [ecx+eax*4]
        push    eax
        push    edx
        push    ebx
        call    grDrawVertexArray

        mov     ecx, ps.vWritePointer[_gc_]
        mov     ps.vReadPointer[_gc_], ecx
        mov     ps.vCounter[_gc_], 2
        mov     ps.stripMode[_gc_], GR_TRIANGLE_STRIP_CONTINUE
        mov     ps.countDelta[_gc_], 2
        jmp     Exit
...
UpdateAndClip:
        mov     ecx, ps.vWritePointer[_gc_]
        mov     ebx, ps.nextReadPointer[_gc_]
        mov     eax, DWORD PTR[ebx+ecx*4]
        mov     ps.vReadPointer[_gc_], eax
        mov     ps.vCounter[_gc_], 2
        mov     ps.stripMode[_gc_], GR_TRIANGLE_STRIP
        mov     ps.countDelta[_gc_], 0
...
AfterClip:
        ...
        flds    IVert.Z[vo]		; oow Z
        fmul    st(0), st(1)		; oow z
        flds    IVert.Y[vo]		; oow z Y
        fmul    st(0), st(2)		; oow z y
        flds    IVert.X[vo]		; oow z y X
        fmul    st(0), st(3)		; oow z y x

        flds    __GLviewport.zScale[vp]	; oow z y x ZS
        fmulp   st(3), st		; oow z y x
        flds    __GLviewport.yScale[vp]	; oow z y x YS
        fmulp   st(2), st		; oow z y x
        flds    __GLviewport.xScale[vp] ; oow z y x XS
        fmulp   st(1), st		; oow z y x
...
Draw:
        ...
        mov     ecx, __GLcontext.glideLineFunc[_gc_]
