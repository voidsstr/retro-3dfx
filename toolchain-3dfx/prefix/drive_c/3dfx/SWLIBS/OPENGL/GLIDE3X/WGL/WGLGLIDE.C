...
/* RETRO3DFX EXPERIMENT: env-gated 32-bit (true-color) rendering on the Napalm
 * VSA-100. The Voodoo3-era Glide WGL path is hardwired to 16-bit RGB565; the
 * VSA-100 renders 32-bit ARGB. When RETRO3DFX_32BPP is set, expose 8/8/8 color
 * so a 32-bit PFD gets selected (→ ICD buffers become 4-byte, __wglFBInitDrawable)
 * and lock the color buffer in 8888. sst_export.c opens the hw buffer in
 * GR_PIXFMT_ARGB_8888 under the same gate. Env-unset = byte-identical to 0.2.2. */
int __retro3dfx_want32bpp(void)
{
    static int c = -1;
    if (c < 0) c = getenv("RETRO3DFX_32BPP") ? 1 : 0;
    return c;
}
...
void __wglGlideLockBuffer(__GLdrawableBuffer *buf, __GLdrawablePrivate *glPriv) {
      ...
      GrLfbWriteMode_t mode = __retro3dfx_want32bpp()
                                ? GR_LFBWRITEMODE_8888 : GR_LFBWRITEMODE_565;
