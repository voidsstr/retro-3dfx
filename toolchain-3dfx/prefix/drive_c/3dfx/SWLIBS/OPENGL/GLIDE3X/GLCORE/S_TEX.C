...
void __glCalcEyeLinear(__GLcontext *gc, __GLvertex *vx)
{
    ...
    maxTxu = gc->grNTexelFx;
    for( txu = 0; txu < maxTxu; txu++ ) {
        __GLtexture     *tex = gc->texture.currentTexture[txu];
        if ( tex ) {
                        ...
                        int x, int y, int w, int h)
{
    ...
    /* SUB-IMAGE FIX (UT / glTexSubImage2D crash): the source buffer lp->buffer is
     * FULL texture width (lp->width), so the row origin is lp->width*y+x, NOT the
     * sub-image width w*y+x — and both origin and row-skip must be scaled by the
     * source bytes-per-pixel. The old code (w*y+x, srcSkip = lp->width-w without
     * *src_bpp) only worked for the full-texture cook (w==lp->width, y==0); any
     * partial update (UT lightmap/texture region) walked out of bounds -> crash. */
    switch (lp->internalFormat) {
    case GL_LUMINANCE:  case GL_INTENSITY: src_bpp = 1; break;
    case GL_LUMINANCE_ALPHA:               src_bpp = 2; break;
    case GL_RGB:                           src_bpp = 3; break;
    case GL_RGBA:                          src_bpp = 4; break;
    default:                               src_bpp = 1; break;
    }

    srcPtr = ((GLubyte *)lp->buffer) + (lp->width * y + x) * src_bpp;
    dstPtr = ((GLubyte *)lp->pixelBuffer) + (lp->width * y + x) * dst_bpp;

    srcSkip = (lp->width - w) * src_bpp;
    dstSkip = (lp->width - w) * dst_bpp;
