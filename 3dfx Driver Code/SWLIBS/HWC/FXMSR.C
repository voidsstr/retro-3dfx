...
extern HANDLE hMemmapFile;  // defined in fxvxd.c
...
**  OR in the type (the types specified in hwc.h match the Intel
**  P6 mem types indentically).  
*/
static FxBool 
genPhysBase(FxU32 physBaseAddress, HwcMemType type, FxU32 *physBase) 
{
  FxBool
    ret = FXFALSE;
  ...
  /* Validate type */
  switch (type) {
  case HWC_MEM_TYPE_UNCACHEABLE:
  case HWC_MEM_TYPE_WRITE_COMBINING:
  case HWC_MEM_TYPE_WRITE_THROUGH:
  case HWC_MEM_TYPE_WRITE_PROTECTED:
  case HWC_MEM_TYPE_WRITEBACK:
    break;
  default:
    /* Invalid Type */
    return ret;
...
/*
**  hwcFindMTRRMatch - find an MTRR that matches this one.
**
...
hwcFindMTRRMatch(FxU32 physBaseAddress, FxU32 physSize,
...
/*
**  hwcFindFreeMTRR - find an MTRR that matches this one.
**
**  We return the MTRR number which is a special-case MSR calculated
**  in the following manner:
**
...
/*
**  hwcSetMTRR - set up a specified MTRR based on physical address, physical
**               size, and type.
**
...
*/
FX_EXPORT FxBool FX_CSTYLE
hwcSetMTRR(FxU32 mtrrNum, FxU32 physBaseAddr, FxU32 physSize,
           HwcMemType type)
{
#ifndef MAPPL_MAPMEM_MINIPORT
  FxBool
