...
{
    v->texture.x = tp[0]; v->texture.y = 0.0F; v->texture.z = 0.0F; v->texture.w = 1.0F;
}
...
{
    v->texture.x = tp[0]; v->texture.y = tp[1]; v->texture.z = 0.0F; v->texture.w = 1.0F;
}
...
{
    v->texture.x = tp[0]; v->texture.y = tp[1]; v->texture.z = tp[2]; v->texture.w = 1.0F;
}
...
{
    v->texture.x = tp[0]; v->texture.y = tp[1]; v->texture.z = tp[2]; v->texture.w = tp[3];
}
...
{
    v->texture.x = gc->state.current.texture.x;
    v->texture.y = gc->state.current.texture.y;
    v->texture.z = gc->state.current.texture.z;
    v->texture.w = gc->state.current.texture.w;
}
...
static void CompileElements_T2F_V3F(__GLcontext *gc, GLint offset, GLint first, GLsizei count)
{
	...
	off += gc->vertexArray.vp_stride;
	v->hasAndClipCode = gc->vertexArray.validateMask;
	v->texture.x = texCoordPtr[0];
	v->texture.y = texCoordPtr[1];
	v->texture.z = 0.0F;
	v->texture.w = 1.0F;
	v->obj.x = vertexPtr[0];
	v->obj.y = vertexPtr[1];
...
static void __glCompileElementsVarrayWrapper(__GLcontext *gc, GLint offset,
					     GLint first, GLsizei count)
{
    __GLVertArrayMachine *va = &gc->vertexArray;    
    void (*proc)(__GLcontext*, GLint, GLint, GLsizei);

    proc = GenerateCompile(gc, va->vp_size, 
...
void __glDrawVertexes_TstripWrapper(__GLcontext *gc, GLint first,
				    GLsizei count, GLuint *elements)
{
    void (*proc)(__GLcontext*, GLint, GLsizei, GLuint*);

    proc = GenerateDrawVertexes(gc, GL_TRIANGLE_STRIP, (GLboolean) (elements != 0));
    gc->vertexArray.drawVertexes[GL_TRIANGLE_STRIP] = proc;
...
void __glGenericPickVertexArrayEnables(__GLcontext *gc)
{
    ...
    */
	...
	if (!disable_og_varray) {
	    va->compileElements = __glCompileElementsVarrayWrapper;
	}
	if (!disable_og_drawvertexes) {
	    va->drawVertexes[GL_TRIANGLE_STRIP] = __glDrawVertexes_TstripWrapper;
	}
#else
	va->compileElements = __glCompileElementsVarrayWrapper;
	va->drawVertexes[GL_TRIANGLE_STRIP] = __glDrawVertexes_TstripWrapper;
...
#if defined(__GL_CODEGEN)
    {
	char *env = getenv("__GL_DISABLE_OG_VARRAY");
	if (env)
	    disable_og_varray = (GLboolean) atoi(env);

	env = getenv("__GL_DISABLE_OG_DRAWVERTEXES");
	if (env)
	    disable_og_drawvertexes = (GLboolean) atoi(env);
    }
#endif
...
#ifdef __GL_ENVVARS
    if (!disable_og_drawvertexes) {
	gc->vertexArray.drawVertexes[GL_TRIANGLE_STRIP] = __glDrawVertexes_TstripWrapper;
    } else {
...
void APIENTRY __glim_ArrayElementEXT(GLint i)
{
...
void APIENTRY __glim_DrawArraysEXT(GLenum mode,
				   GLint first,
				   GLsizei count)
{
...
void APIENTRY __gllc_ArrayElementEXT(GLint i)
{
    __glim_ArrayElement(i);
}
...
void APIENTRY __gllc_DrawArraysEXT(GLenum mode,
				   GLint first,
				   GLsizei count)
{
