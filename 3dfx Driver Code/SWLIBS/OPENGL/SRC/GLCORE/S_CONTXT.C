...
void __glSoftResetContext(__GLcontext *gc)
{
    ...
    */
    ...
    gc->constants.renderer = __glCanMmx(gc) ? "Generic/MMX" : "Generic";
#else
    gc->constants.renderer = "Generic";
#endif
    gc->constants.version = "1.1.0 SGI";
    /*
...
GLboolean __glDestroyContext(__GLcontext *gc)
{
...
#ifdef __GL_CODEGEN
    if (gc->ogState.mxconstant_pool) (*gc->imports.free)(gc, gc->ogState.mxconstant_pool);
    if (gc->ogState.fpconstant_pool) (*gc->imports.free)(gc, gc->ogState.fpconstant_pool);
    if (gc->geomOGdata.slots[0].space) (*gc->imports.free)(gc, gc->geomOGdata.slots[0].space);
    ...
    if(gc->transform.textureStack) (*gc->imports.free)(gc, gc->transform.textureStack);
