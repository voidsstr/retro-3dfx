...
void __glClipLine(__GLcontext *gc, __GLvertex *a, __GLvertex *b)
{
    ...
    */
	...
	gc->polygon.shader.modeFlags & __GL_SHADE_TEXTURE_PROJSCALED) {
	a->texture.x *= a->window.w;
	a->texture.y *= a->window.w;
	a->texture.w *= a->window.w;
	...
	gc->polygon.shader.modeFlags & __GL_SHADE_TEXTURE_PROJSCALED) {
	b->texture.x *= b->window.w;
	b->texture.y *= b->window.w;
	b->texture.w *= b->window.w;
