...
GLboolean __glCopyContext(__GLcontext *dst, const __GLcontext *src, GLuint mask)
{
	...
	dst->state.texture.scale[0] = sp->texture.scale[0];
	dst->state.texture.scale[1] = sp->texture.scale[1];
	dst->state.texture.scale[2] = sp->texture.scale[2];
	dst->state.texture.scale[3] = sp->texture.scale[3];

	dst->state.texture.bias[0] = sp->texture.bias[0];
	dst->state.texture.bias[1] = sp->texture.bias[1];
	dst->state.texture.bias[2] = sp->texture.bias[2];
	dst->state.texture.bias[3] = sp->texture.bias[3];
	...
	*/
	{
	    GLuint targetIndex;
	    const __GLperTextureState *pts, *spPts;

	    pts = dst->state.texture.texture;
	...
	dst->state.enables.texture &= ~__GL_TEXTURE_ENABLES;
	dst->state.enables.texture |=
	    sp->enables.texture & __GL_TEXTURE_ENABLES;
    }
...
void APIENTRY __glim_PushAttrib(GLuint mask)
{
	...
	if (mask & GL_TEXTURE_BIT) {
	    ...
	    sp->texture.scale[0] = gc->state.texture.scale[0];
	    sp->texture.scale[1] = gc->state.texture.scale[1];
	    sp->texture.scale[2] = gc->state.texture.scale[2];
	    sp->texture.scale[3] = gc->state.texture.scale[3];

	    sp->texture.bias[0] = gc->state.texture.bias[0];
	    sp->texture.bias[1] = gc->state.texture.bias[1];
	    sp->texture.bias[2] = gc->state.texture.bias[2];
	    sp->texture.bias[3] = gc->state.texture.bias[3];
...
void APIENTRY __glim_PopAttrib(void)
{
	...
	if (mask & GL_TEXTURE_BIT) {
	    ...
	    gc->state.texture.scale[0] = sp->texture.scale[0];
	    gc->state.texture.scale[1] = sp->texture.scale[1];
	    gc->state.texture.scale[2] = sp->texture.scale[2];
	    gc->state.texture.scale[3] = sp->texture.scale[3];

	    gc->state.texture.bias[0] = sp->texture.bias[0];
	    gc->state.texture.bias[1] = sp->texture.bias[1];
	    gc->state.texture.bias[2] = sp->texture.bias[2];
	    gc->state.texture.bias[3] = sp->texture.bias[3];
	    ...
	    */
	    ...
	    sp->texture.env = 0;
	    gc->state.enables.texture &= ~__GL_TEXTURE_ENABLES;
	    gc->state.enables.texture |=
		sp->enables.texture & __GL_TEXTURE_ENABLES;
	}
	if (mask & GL_TRANSFORM_BIT) {
...
GLboolean APIENTRY __glim_IsEnabled(GLenum cap)
{
	...
	break;
      case GL_TEXTURE_1D:
	bit = gc->state.enables.texture & __GL_TEXTURE_1D_ENABLE;
	break;
      case GL_TEXTURE_2D:
	bit = gc->state.enables.texture & __GL_TEXTURE_2D_ENABLE;
	break;
      case GL_AUTO_NORMAL:
	bit = gc->state.enables.general & __GL_AUTO_NORMAL_ENABLE;
	break;
      case GL_TEXTURE_GEN_S:
	bit = gc->state.enables.texture & __GL_TEXTURE_GEN_S_ENABLE;
	break;
      case GL_TEXTURE_GEN_T:
	bit = gc->state.enables.texture & __GL_TEXTURE_GEN_T_ENABLE;
	break;
      case GL_TEXTURE_GEN_R:
	bit = gc->state.enables.texture & __GL_TEXTURE_GEN_R_ENABLE;
	break;
      case GL_TEXTURE_GEN_Q:
	bit = gc->state.enables.texture & __GL_TEXTURE_GEN_Q_ENABLE;
	break;
