...
void __glGenericPickCalcTextureProcs(__GLcontext *gc)
{
    ...
    } else if (modeFlags & __GL_SHADE_TEXTURE_PROJSCALED) {
	gc->procs.calcTexture2 = gc->procs.calcTexture;
	if (gc->procs.calcTexture == __glCalcTexture) {
...
void __glGenericPickTextureEnables(__GLcontext *gc)
{
    ...
    if (gc->state.enables.texture & __GL_TEXTURE_2D_ENABLE) {
	__GLtexture *tex = __glLookUpTexture(gc, GL_TEXTURE_2D);

	if (__glIsTextureConsistent(gc, tex)) {
	    gc->texture.currentTexture = tex;
	    params = __glLookUpTextureParams(gc, GL_TEXTURE_2D);
	}
    } else
    if (gc->state.enables.texture & __GL_TEXTURE_1D_ENABLE) {
	__GLtexture *tex = __glLookUpTexture(gc, GL_TEXTURE_1D);
	...
	gc->texture.textureEnabled = GL_TRUE;

	/* XXX most of this should be bound into the texture param code, right? */
        gc->texture.currentTexture->params = *params;
    } else {
	gc->texture.textureEnabled = GL_FALSE;
    }
}
...
void __glGenericPickTextureProcs(__GLcontext *gc)
{
	...
	*/
	...
	mode = gc->state.texture.env[0].mode;
	switch (mode) {
	  case GL_MODULATE:
	    switch (baseFormat) {
	      case GL_LUMINANCE:
    ...
    if (gc->texture.currentTexture) {
         if (gc->texture.currentTexture->residence == __GL_TEXTURE_RESIDENCE_LOADED) 
             (*gc->texture.currentTexture->makeResident)(
		 gc, gc->texture.currentTexture,
...
void __glGenericPickAllProcs(__GLcontext *gc)
{
	...
	*/
	...
	if ((gc->texture.currentTexture->params.minFilter != GL_NEAREST) &&
	    (gc->texture.currentTexture->params.minFilter != GL_LINEAR))
	{
	    modeFlags |= __GL_SHADE_TEXTURE_MIPMAP;
	} else {
	...
	if (gc->state.hints.perspectiveCorrection != GL_FASTEST ||
	    gc->texture.currentTexture->params.minFilter !=
	    gc->texture.currentTexture->params.magFilter) {
	...
	if ((!(modeFlags & __GL_SHADE_TEXTURE) ||
	     (gc->state.texture.env[0].mode == GL_ADD)) &&
	...
	if (gc->texture.textureEnabled) {
	    ...
	    if (modeFlags & __GL_SHADE_TEXTURE_UVSCALED) {
		needs |= __GL_HAS_UVSCALED_TEXTURE;
	    }
	    if (modeFlags & __GL_SHADE_TEXTURE_PROJSCALED) {
