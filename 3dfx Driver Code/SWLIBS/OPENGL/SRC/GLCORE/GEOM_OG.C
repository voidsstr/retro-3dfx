...
/* Not yet */
#define __GL_DYNAMIC_VERTEX 0

#if __GL_DYNAMIC_VERTEX
...
#define vx(M)  \
 value(gc, offsetof(M, __GLvertex))

#define VX_SIZE (sizeof(__GLvertex))
...
/* Cache key flags */
#define OG_VERTEXES_MODE		0x0000000f
#define OG_VERTEXES_SMOOTH		0x00000010
#define OG_VERTEXES_SMOOTH_LIGHT	0x00000020
#define OG_VERTEXES_CHEAP_FOG		0x00000040	
#define OG_VERTEXES_EDGE_FLAGS  	0x00000080
#define OG_VERTEXES_ELEMENTS		0x00000100

extern void __glDoClipTriangle(__GLcontext *gc, __GLvertex *v0,
			       __GLvertex *v1, __GLvertex *v2);

void* GenerateDrawVertexes(__GLcontext *gc, GLenum mode, GLboolean useElements)
{
    struct celem
	*glDrawVertexes,
	*for_count,
	...
	*tr;
    ...
    /* Only supported modes are generated */
    assert(mode == GL_TRIANGLE_STRIP);

    /* For now, always useElements */
    useElements = GL_TRUE;

    /* Build a cache key */
    key0 = mode;
    if (gc->polygon.shader.modeFlags & __GL_SHADE_SMOOTH)
	key0 |= OG_VERTEXES_SMOOTH;
    if (gc->polygon.shader.modeFlags & __GL_SHADE_SMOOTH_LIGHT)
	key0 |= OG_VERTEXES_SMOOTH_LIGHT;
    if (gc->polygon.shader.modeFlags & __GL_SHADE_CHEAP_FOG)
	key0 |= OG_VERTEXES_CHEAP_FOG;
    if (useElements)
	key0 |= OG_VERTEXES_ELEMENTS;

    if ((gc->state.polygon.frontMode != GL_FILL) |
	(gc->state.polygon.backMode != GL_FILL)) {
	key0 |= OG_VERTEXES_EDGE_FLAGS;
    }
    key1 = (GLuint) vBuf;

    if (__glAllocCodeSpace(&gc->drawVertexesCache, key0, key1, NULL, &space)) {
      /* Found existing proc which satisfies request */
      return (void *) space;
    }
...
cb = emit_code(gc, cb, ret(gc));
    ...
    assert(cb <= space + gc->constants.drawVertexesCacheSize);
    return (void *) space;
}
				...
				struct celem* aa)
{
    ...
    red_s = gc->constants.redScale * scale;
    green_s = gc->constants.greenScale * scale;
    blue_s = gc->constants.blueScale * scale;
    alpha_s = gc->constants.alphaScale * scale;
...
cb = emit_code(gc, cb, fstp(gc, aa));
	...
	u[0].f = gc->constants.redScale;
	u[1].f = gc->constants.greenScale;
	u[2].f = gc->constants.blueScale;
	u[3].f = gc->constants.alphaScale;
...
cb = emit_code(gc, cb, fld(gc, fpk(gc, 0.0)));
	...
	/* This color is the clamped sum of
			...
			GLuint vertex_size)
{
    ...
    k[i++] = (unsigned long)gc->light.front.specTable;
...
#define OG_STATS_CHUNK 64
int og_stats_count;
int og_stats_used;
static struct og_stats_rec *og_stats;
static void print_og_stats(void);

static struct og_stats_rec *
og_stats_find_cell(int keysize, unsigned long *key)
{
    int ii;
    struct og_stats_rec *stat;

    if (0 == og_stats_count) {
	og_stats_count = OG_STATS_CHUNK;
	og_stats = (struct og_stats_rec *)
	    malloc(og_stats_count * sizeof(struct og_stats_rec));
	atexit(print_og_stats);
    }

    for (ii = 0; ii < og_stats_used; ii++) {
	stat = og_stats + ii;
	if (stat->keysize == keysize &&
	    memcmp(stat->key, key, keysize) == 0)
	    return stat;
    }

    assert(og_stats_used <= og_stats_count);
    if (og_stats_used == og_stats_count) {
	og_stats_count += OG_STATS_CHUNK;
	og_stats = (struct og_stats_rec *)
	    realloc(og_stats, og_stats_count * sizeof(struct og_stats_rec));
    }

    stat = og_stats + og_stats_used++;
    stat->keysize = keysize;
    memcpy(stat->key, key, keysize);
    stat->cache_hits = stat->cache_misses = stat->calls = 0;
...
static void print_og_stats(void)
{
	...
	for (ii = 0; ii < og_stats_used; ii++) {
	    struct og_stats_rec *stat = og_stats + ii;
	    generated += stat->cache_misses;
	    lookups += stat->cache_hits + stat->cache_misses;
	    if (stat->calls)
		called++;
	    else
		wasted += stat->cache_misses;
	}

	fprintf(f, "GenerateCompile cache statistics\n\n");
	fprintf(f, "A total of %d records were created.\n", og_stats_used);
	fprintf(f, "A total of %d pipelines were generated.\n",
		generated);
	fprintf(f, "A total of %d records were never called.\n",
		og_stats_used - called);
	fprintf(f, "A total of %d pipelines were never called, or %.2f %%\n\n",
		wasted, 100.0f * (float)wasted/(float)generated);
	fprintf(f, "Cache misses accounted for %.2f %% of lookups\n\n",
		100.0f * (float)generated/(float)lookups);

	fprintf(f, "ID\thits\tmisses\tcalls\n");
	fprintf(f, "--\t----\t------\t-----\n");
	for (ii = 0; ii < og_stats_used; ii++) {
	    struct og_stats_rec *stat = og_stats + ii;
	    fprintf(f, "%d\t%d\t%d\t%d\n", ii, stat->cache_hits,
		    stat->cache_misses, stat->calls);
	}
    }
}
#endif /* OG_STATS */
	    ...
	    key_builder make_key)
{
...
#ifdef OG_STATS
    stat = og_stats_find_cell(keysize, key);
    if (nocopy)
	stat->calls++;
#endif
    ...
    *hit = GL_FALSE;
    ...
    /* Randomly choose a cache slot to replace, but don't overwrite one
     * that is currently in use!
     */
    while (1) {
	void *ptr;
	i = rand() % GEOM_OG_NUM_SLOTS;
	ptr = god->slots[i].space;
	if (ptr != gc->vertexCache.compileElements[0] &&
	    ptr != gc->vertexCache.compileElements[1] &&
	    ptr != gc->vertexCache.compileElements[2] &&
	    ptr != gc->vertexArray.compileElementsArray)
	    break;
    }
...
void* GenerateCompile(__GLcontext *gc, GLuint vertex_size, GLuint flags)
{
  ...
  current_normal = !(vaMask & va->mask & VERTARRAY_N_MASK);
  current_color = !(vaMask & va->mask & VERTARRAY_C_MASK);
  current_texcoord = !(vaMask & va->mask & VERTARRAY_T_MASK);
  ...
  ptask = gimme_space(&hit, gc, flags, vertex_size,
#ifdef OG_STATS
		      nocopy,
#endif
		      make_key);
