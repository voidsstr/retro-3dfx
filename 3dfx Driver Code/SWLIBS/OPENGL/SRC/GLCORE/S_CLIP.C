...
#define __GL_CLIP_TEXTURE(d,a,b,t) \
	...
	if (!(a->hasAndClipCode & __GL_CLIP_MASK) && \
	    ...
	    ax = a->texture.x * a->clip.w; \
	    ay = a->texture.y * a->clip.w; \
	    az = a->texture.z * a->clip.w; \
	    aw = a->texture.w * a->clip.w; \
	} else { \
	    ax = a->texture.x; \
	    ay = a->texture.y; \
	    az = a->texture.z; \
	...
	if (!(b->hasAndClipCode & __GL_CLIP_MASK) && \
	    ...
	    bx = b->texture.x * b->clip.w; \
	    by = b->texture.y * b->clip.w; \
	    bz = b->texture.z * b->clip.w; \
	    bw = b->texture.w * b->clip.w; \
	} else { \
	    bx = b->texture.x; \
	    by = b->texture.y; \
	    bz = b->texture.z; \
