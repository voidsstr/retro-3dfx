				 ...
				 __GLfloat dx, __GLfloat dy)
{
	...
	if (modeFlags & __GL_SHADE_TEXTURE) {
	    __GLfloat s = a->texture.x;
	    __GLfloat t = a->texture.y;
	    __GLfloat q = a->texture.w;
...

{
    ...
    */
    ...
    if (modeFlags & __GL_SHADE_TEXTURE) {
	__GLfloat ax, ay, aw, bx, by, bw, cx, cy, cw;
	__GLfloat dsAC, dsBC, dtAC, dtBC, dqwAC, dqwBC;
	__GLfloat drhowAC, drhowBC, cRhow;

	ax = a->texture.x; ay = a->texture.y; aw = a->texture.w;
	bx = b->texture.x; by = b->texture.y; bw = b->texture.w;
	cx = c->texture.x; cy = c->texture.y; cw = c->texture.w;

	if ((modeFlags & __GL_SHADE_TEXTURE_PERSP) &&
	    !(modeFlags & __GL_SHADE_TEXTURE_PROJSCALED))
	{
	    ax *= a->window.w; ay *= a->window.w; aw *= a->window.w;
	    bx *= b->window.w; by *= b->window.w; bw *= b->window.w;
	    cx *= c->window.w; cy *= c->window.w; cw *= c->window.w;
	}
	...
	aRhow = (*gc->procs.calcPolygonRho)(gc, &gc->polygon.shader,
					    ax, ay, aw) * aw;
	bRhow = (*gc->procs.calcPolygonRho)(gc, &gc->polygon.shader,
					    bx, by, bw) * bw;
	cRhow = (*gc->procs.calcPolygonRho)(gc, &gc->polygon.shader,
					    cx, cy, cw) * cw;
