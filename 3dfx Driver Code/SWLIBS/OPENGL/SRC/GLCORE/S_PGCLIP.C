...
**
** $Revision: 2$
** $Date: 10/11/00 8:04:15 PM$
*/
#include <math.h>
#include "context.h"
#include "global.h"

#define COPY_HAS(dst, src) \
  (dst->hasAndClipCode = (dst->hasAndClipCode & __GL_CLIP_MASK) | (src->hasAndClipCode & ~__GL_CLIP_MASK))
		       ...
		       GLuint allClipCodes)
{
    ...
    */
	...
	for (i = nout; --i >= 0; ) {
	    __GLvertex *vx = *ovp++;
	    if (~vx->hasAndClipCode & needs) DO_VALIDATE(gc, vx, needs);
	}
	...
	*/
	...
	for (i = nout; --i >= 0; ) {
	    ...
	    */
	    ...
	    p0->window.x = winx;
	    p0->window.y = winy;
	    if ((p0->hasAndClipCode & __GL_CLIP_MASK) &&
		gc->polygon.shader.modeFlags & __GL_SHADE_TEXTURE_PROJSCALED) {
		p0->texture.x *= p0->window.w;
		p0->texture.y *= p0->window.w;
		p0->texture.w *= p0->window.w;
		p0->hasAndClipCode |= __GL_HAS_WINDOW_TEXTURE;
	    }
	}
    }
...
void __glClipPolygon(__GLcontext *gc, __GLvertex *v0, GLint nv)
{
	...
	*/
    ...
    __glDoPolygonClip(gc, &iv[0], nv, orCodes & __GL_ALL_CLIP_MASK);
}
...
void __glClipAndRenderTriangle(__GLcontext *gc,
			       ...
			       __GLvertex *v2)
{
    __GLdrawableRegion *glRegion = &gc->drawablePrivate->ownershipRegion;
    __GLregionRect *rect = &glRegion->rects[0];
    void (*rt)(__GLcontext*, __GLvertex*, __GLvertex*, __GLvertex*);

    rt = gc->procs.renderTriangle2;
	...
	for (i = 0; i < glRegion->numRects; i++) {
		...
		for (tour = 0; tour < 4; tour++) {
			...
			if ((a->outcode ^ b->outcode) & edge_flags[tour]) {
			    ...
			    if (tour < 2) {
				t = (b->v.window.x - edge) / (b->v.window.x - a->v.window.x);
				newVertex->v.window.x = edge;
				newVertex->v.window.y = 
				    b->v.window.y + t * (a->v.window.y - b->v.window.y);
			    } else {
				t = (b->v.window.y - edge) / (b->v.window.y - a->v.window.y);
			    ...
			    if (1) {
				newVertex->v.texture.x = b->v.texture.x + t * (a->v.texture.x - b->v.texture.x);
				newVertex->v.texture.y = b->v.texture.y + t * (a->v.texture.y - b->v.texture.y);
				newVertex->v.texture.w = b->v.texture.w + t * (a->v.texture.w - b->v.texture.w);
			    }

			    /* copy the boundary flag if we are entering, set it if we are leaving */
			    if (!reversed && (a->outcode & edge_flags[tour])) {
				newVertex->v.boundaryEdge = a->v.boundaryEdge;
			    } else if (reversed && (b->outcode & edge_flags[tour])) {
				newVertex->v.boundaryEdge = b->v.boundaryEdge;
			    } else {
				newVertex->v.boundaryEdge = GL_FALSE;
			    }
			    newVertex->v.hasAndClipCode = a->v.hasAndClipCode;

			    calc2DOutcode(gc, newVertex, rect);
