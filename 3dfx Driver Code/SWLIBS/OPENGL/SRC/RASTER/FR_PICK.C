...
int __glPCPickTriangleProcs(__GLcontext *gc)
{
	...
	case GL_NEAREST:
		...
		gc->procs.slowMinFilter = __glFRNearestFilter;
	    }
	    break;
	case GL_LINEAR:
	    if (modeFlags & __GL_SHADE_TEXTURE_UVSCALED) {
		...
		gc->procs.slowMinFilter = __glFRLinearFilter;
	    }
	    genericTexture = GL_TRUE;
	    break;
	...
	case GL_NEAREST:
	    if (modeFlags & __GL_SHADE_TEXTURE_UVSCALED) {
		gc->procs.slowMagFilter = __glFRNearestFilterUVScaled;
	    } else {
		gc->procs.slowMagFilter = __glFRNearestFilter;
	    }
	    break;
	case GL_LINEAR:
	    if (modeFlags & __GL_SHADE_TEXTURE_UVSCALED) {
		gc->procs.slowMagFilter = __glFRLinearFilterUVScaled;
	    } else {
		gc->procs.slowMagFilter = __glFRLinearFilter;
	    }
	    genericTexture = GL_TRUE;
	    break;
	default:
	...
	if (gc->modes.rgbMode) {
		...
		if (needAlpha | (gc->state.texture.env[0].mode == GL_DECAL)) {
		    texFlags |= __FR_TEXEL_NEED_ALPHA;
		    pixFlags |= __FR_PIXEL_TEXTURE_ALPHA;
		}
		break;
	...
	case GL_BLEND:
	    ...
	    gc->texture.envColor[0] =
		__GL_FLOAT_TO_UB(gc->state.texture.env[0].color.r *
				 gc->frontBuffer.oneOverRedScale);
	    gc->texture.envColor[1] =
		__GL_FLOAT_TO_UB(gc->state.texture.env[0].color.g *
				 gc->frontBuffer.oneOverGreenScale);
	    gc->texture.envColor[2] =
		__GL_FLOAT_TO_UB(gc->state.texture.env[0].color.b *
				 gc->frontBuffer.oneOverBlueScale);
	    gc->texture.envColor[3] =
		__GL_FLOAT_TO_UB(gc->state.texture.env[0].color.a *
				 gc->frontBuffer.oneOverAlphaScale);
	    /* If dithering, scale to component max to avoid overflow */
	    if (modeFlags & __GL_SHADE_DITHER) {
		gc->texture.envColor[0] -=
		    (gc->texture.envColor[0] >> gc->modes.redBits);
		gc->texture.envColor[1] -=
		    (gc->texture.envColor[1] >> gc->modes.greenBits);
		gc->texture.envColor[2] -=
		    (gc->texture.envColor[2] >> gc->modes.blueBits);
		if (gc->modes.alphaBits) {
		    gc->texture.envColor[3] -=
			(gc->texture.envColor[3] >> gc->modes.alphaBits);
		}
	    }
	    break;
	case GL_REPLACE:
...
#define __GL_CODEGEN_NUM 0
	...
	((modeFlags & __GL_SHADE_TEXTURE) &&
	 ...
	 (gc->texture.currentTexture &&
	  gc->texture.currentTexture->dim != 2)) ||
    ...
    if (genericSetup)
	gc->procs.fillTriangle = __glFRRenderTriangle;
    else
	gc->procs.fillTriangle = GenerateSetup(gc, usingSpanlets);
#else
    gc->procs.fillTriangle = triProcs[triFlags];
#endif
    assert(gc->procs.fillTriangle);
