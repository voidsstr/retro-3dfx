/*
 * unsign42.c - test unsigned textures: number of source components is 4,
...
TESTMOD(unsign42)
{
    ...
    for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
        ogEnvLog(5, "ubyte(%i): 0x%8x\n", j, ogLib2Ub("MODULATE", tubyte[i], tubyte[i+3], 1.0, 1.0, 1.0, 1.0));
    }
    for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
        ogEnvLog(5, "ushort(%i): 0x%8x\n", j, ogLib2Us("MODULATE", tushort[i], tushort[i+3], 1.0, 1.0, 1.0, 1.0));
    }
    for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
        ogEnvLog(5, "uint(%i): 0x%8x\n", j, ogLib2Ui("MODULATE", tuint[i], tuint[i+3], 1.0, 1.0, 1.0, 1.0));
    }
