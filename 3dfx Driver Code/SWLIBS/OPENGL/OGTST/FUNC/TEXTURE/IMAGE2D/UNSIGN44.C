/*
 * unsign44.c - test unsigned textures: number of source components is 4,
...
TESTMOD(unsign44)
{
	...
	for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
	    ogEnvLog(5, "ubyte(%i): 0x%8x\n", j, ogLib4Ub("DECAL", tubyte[i], tubyte[i+1], tubyte[i+2], tubyte[i+3], 1.0, 1.0, 1.0, 1.0));
	}
	for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
	    ogEnvLog(5, "ushort(%i): 0x%8x\n", j, ogLib4Us("DECAL", tushort[i], tushort[i+1], tushort[i+2], tushort[i+3], 1.0, 1.0, 1.0, 1.0));
	}
	for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
	    ogEnvLog(5, "uint(%i): 0x%8x\n", j, ogLib4Ui("DECAL", tuint[i], tuint[i+1], tuint[i+2], tuint[i+3], 1.0, 1.0, 1.0, 1.0));
	}
