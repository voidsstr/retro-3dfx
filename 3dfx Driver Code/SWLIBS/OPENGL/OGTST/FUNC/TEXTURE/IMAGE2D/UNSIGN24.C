/*
 * unsign24.c - test unsigned textures: number of source components is 2,
...
TESTMOD(unsign24)
{
    ...
    for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
        ogEnvLog(5, "ubyte(%i): 0x%8x\n", j, ogLib4Ub("MODULATE", tubyte[i], tubyte[i], tubyte[i], tubyte[i+1], 1.0, 1.0, 1.0, 1.0));
    }
    for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
        ogEnvLog(5, "ushort(%i): 0x%8x\n", j, ogLib4Us("MODULATE", tushort[i], tushort[i], tushort[i], tushort[i+1], 1.0, 1.0, 1.0, 1.0));
    }
    for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
        ogEnvLog(5, "uint(%i): 0x%8x\n", j, ogLib4Ui("MODULATE", tuint[i], tuint[i], tuint[i], tuint[i+1], 1.0, 1.0, 1.0, 1.0));
    }
	...
	switch( i % 8 ){
	  case 0:
	  case 6:
	    glTexImage2D(GL_TEXTURE_2D, 0, 4, TEXSIZE, TEXSIZE, 
	  ...
	  case 5:
	    ...
	    break;
	  default:
	    glTexImage2D(GL_TEXTURE_2D, 0, 4, 0, TEXSIZE, 
				0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_INT, tuint);
	    break;
	}
