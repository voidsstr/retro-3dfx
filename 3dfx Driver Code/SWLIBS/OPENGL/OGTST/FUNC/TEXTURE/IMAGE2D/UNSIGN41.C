/*
 * unsign41.c - test unsigned textures: number of source components is 4,
...
TESTMOD(unsign41)
{
    ...
    for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
        ogEnvLog(5, "ubyte(%i): 0x%8x\n", j, ogLib1Ub("MODULATE", tubyte[i], 1.0, 1.0, 1.0, 1.0));
    }
    for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
        ogEnvLog(5, "ushort(%i): 0x%8x\n", j, ogLib1Us("MODULATE", tushort[i], 1.0, 1.0, 1.0, 1.0));
    }
    for (i = 0, j = 0; i < (TEXSIZE*TEXSIZE*COMPON); i = i + COMPON, j++){
        ogEnvLog(5, "uint(%i): 0x%8x\n", j, ogLib1Ui("MODULATE", tuint[i], 1.0, 1.0, 1.0, 1.0));
    }
	...
	switch( i % 8 ){
	  ...
	  case 6:
	    glTexImage2D(GL_TEXTURE_2D, 0, 1, TEXSIZE, 
				TEXSIZE, 0, GL_RGBA, GL_UNSIGNED_BYTE, tubyte);
	    break;

	  case 1:
	  case 4:
	    glTexImage2D(GL_TEXTURE_2D, 0, 1, TEXSIZE, 
			TEXSIZE, 0, GL_RGBA, GL_UNSIGNED_SHORT, tushort);
	    break;

	  case 2:
	  case 5:
	    glTexImage2D(GL_TEXTURE_2D, 0, 1, TEXSIZE, 
				TEXSIZE, 0, GL_RGBA, GL_UNSIGNED_INT, tuint);
	    break;
	  default:
	    glTexImage2D(GL_TEXTURE_2D, 0, 1, 0, TEXSIZE, 
					0, GL_RGBA, GL_UNSIGNED_INT, tuint);
	    break;
	}
