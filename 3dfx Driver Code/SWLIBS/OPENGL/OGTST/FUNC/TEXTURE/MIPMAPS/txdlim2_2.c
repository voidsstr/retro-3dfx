/*
 * txdlim2_2.c - test combination of the display lists and immediate mode 2 
 * components texture
 */
...
TESTMOD(txdlim2_2)
{
    ...
    GLubyte *t;
    GLuint color[NUM_COLORS][COMPON] = {{0x33333333, 0xffffffff},
		{0x55555555, 0xdddddddd}, {0x88888888, 0xaaaaaaaa}, 
		{0xbbbbbbbb, 0x88888888}, {0xdddddddd, 0x55555555}, 
	...
	glTexImage2D(GL_TEXTURE_2D, lod, 2, size_x, size_y,
			0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, texture[lod]);
	size_x = size_x >> 1;
    ...
    for (i = 0; i <TEXSIZE_0_Y; i ++){
      for(j = 0; j < TEXSIZE_0_X; j ++){
        if (j < TEXSIZE_0_X_4){
           t0[i*TEXSIZE_0_X*COMPON + j*COMPON] = color[0][0];
           t0[i*TEXSIZE_0_X*COMPON + j*COMPON + 1] = color[0][1];
        }else if (j < (TEXSIZE_0_X_4*2)){
           t0[i*TEXSIZE_0_X*COMPON + j*COMPON] = color[1][0];
           t0[i*TEXSIZE_0_X*COMPON + j*COMPON + 1] = color[1][1];
        }else if (j < (TEXSIZE_0_X_4*3)){
           t0[i*TEXSIZE_0_X*COMPON + j*COMPON] = color[2][0];
           t0[i*TEXSIZE_0_X*COMPON + j*COMPON + 1] = color[2][1];
        }else{
           t0[i*TEXSIZE_0_X*COMPON + j*COMPON] = color[3][0];
           t0[i*TEXSIZE_0_X*COMPON + j*COMPON + 1] = color[3][1];
        }
      }
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, 2, TEXSIZE_0_X, 
		TEXSIZE_0_Y, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, t0);
    drawLevels();
