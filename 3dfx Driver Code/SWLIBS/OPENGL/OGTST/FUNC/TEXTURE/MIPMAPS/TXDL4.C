/*
 * txdl4.c - test display list mode 4 components texture
 */
...
static GLubyte t0[TEXSIZE_0*TEXSIZE_0*COMPON], t1[TEXSIZE_1*TEXSIZE_1*COMPON];
static GLubyte t2[TEXSIZE_2*TEXSIZE_2*COMPON], t3[TEXSIZE_3*TEXSIZE_3*COMPON];
static GLubyte t4[TEXSIZE_4*TEXSIZE_4*COMPON], t5[TEXSIZE_5*TEXSIZE_5*COMPON];
static GLubyte t6[TEXSIZE_6*TEXSIZE_6*COMPON], t7[TEXSIZE_7*TEXSIZE_7*COMPON];
static GLubyte t8[TEXSIZE_8*TEXSIZE_8*COMPON], t9[TEXSIZE_9*TEXSIZE_9*COMPON];
static GLubyte t10[TEXSIZE_10*TEXSIZE_10*COMPON];
static GLubyte t11[TEXSIZE_11*TEXSIZE_11*COMPON];
static GLubyte t12[TEXSIZE_12*TEXSIZE_12*COMPON];
static GLubyte *tplain[13] = {t0,t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12};
...
TESTMOD(txdl4)
{
...
#ifdef WIN32
    GLuint *tpattern =
	(GLuint *) malloc(TEXSIZE_0*TEXSIZE_0*COMPON*sizeof(GLuint));
#else
    GLuint tpattern[TEXSIZE_0*TEXSIZE_0*COMPON];
#endif
    GLubyte *texel;
    GLuint color[NUM_COLORS][COMPON] = {{ONE, 0, 0, A_1}, {0, ONE, 0, A_2}, 
				{0, 0, ONE, A_3}, {ONE, ONE, 0, A_4}, 
				{0, ONE, ONE, A_5}, {ONE, 0, ONE, A_6}};
	...
	switch((s + t) % 4){
	  case 0:
            ...
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 3] = A_1;
	    break;
	  case 1:
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON] = ONE;
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 1] = 0;
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 2] = 0;
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 3] = A_2;
	    break;
	  case 2:
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON] = 0;
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 1] = ONE;
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 2] = 0;
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 3] = A_3;
	    break;
	  case 3:
	    tpattern[i*TEXSIZE_0*COMPON + j*COMPON] = 0;
	    tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 1] = 0;
	    tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 2] = ONE;
	    tpattern[i*TEXSIZE_0*COMPON + j*COMPON + 3] = A_4;
	    break;
        }
      }
    }
    ...
    for ( lod = 0, size = TEXSIZE_0; size > 0; size >>= 1, lod ++ )
	glTexImage2D(GL_TEXTURE_2D, lod, 4, size, size, 0, GL_RGBA, 
						GL_UNSIGNED_BYTE, tplain[lod]);
    glEndList();

    glNewList(2, GL_COMPILE);
    gluBuild2DMipmaps(GL_TEXTURE_2D, 4, TEXSIZE_0, TEXSIZE_0, 
					GL_RGBA, GL_UNSIGNED_INT, tpattern);
    glEndList();
...
static void drawLevels(GLboolean transRight)
{
    ...
    glFinish(); /* temporary workaround for RE3 */
}
