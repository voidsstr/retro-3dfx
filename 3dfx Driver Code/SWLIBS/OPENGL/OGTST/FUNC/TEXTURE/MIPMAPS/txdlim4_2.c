/*
 * txdlim4_2.c - test combination of the display lists and immediate mode 4 
 * components texture
 */
...
TESTMOD(txdlim4_2)
{
	...
	glTexImage2D(GL_TEXTURE_2D, lod, 4, size_x, size_y, 
				0, GL_RGBA, GL_UNSIGNED_BYTE, texture[lod]);
	size_x = size_x >> 1;
    ...
    for ( lod = 1; majer > 0; majer >>= 1, lod ++ ){
        ...
        glTexImage2D(GL_TEXTURE_2D, lod, 4, size_x, size_y, 0,
                                GL_RGBA, GL_UNSIGNED_BYTE, texture[lod]);
        size_x = size_x >> 1;
        if (size_x == 0) size_x = 1;
        size_y = size_y >> 1;
        if (size_y == 0) size_y = 1;
    ...
    for (i = 0; i <TEXSIZE_0_Y; i ++){
      for(j = 0; j < TEXSIZE_0_X; j ++){
        if (j < TEXSIZE_0_X_4){
           ...
           t0[i*TEXSIZE_0_X*COMPON + j*COMPON + 3] = ONE;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, 4, TEXSIZE_0_X, TEXSIZE_0_Y, 0, 
