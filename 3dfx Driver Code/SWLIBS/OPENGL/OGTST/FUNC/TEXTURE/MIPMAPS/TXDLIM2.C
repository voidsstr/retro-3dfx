/*
 * txdlim2.c - test combination of the display list and immediate mode 2 
 * components texture
 */
...
#define NUM_COLORS	6
static GLuint color[NUM_COLORS][COMPON] = {{0x33333333, 0xffffffff},
{0x55555555, 0xdddddddd}, {0x88888888, 0xaaaaaaaa}, {0xbbbbbbbb, 0x88888888},
{0xdddddddd, 0x55555555}, {0xffffffff, 0x33333333}};
...
TESTMOD(txdlim2)
{
	...
	glTexImage2D(GL_TEXTURE_2D, lod, 2, size_x, 
		size_y, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, tpattern[lod]);
	size_x = size_x >> 1;
...
static void defineImLevels(GLint firstLevel)
{
	...
	glTexImage2D(GL_TEXTURE_2D, lod, 2, size_x, 
			size_y, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_INT, tplain);
	size_x = size_x >> 2;
	if (size_x == 0) size_x = 1;
	size_y = size_y >> 2;
	if (size_y == 0) size_y = 1;
