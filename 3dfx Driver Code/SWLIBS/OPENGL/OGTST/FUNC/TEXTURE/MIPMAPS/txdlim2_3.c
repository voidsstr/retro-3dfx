/*
 * txdlim2_3.c - test combination of the display list and immediate mode 2
 * components texture
 */
					...
					GLboolean oneDlist,GLint dlistIdStep)
{
	...
	glTexImage2D(GL_TEXTURE_2D, lod, 2, size_x, size_y,
			 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, tplain);
	size_x = size_x >> 2;
	if (size_x == 0) size_x = 1;
	size_y = size_y >> 2;
	if (size_y == 0) size_y = 1;
	...
	glTexImage2D(GL_TEXTURE_2D, lod, 2, size_x, size_y,
			 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_INT, tpattern[lod]);
