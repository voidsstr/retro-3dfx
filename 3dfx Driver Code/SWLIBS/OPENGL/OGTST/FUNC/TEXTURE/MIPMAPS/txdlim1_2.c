/*
 * txdlim1_2.c - test combination of the display lists and immediate mode 1
 * components texture
 */
...
TESTMOD(txdlim1_2)
{
    ...
    GLubyte *t;
    GLuint color[NUM_COLORS] = {0x33333333, 0x55555555, 0x88888888, 
				0xbbbbbbbb, 0xdddddddd, 0xffffffff};
	...
	for (i = 0, t = texture[lod]; i < size_x * size_y; i ++, t += COMPON){
		...
		if (i < (size_x * (size_y >> 1)))
		    *t = color[lod % NUM_COLORS];
		else 
		    *t = color[(lod + 1) % NUM_COLORS];
	    }else{
		if (i < (size_x * (size_y >> 1)))
			*t = color[(lod + 2) % NUM_COLORS];
		else 
			*t = color[(lod + 3) % NUM_COLORS];
	    }
	}
	glTexImage2D(GL_TEXTURE_2D, lod, 1, size_x, size_y,
			0, GL_LUMINANCE, GL_UNSIGNED_BYTE, texture[lod]);
	size_x = size_x >> 1;
    ...
    for (i = 0; i <TEXSIZE_0_Y; i ++){
      for(j = 0; j < TEXSIZE_0_X; j ++){
        if (j < TEXSIZE_0_X_4){
           t0[i*TEXSIZE_0_X + j] = color[0];
        }else if (j < (TEXSIZE_0_X_4*2)){
           t0[i*TEXSIZE_0_X + j] = color[1];
        }else if (j < (TEXSIZE_0_X_4*3)){
           t0[i*TEXSIZE_0_X + j] = color[2];
        }else{
           t0[i*TEXSIZE_0_X + j] = color[3];
        }
      }
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, 1, TEXSIZE_0_X, 
		TEXSIZE_0_Y, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, t0);
    drawLevels();
