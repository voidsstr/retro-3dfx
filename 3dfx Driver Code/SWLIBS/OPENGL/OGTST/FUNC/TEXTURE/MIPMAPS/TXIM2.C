/*
 * txim2.c - test immediate mode 2 components texture
 */
...
static GLint tpattern [TEXSIZE_0*TEXSIZE_0*COMPON];
...
TESTMOD(txim2)
{
	...
	/* init texture */
    for ( lod = 0, size = TEXSIZE_0; size > 0; size /= 2, lod ++ ){
    ...
    errorCode = gluBuild2DMipmaps(GL_TEXTURE_2D, 2, TEXSIZE_0, 
				  TEXSIZE_0, GL_LUMINANCE_ALPHA, 
				  GL_INT, tpattern);
    if (errorCode != 0)
	ogEnvLog (OG_LFAIL, "Error in gluBuild2DMipmaps: %s\n", 
...
static void drawLevels(GLboolean transRight)
{
	...
	if (transRight)
	    glTranslatef((size * 2) + 5.0, 0.0, 0.0);
	else
