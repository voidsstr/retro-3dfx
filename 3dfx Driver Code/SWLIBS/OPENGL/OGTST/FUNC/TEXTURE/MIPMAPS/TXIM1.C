/*
 * txim1.c - test immediate mode 1 components texture
 */
...
static GLshort t0[TEXSIZE_0*TEXSIZE_0*COMPON], t1[TEXSIZE_1*TEXSIZE_1*COMPON];
static GLshort t2[TEXSIZE_2*TEXSIZE_2*COMPON], t3[TEXSIZE_3*TEXSIZE_3*COMPON];
static GLshort t4[TEXSIZE_4*TEXSIZE_4*COMPON], t5[TEXSIZE_5*TEXSIZE_5*COMPON];
static GLshort t6[TEXSIZE_6*TEXSIZE_6*COMPON], t7[TEXSIZE_7*TEXSIZE_7*COMPON];
static GLshort t8[TEXSIZE_8*TEXSIZE_8*COMPON], t9[TEXSIZE_9*TEXSIZE_9*COMPON];
static GLshort t10[TEXSIZE_10*TEXSIZE_10*COMPON];
static GLshort t11[TEXSIZE_11*TEXSIZE_11*COMPON];
static GLshort t12[TEXSIZE_12*TEXSIZE_12*COMPON];
static GLshort *tplain[13] = {t0,t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12};

static GLfloat tpattern[TEXSIZE_0*TEXSIZE_0*COMPON];
...
TESTMOD(txim1)
{
	...
	switch((s + t) % 4){
	  case 0:
	    ...
	    break;
	  case 1:
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON] = 0.7;
	    break;
	  case 2:
            tpattern[i*TEXSIZE_0*COMPON + j*COMPON] = 0.4;
	    break;
	  case 3:
	    tpattern[i*TEXSIZE_0*COMPON + j*COMPON] = 0.2;
	    break;
        }
      }
    }
    ...
    for ( lod = 0, size = TEXSIZE_0; size > 0; size >>= 1, lod ++ )
	glTexImage2D(GL_TEXTURE_2D, lod, 1, size, size, 
			0, GL_LUMINANCE, GL_SHORT, tplain[lod]);
    drawLevels(GL_TRUE);

    glLoadIdentity();
    glTranslatef(width - TEXSIZE_0 - 20.0, height - TEXSIZE_0 - 20.0, -10.0);

    gluBuild2DMipmaps(GL_TEXTURE_2D, 1, TEXSIZE_0, TEXSIZE_0, 
					GL_LUMINANCE, GL_FLOAT, tpattern);
    drawLevels(GL_FALSE);
