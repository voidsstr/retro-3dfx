...
static void  poly_box(int,int,int,int);
static void extras(void);
void random_poly(void);
...
TESTMOD(polys)
{
	...
	glBegin(GL_POLYGON);	/* draw nothing */
	glEnd();
	ogEnvLog(1, "glEnd(); \n");
	...
	ogEnvLog(1, "--- triangle as poly test---\n");
	START_DL_OR_IM(1);
	...
	for (i=0; i<10; i++) {
	    ogEnvLog(1, "---many vertex poly test---\n");
            ogLibClear(0) ;
	    maxcolor = ogLibColor() ;
	    do {
		x1 = 2 + ogLibIntRand(0,xmax - 4);
        ...
        /* don't test the result, just make sure it doesn't crash */
	for (i=0; i < 5; i++) {
	    ogEnvLog(1,"---try to crash the pipe---\n");
	    ogLibClear(0);
	    random_poly();
	}	
    }
...
static void
poly_box(int x0,int y0,int x1,int y1)
{
    int n,i;
    float x,y,xf,yf;
...
random_poly(void)
{
    ...
    n = ogLibIntRand(257,2049);
    glBegin(GL_POLYGON);
        
    for (i=0; i<n; i++) {
	ogLibColor() ;
	ogLibSetVertex(ogLibIntRand(0,639),ogLibIntRand(0,639),0,1);
    }
    glEnd();
}
