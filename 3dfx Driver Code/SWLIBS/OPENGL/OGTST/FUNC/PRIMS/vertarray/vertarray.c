...
/* 
 * This test verifies vertex array, including DrawArrays,
 * ArrayElement
 */
...
static void print_type(const char *name, GLenum type, int enabled) 
{
  ...
  case GL_DOUBLE: ctype = "DOUBLE"; break;
  default:
    assert(0);
    break;
  }
...
{
  if (!enabled) return;
  ogEnvLog(1, "GL_%s_ARRAY_STRIDE:  %d\n", name, stride);
}
  ...
  if (IS_ONEONE()) {
     ...
     *enumbers = (unsigned char *)en;
     ...
     while (pass--) {
       ...
       TEST_ENABLE(GL_VERTEX_ARRAY, FALSE);
       TEST_ENABLE(GL_NORMAL_ARRAY, FALSE);
       TEST_ENABLE(GL_COLOR_ARRAY, FALSE);
       TEST_ENABLE(GL_INDEX_ARRAY, FALSE);
       TEST_ENABLE(GL_TEXTURE_COORD_ARRAY, FALSE);
       TEST_ENABLE(GL_EDGE_FLAG_ARRAY, FALSE);
	...
	* we want to exercise them. */
	 ...
	 *np = nnumbers, *tp = tnumbers, *ep = enumbers;
	 ...
	 assert(ep <= enumbers + sizeof(en));
       ...
       if (venab) glEnable(GL_VERTEX_ARRAY);
       nenab = maybe_enable(GL_NORMAL_ARRAY);
       if (rgbMode) {
	 cenab = maybe_enable(GL_COLOR_ARRAY);
	 ienab = 0;
       } else {
	 ienab = maybe_enable(GL_INDEX_ARRAY);
	 cenab = 0;
       }
       tenab = maybe_enable(GL_TEXTURE_COORD_ARRAY);
       if (!float_only) eenab = maybe_enable(GL_EDGE_FLAG_ARRAY);
       else {
	 eenab = 0;
	...
	* the default setup is correct. */
       if (venab) {
	   glVertexPointer(vsize, vtype, vstride, (GLvoid *)vnumbers);
       } else if (ogLibBitRand(1)) {
	   glVertexPointer(vsize, vtype, vstride, (GLvoid *)0);
       }
       if (nenab) {
	   glNormalPointer(ntype, nstride, (GLvoid *)nnumbers);
       } else if (ogLibBitRand(1)) {
	   glNormalPointer(ntype, nstride, (GLvoid *)0);
       }
       if (cenab) {
	   glColorPointer(csize, ctype, cstride, (GLvoid *)cnumbers);
       } else if (ogLibBitRand(1)) {
	   glColorPointer(csize, ctype, cstride, (GLvoid *)0);

       }
       if (ienab) {
	   glIndexPointer(itype, istride, (GLvoid *)inumbers);
       } else if (ogLibBitRand(1)) {
	   glIndexPointer(itype, istride, (GLvoid *)0);
       }
       if (tenab) {
	   glTexCoordPointer(tsize, ttype, tstride, (GLvoid *)tnumbers);
       } else if (ogLibBitRand(1)) {
	   glTexCoordPointer(tsize, ttype, tstride, (GLvoid *)0);
       }
	   glEdgeFlagPointer(estride, enumbers);

       glPointSize(3);
       START_DL_OR_IM(1);
       ogLibClear(0); /*glClear(GL_COLOR_BUFFER_BIT); */
       glBegin(GL_POINTS);
       ogEnvLog(1, "glArrayElement(%d)\n", elem);
       glArrayElement(elem);
       glEnd();
       FINIS_DL_OR_IM(1);
       glPointSize(1);
	 ...
	 do_verify_floatv("CURRENT_INDEX", GL_CURRENT_INDEX, &set_index, 1);
       ...
       START_DL_OR_IM(1);
       ogEnvLog(1, "glDrawArrays(GL_POINTS, %d, 0)\n", first);
       glDrawArrays(GL_POINTS, first, 0);
       FINIS_DL_OR_IM(1);

       glPointSize(3);

       START_DL_OR_IM(1);
       ogEnvLog(1, "glDrawArrays(GL_POINTS, %d, %d)\n", first, count);
	...
	* undefined if something is enabled, don't check that */
	 ...
	 do_verify_bool("EDGE_FLAG", GL_EDGE_FLAG, set_edge);
       ...
       /* test get commands */
       if (venab) {
	   do_verify_int("vsize", GL_VERTEX_ARRAY_SIZE, vsize, 0);
	   do_verify_int("vtype", GL_VERTEX_ARRAY_TYPE, vtype, 1);
	   do_verify_int("vstride", GL_VERTEX_ARRAY_STRIDE, vstride, 0);
	   do_verify_pointer("vpointer", GL_VERTEX_ARRAY_POINTER, 
			     (GLvoid *)vnumbers);
       }

       if (nenab) {
	   do_verify_int("ntype", GL_NORMAL_ARRAY_TYPE, ntype, 1);
	   do_verify_int("nstride", GL_NORMAL_ARRAY_STRIDE, nstride, 0);
	   do_verify_pointer("npointer", GL_NORMAL_ARRAY_POINTER, nnumbers);
       }

       if (cenab) {
	   do_verify_int("csize", GL_COLOR_ARRAY_SIZE, csize, 0);
	   do_verify_int("ctype", GL_COLOR_ARRAY_TYPE, ctype, 1);
	   do_verify_int("cstride", GL_COLOR_ARRAY_STRIDE, cstride, 0);
	   do_verify_pointer("npointer", GL_COLOR_ARRAY_POINTER, 
			     (GLvoid *)cnumbers);
       }

       if (ienab) {
	   do_verify_int("itype", GL_INDEX_ARRAY_TYPE, itype, 1);
	   do_verify_int("istride", GL_INDEX_ARRAY_STRIDE, istride, 0);
	   do_verify_pointer("ipointer", GL_INDEX_ARRAY_POINTER,
			     (GLvoid *)inumbers);
       }

       if (tenab) {
	   do_verify_int("tsize", GL_TEXTURE_COORD_ARRAY_SIZE, tsize, 0);
	   do_verify_int("ttype", GL_TEXTURE_COORD_ARRAY_TYPE, ttype, 1);
	   do_verify_int("tstride", GL_TEXTURE_COORD_ARRAY_STRIDE, tstride, 
			 0);
	   do_verify_pointer("tpointer", GL_TEXTURE_COORD_ARRAY_POINTER,
			     tnumbers);
       }

       if (eenab) {
	   do_verify_int("estride", GL_EDGE_FLAG_ARRAY_STRIDE, estride, 0);
	   do_verify_pointer("epointer", GL_EDGE_FLAG_ARRAY_POINTER,
			     enumbers);
       }
	...
	* see it things worked. */
	 ...
	 START_DL_OR_IM(1);
	 ogEnvLog(1, "glDrawArrays(%d, %d, 4)\n", mode, corner);
	 glDrawArrays(mode, corner, 4);
	 FINIS_DL_OR_IM(1);
