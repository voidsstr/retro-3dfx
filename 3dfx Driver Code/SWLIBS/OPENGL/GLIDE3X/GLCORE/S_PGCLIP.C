...
#define COPY_HAS(dst, src) \
  (dst->hasAndClipCode = (dst->hasAndClipCode & __GL_ALL_CLIP_MASK) | (src->hasAndClipCode & ~__GL_ALL_CLIP_MASK))
    ...
    if (allClipCodes) {
        ...
        */
        ...
        for (i = nout; --i >= 0; ) {
            ...
            if ((p0->hasAndClipCode & __GL_HAS_WINDOW)) {
              /* only vertices added during clipping don't have window coords yet */
              continue;
            }
            ...
            p0->sst.x = x * vpXScale * wInv + vpXCenter;
            p0->sst.y = y * vpYScale * wInv + vpYCenter;
            p0->sst.ooz = z * vpZScale * wInv + vpZCenter;
            p0->window.w = wInv;

            p0->sst.r = p0->colors[__GL_FRONTFACE].r;
            p0->sst.g = p0->colors[__GL_FRONTFACE].g;
            p0->sst.b = p0->colors[__GL_FRONTFACE].b;
            p0->sst.a = p0->colors[__GL_FRONTFACE].a;
#else
            p0->window.x = x * vpXScale * wInv + vpXCenter;
            p0->window.y = y * vpYScale * wInv + vpYCenter;
            p0->window.z = z * vpZScale * wInv + vpZCenter;
            p0->window.w = wInv;
#endif
#else
            /* sst doesn't need this - the clip window handles it */
...
#endif
#ifdef GLINT
            p0->fixed.y = __GL_COORD_FLOAT_TO_FIXED(p0->window.y);
#endif
            if ((p0->hasAndClipCode & __GL_ALL_CLIP_MASK) &&
                (p0->hasAndClipCode & __GL_HAS_WINDOW_TEXTURE)) {
#if __GL_SST_GLIDE_VTX
                p0->sst.tmuvtx[0].s = p0->texture.x * p0->window.w * gc->texture.sst.w;
                p0->sst.tmuvtx[0].t = p0->texture.y * p0->window.w * gc->texture.sst.h;
                p0->sst.oow = p0->window.w;
#else
                p0->texture[0].x *= p0->window.w;
                p0->texture[0].y *= p0->window.w;
                p0->texture[0].w *= p0->window.w;
                p0->texture[1].x *= p0->window.w;
                p0->texture[1].y *= p0->window.w;
                p0->texture[1].w *= p0->window.w;
#endif
            }
        }
    }
