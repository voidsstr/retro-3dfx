...
#ifdef DEBUG
	ztest = floor((offsetZtest + .5) / pow(2.,(GLdouble)numFracBits));
#endif
...
#endif
	    z += dzdx[i];
	    zfb = (__GLzValue *) ((GLubyte *) zfb + gc->depthBuffer.buf.elementSize);
#ifdef __GL_STIPPLE_MSB
	    bit >>= 1;
#else
	    bit <<= 1;
...
{
    __GLcolor *cp;
    __GLfloat s, t, qw, rhow, rho;
    __GLfloat sw, tw, qwinv;
    GLint w;	
...
GLboolean __glTextureStippledSpan(__GLcontext *gc)
{
    ...
    s = gc->polygon.shader.frag.s;
    t = gc->polygon.shader.frag.t;
    qwinv = gc->polygon.shader.frag.qw; /* frag.qw in actually inverse */
    rhow = gc->polygon.shader.frag.rhow;
...
/*
** Index test span uses a lookup table to do the index test function.
** Output a stipple with 1's where the test passed, and 0's where the
** test failed.
*/
GLboolean __glIndexTestSpan(__GLcontext *gc)
{
    GLubyte *itft;
    GLint failed, count;
    __GLstippleWord bit, outMask, *osp;
    ...
    itft = &gc->frontBuffer.indexTestFuncTable[0];
    cp = gc->polygon.shader.colors;
    osp = gc->polygon.shader.stipplePat;
    failed = 0;
    while (w) {
	...
	bit = __GL_STIPPLE_SHIFT(0);
	while (--count >= 0) {
	    GLint ii = (GLint)(cp->r);  /* red holds index */
	    if (!itft[ii]) {
		/* Test failed */
		outMask &= ~bit;
		failed++;
...
/*
** Stippled form of index test span that checks the stipple at each
** pixel and avoids the test where the stipple disallows it.
*/
GLboolean __glIndexTestStippledSpan(__GLcontext *gc)
{
    GLubyte *itft;
    GLint count, failed;
    __GLstippleWord bit, inMask, outMask, *isp;
