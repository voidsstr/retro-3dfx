...
/*
** Store fragment proc.
** index test on, stencil test on, depth test on
*/
void __glDoStore_SDI(__GLcolorBuffer *cfb, const __GLfragment *frag)
{
    __GLcontext *gc;
    GLint x, y;
    ...
    if (!gc->frontBuffer.indexTestFuncTable[(GLint) (frag->color.r)]) {
	/* index test failed */
	return;
    }
    if (!(*gc->stencilBuffer.testFunc)(&gc->stencilBuffer, x, y)) {
...
/*
** Store fragment proc.
** index test on, stencil test on, depth test off
*/
void __glDoStore_SI(__GLcolorBuffer *cfb, const __GLfragment *frag)
{
    __GLcontext *gc;
    GLint x, y;
...
/*
** Store fragment proc.
** index test on, stencil test off, depth test on
*/
void __glDoStore_DI(__GLcolorBuffer *cfb, const __GLfragment *frag)
{
    __GLcontext *gc;
    GLint x, y;
...
/*
** Store fragment proc.
** index test on, stencil test off, depth test off
*/
void __glDoStore_I(__GLcolorBuffer *cfb, const __GLfragment *frag)
{
    __GLcontext *gc;
    GLint x, y;
...
static StoreProc storeProcs[16] = {
    &__glDoStore,
    &__glDoStore_A,
    &__glDoStore_S,
    &__glDoStore_AS,
...
void __glGenericPickStoreProcs(__GLcontext *gc)
{
    ...
    if ((modeFlags & __GL_SHADE_INDEX_TEST) && (!gc->modes.rgbMode)) {
	ix |= 8;
    }
