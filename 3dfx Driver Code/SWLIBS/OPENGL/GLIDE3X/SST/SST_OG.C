...
cb = emit_code(gc, cb, lea(gc, esi, mem(gc, vx(normal.x), edi, NULL, 1)));
            ...
            if (vaMask & VERTARRAY_T_MASK) {
                if (current_texcoord) {
cb = emit_code(gc, cb, mov(gc, eax, mem(gc, value(gc, (int)&gc->state.current.texture[0].x), NULL, NULL, 1)));
cb = emit_code(gc, cb, mov(gc, mem(gc, vx(texture.x), edi, NULL, 1), eax));
cb = emit_code(gc, cb, mov(gc, eax, mem(gc, value(gc, (int)&gc->state.current.texture[0].y), NULL, NULL, 1)));
cb = emit_code(gc, cb, mov(gc, mem(gc, vx(texture.y), edi, NULL, 1), eax));
cb = emit_code(gc, cb, mov(gc, eax, mem(gc, value(gc, (int)&gc->state.current.texture[0].z), NULL, NULL, 1)));
cb = emit_code(gc, cb, mov(gc, mem(gc, vx(texture.z), edi, NULL, 1), eax));
cb = emit_code(gc, cb, mov(gc, eax, mem(gc, value(gc, (int)&gc->state.current.texture[0].w), NULL, NULL, 1)));
...
cb = emit_code(gc, cb, fmul(gc, mem(gc, vx(texture.x), edi, NULL, 1)));
cb = emit_code(gc, cb, fmul(gc, mem(gc, value(gc, (int)&(gc->texture.sst.w)), NULL, NULL, 1)));
cb = emit_code(gc, cb, fstp(gc, mem(gc, vx(sst.tmuvtx[0].s), edi, NULL, 1)));
cb = emit_code(gc, cb, fld(gc, st0));
cb = emit_code(gc, cb, fmul(gc, mem(gc, vx(texture.y), edi, NULL, 1)));
cb = emit_code(gc, cb, fmul(gc, mem(gc, value(gc, (int)&(gc->texture.sst.h)), NULL, NULL, 1)));
cb = emit_code(gc, cb, fstp(gc, mem(gc, vx(sst.tmuvtx[0].t), edi, NULL, 1)));
cb = emit_code(gc, cb, fmul(gc, mem(gc, vx(texture.w), edi, NULL, 1)));
cb = emit_code(gc, cb, fstp(gc, mem(gc, vx(sst.oow), edi, NULL, 1)));
#else
...
cb = emit_code(gc, cb, fstp(gc, windoww));
            ...
            if ((modeFlags & __GL_SHADE_TEXTURE) &&
                (modeFlags & __GL_SHADE_TEXTURE_PERSP)) {
#if __GL_SST_GLIDE_VTX
cb = emit_code(gc, cb, fld(gc, mem(gc, vx(texture.x), edi, NULL, 1)));
cb = emit_code(gc, cb, fmul(gc, mem(gc, value(gc, (int)&(gc->texture.sst.w)), NULL, NULL, 1)));
cb = emit_code(gc, cb, fstp(gc, mem(gc, vx(sst.tmuvtx[0].s), edi, NULL, 1)));
cb = emit_code(gc, cb, fld(gc, mem(gc, vx(texture.y), edi, NULL, 1)));
cb = emit_code(gc, cb, fmul(gc, mem(gc, value(gc, (int)&(gc->texture.sst.h)), NULL, NULL, 1)));
cb = emit_code(gc, cb, fstp(gc, mem(gc, vx(sst.tmuvtx[0].t), edi, NULL, 1)));
cb = emit_code(gc, cb, mov(gc, mem(gc, vx(sst.oow), edi, NULL, 1), value(gc, FP_ONE)));
#else
...
cb = emit_code(gc, cb, fxch(gc, st1));
#if __GL_SST_GLIDE_VTX
cb = emit_code(gc, cb, fstp(gc, mem(gc, vx(sst.x), edi, NULL, 1)));
#else
...
#if __GL_SST_GLIDE_VTX
cb = emit_code(gc, cb, fstp(gc, mem(gc, vx(sst.ooz), edi, NULL, 1)));
cb = emit_code(gc, cb, fstp(gc, mem(gc, vx(sst.y), edi, NULL, 1)));
#else
