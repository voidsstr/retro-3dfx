...
static void print_key(unsigned long *k) {
    ...
    fprintf(__gl_debug_log, "%8x %2d %2d %8x %1d %2d %4x %1d %1d %8x %1d %8x %4x %1d %1d\n",
			...
			unsigned long mflags)
{
    ...
    mk[0] = mflags | gc->ogKey;
    mk[1] = (unsigned long) gc->transform.modelView;
    mk[2] = gc->transform.modelView->mvp.matrixType;
	    ...
	    key_builder make_key)
{
    ...
    mk[0] = gc->ogKey;
    mk[0] |= __GL_GEOM_OG_KEY_FLAG_NOCOPY;
    mk[0] |= ((vertex_size - 2) << __GL_GEOM_OG_KEY_VXSIZE_SHIFT);
    mk[1] = (unsigned long) gc->transform.modelView;
    mk[2] = gc->transform.modelView->mvp.matrixType;
    ...
    *hit = GL_FALSE;
    ...
    return &god->slots[i];
#else    
    keysize = (*make_key)(key, gc, flags, vertex_size, mk, mflag);
...
void* __glSSTImmedGenerateCompile(__GLcontext *gc, GLuint vertex_size, GLuint flags)
{
  __GLtransform *tr;
...
cb = emit_code(gc, cb, or(gc, edi, value(gc, __GL_HAS_WINDOW)));
	if (nocopy)
