...
/* XXXwheeler: snap and unsnap so that the rounding matches previous work,
   and conform is happy */
#define OLD_SNAP ((float)(3<<18))
    vx->window.x += OLD_SNAP;
    vx->window.y += OLD_SNAP;
    /*
    ...
    */
    ...
    if (pointSize & 1) {
	/* odd point size */
	xLeft = (((GLint)(vx->window.x-OLD_SNAP)) - pointSizeHalf);
	yBottom = (((GLint)(vx->window.y-OLD_SNAP)) - pointSizeHalf);
    } else {
	/* even point size */
	xLeft = (((GLint)(vx->window.x-OLD_SNAP+__glHalf)) - pointSizeHalf);
	yBottom = (((GLint)(vx->window.y-OLD_SNAP+__glHalf)) - pointSizeHalf);
    }
...
#define OLD_SNAP ((float)(3<<18))
    ...
    prevx = prevy = 0.0f;
    vtxA.x = vx->window.x-OLD_SNAP;
    vtxA.y = vx->window.y-OLD_SNAP;
    ...
    */
    ...
    for (i=0; i <= n; i++) {
        ...
        x = (vx->window.x + radius * __GL_COSF(ang))-OLD_SNAP;
        y = (vx->window.y + radius * __GL_SINF(ang))-OLD_SNAP;
        vtxB.x = prevx;
        vtxB.y = prevy;
        vtxC.x = x;
        vtxC.y = y;
