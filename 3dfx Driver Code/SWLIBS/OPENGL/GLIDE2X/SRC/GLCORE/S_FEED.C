...
**
** $Revision: 4$
** $Date: 10/11/00 7:51:11 PM$
*/
#include "context.h"
#include "g_imfncs.h"
#include "global.h"

void APIENTRY __glim_FeedbackBuffer(GLsizei bufferLength, GLenum type, GLfloat buffer[])
{
...
void APIENTRY __glim_PassThrough(GLfloat element)
{
    __GL_SETUP_NOT_IN_BEGIN();
    __GL_API_BLAND();
...
static void feedback(__GLcontext *gc, __GLvertex *v)
{
...
#if __GL_SST
    __GLfloat voffset = 0.0f;
#else
    __GLfloat voffset = __glHalf;
#endif

    switch (type) {
      case GL_2D:
        __glFeedbackTag(gc, (v->window.x - voffset) - 
        ...
        if (gc->constants.yInverted) {
            ...
            __glFeedbackTag(gc, (v->window.y - voffset) - 
                        gc->constants.fviewportYAdjust);
        }
        break;
      case GL_3D:
    ...
    switch (type) {
      ...
      case GL_4D_COLOR_TEXTURE:
        ...
        */
        if ((v->hasAndClipCode & __GL_HAS_TEXTURE) == 0) {
            DO_VALIDATE(gc, v, gc->vertex.needs | __GL_HAS_TEXTURE);
        }
        __glFeedbackTag(gc, v->texture[0].x);
        __glFeedbackTag(gc, v->texture[0].y);
        __glFeedbackTag(gc, v->texture[0].z);
        __glFeedbackTag(gc, v->texture[0].w);
        break;
      case GL_2D:
      case GL_3D:
      case GL_3D_COLOR:
...
static void feedbackVertex(__GLcontext *gc, __GLvertex *v)
{
    /*
    ** Make sure texture coordinates are valid too.  They may not
    ** be in the needs flag if texturing is disabled.  By definition
    ...
    */
    ...
    if ((v->hasAndClipCode & __GL_HAS_WINDOW_TEXTURE) ||
        (gc->polygon.shader.modeFlags & __GL_SHADE_TEXTURE_UVSCALED)) {
        __GLfloat tx = v->texture[0].x;
        __GLfloat ty = v->texture[0].y;
        __GLfloat tw = v->texture[0].w;

        if (v->hasAndClipCode & __GL_HAS_WINDOW_TEXTURE) {
            v->texture[0].x *= v->clip.w;
            v->texture[0].y *= v->clip.w;
            v->texture[0].w *= v->clip.w;
        }
        if (gc->polygon.shader.modeFlags & __GL_SHADE_TEXTURE_UVSCALED) {
            __GLtexture *tex = gc->texture.currentTexture[gc->texture.currentTexUnit];
            __GLmipMapLevel *lp = &tex->level[0];

            v->texture[0].x /= lp->width2f;
            v->texture[0].y /= lp->height2f;
        }
        feedback(gc, v);
        v->texture[0].x = tx;
        v->texture[0].y = ty;
