...
void __glInitLineData(__GLcontext *gc, __GLvertex *v0, __GLvertex *v1)
{
    ...
    half = __glHalf;
    x0=v0->window.x+__glHalf;
    y0=v0->window.y+__glHalf;
    x1=v1->window.x+__glHalf;
    y1=v1->window.y+__glHalf;
    dx=x1-x0;
    dy=y1-y0;
    ...
    */
    ...
    if (modeFlags & __GL_SHADE_TEXTURE) {
        ...
        */
        v0QW = v0->texture[0].w;
        v1QW = v1->texture[0].w;
        dS = (v1->texture[0].x - v0->texture[0].x) * invDelta;
        dT = (v1->texture[0].y - v0->texture[0].y) * invDelta;
        gc->polygon.shader.dsdx = dS;
        gc->polygon.shader.dtdx = dT;
        gc->polygon.shader.dqwdx = dQWdX = (v1QW - v0QW) * invDelta;
        gc->polygon.shader.frag.s = v0->texture[0].x + dS * offset;
        gc->polygon.shader.frag.t = v0->texture[0].y + dT * offset;
        gc->polygon.shader.frag.qw = v0->texture[0].w + dQWdX * offset;

        v0rhow = (*gc->procs.calcLineRho)(gc,
                                          v0->texture[0].x,
                                          v0->texture[0].y,
                                          v0QW)
            * v0QW;
        ...
        v1rhow = (*gc->procs.calcLineRho)(gc,
                                          v1->texture[0].x,
                                          v1->texture[0].y,
                                          v1QW)
            * v1QW;
        ...
        drho = (v1rhow - v0rhow) * invDelta;
        gc->polygon.shader.drhowdx = drho;
        gc->polygon.shader.frag.rhow = v0rhow + drho * offset;
    } 
    ...
    */
    ...
    if (gc->texture.textureEnabled) {
        ...
        */
        v0QW = v0->texture[0].w;
        v1QW = v1->texture[0].w;
        dS = (v1->texture[0].x * v1QW - v0->texture[0].x * v0QW) * invDelta;
        dT = (v1->texture[0].y * v1QW - v0->texture[0].y * v0QW) * invDelta;
        gc->polygon.shader.dsdx = dS;
        gc->polygon.shader.dtdx = dT;
        gc->polygon.shader.dqwdx = dQWdX = (v1QW - v0QW) * invDelta;
        gc->polygon.shader.frag.s = v0->texture[0].x + dS * offset;
