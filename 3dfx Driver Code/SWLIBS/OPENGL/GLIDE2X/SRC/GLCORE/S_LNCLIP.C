...
void __glClipLine(__GLcontext *gc, __GLvertex *a, __GLvertex *b)
{
    ...
    allClipCodes = (a->hasAndClipCode | b->hasAndClipCode) & __GL_CLIP_MASK;
    ...
    */
    if (~(a->hasAndClipCode) & needs) {
        DO_VALIDATE(gc, a, needs);
    }
    if (~(b->hasAndClipCode) & needs) {
    ...
    */
            ...
            if (clipCodes & (__GL_CLIP_LEFT >> __GL_CLIP_SHIFT)) {
                ...
                if (d1 < zero) {
                    ...
                    */
                    ...
                    a = &np1;
                    a->hasAndClipCode = 
                      (b->hasAndClipCode & ~__GL_HAS_VERTEX_MASK) | 
                        __GL_HAS_VERTEX_0D;
                    ...
                    if (d2 < zero) {
                        ...
                        */
                        ...
                        b = &np2;
                        b->hasAndClipCode = 
                          (a->hasAndClipCode & ~__GL_HAS_VERTEX_MASK) | 
                            __GL_HAS_VERTEX_0D;
    ...
    allClipCodes &= (__GL_FRUSTUM_CLIP_MASK >> __GL_CLIP_SHIFT);
    if (allClipCodes) {
                    ...
                    */
                    ...
                    if (d2 < zero) {
                        ...
                        */
                        ...
                        (*clip)(&np2, b, a, t);
                        b = &np2;
                        b->hasAndClipCode = (a->hasAndClipCode & ~__GL_HAS_VERTEX_MASK) | 
                          __GL_HAS_VERTEX_0D;
                        b->color = &b->colors[__GL_FRONTFACE];
                    } else {
                        /* A and B are in */
    ...
    a->window.y = winy;
#ifdef GLINT
    /* Evaluate fixed point values */
    a->fixed.x = __GL_COORD_FLOAT_TO_FIXED(a->window.x);
    a->fixed.y = __GL_COORD_FLOAT_TO_FIXED(a->window.y);
#endif
...
#ifdef GLINT
    /* Evaluate fixed point values */
    b->fixed.x = __GL_COORD_FLOAT_TO_FIXED(b->window.x);
    b->fixed.y = __GL_COORD_FLOAT_TO_FIXED(b->window.y);
#endif
    ...
    if (gc->state.light.shadingModel == GL_FLAT) {
        ...
        if (~provoking->hasAndClipCode & pn) {
...
void __glFastClipSmoothLine(__GLcontext *gc, __GLvertex *a, __GLvertex *b)
{
    ...
    if ((a->hasAndClipCode | b->hasAndClipCode) & __GL_CLIP_MASK) {
        /*
        ** The line must be clipped more carefully.  Cannot trivially
        ** accept the lines.
        */
        if ((a->hasAndClipCode & b->hasAndClipCode & __GL_CLIP_MASK) != 0) {
            /*
