...
/* In the SST build, this is exposed in pixel.h so we can share it. */
...
*/
GLint __glPickSpanModifiers(__GLcontext *gc, __GLpixelSpanInfo *spanInfo,
	...
	*/
	switch(dstFormat) {
	  ...
	  case __GL_RED_ALPHA:
	    ...
	    break;
	  case GL_DEPTH_COMPONENT:
        gc->buffers.depthbuffer = &gc->depthBuffer.buf;
	    if (zoomx2) {
			    ...
			    GLenum type, const GLvoid *pixels)
{
    ...
    x = gc->state.current.rasterPos.window.x - __glHalf;
    y = gc->state.current.rasterPos.window.y - __glHalf;
			    ...
			    GLenum format)
{
    ...
    rpx = gc->state.current.rasterPos.window.x - __glHalf;
    rpy = gc->state.current.rasterPos.window.y - __glHalf;
    spanInfo->fragz = gc->state.current.rasterPos.window.z;
...
*/
GLint __glGenericPickCopyImageSpan(__GLcontext *gc, __GLpixelSpanInfo *spanInfo,
                              ...
                              GLboolean applyPixelTransfer)
{
    ...
    spanCount = __glGenericPickCopyImageSpan(gc, spanInfo, applyPixelTransfer);
...
*/
void __glGenericPickReadImage(__GLcontext *gc, __GLpixelSpanInfo *spanInfo)
{
    void (*reader)(__GLcontext *gc, __GLpixelSpanInfo *spanInfo,
		   GLvoid *outspan);
    void (*rpfn)(__GLcontext *gc, __GLpixelSpanInfo *spanInfo);
