     ...
     GLenum texenv)
{
  ...
  if (modeFlags & __GL_SHADE_TEXTURE) {
    current = gc->texture.currentTexture[gc->texture.currentTexUnit];
    lp = current->level;
    wl2 = lp->widthLog2;
    hl2 = lp->heightLog2;
...
BuildMMXRasterizer(__GLcontext *gc, unsigned char* cb)
{
    ...
    if (modeFlags & __GL_SHADE_TEXTURE) {
        ...
        texenv = gc->state.texture[gc->texture.currentTexUnit].env[0].mode;
        if (texenv == GL_DECAL &&
            gc->texture.currentTexture[gc->texture.currentTexUnit]->texelFormat == GL_RGB)
            texenv = GL_REPLACE;
    } else {
    ...
    if (need_alpha &&
        (interpolate_tex && (texenv == GL_REPLACE)) &&
         !has_alpha(gc->texture.currentTexture[gc->texture.currentTexUnit]->texelFormat) &&
        (modeFlags & __GL_SHADE_SMOOTH))
...
cb = emit_code(gc, cb, paddw(gc, mm7, mem(gc, value(gc, DISPOF(mmxtr.t.istagger)), NULL, eax, 8)));
            ...
            if (gc->texture.currentTexture[gc->texture.currentTexUnit]->texelFormat != GL_COLOR_INDEX) {
...
cb = emit_code(gc, cb, mov(gc, edx, mem(gc, value(gc, (int)&(gc)->procs.unlockBuffers), NULL, NULL, 1)));
cb = emit_code(gc, cb, test(gc, edx, edx));
cb = emit_code(gc, cb, je(gc, noneed));

cb = emit_code(gc, cb, mov(gc, edx, mem(gc, value(gc, (int)&(gc)->buffers.lock), NULL, NULL, 1)));
cb = emit_code(gc, cb, dec(gc, edx));
cb = emit_code(gc, cb, mov(gc, mem(gc, value(gc, (int)&(gc)->buffers.lock), NULL, NULL, 1), edx));
cb = emit_code(gc, cb, cmp(gc, edx, value(gc, 0)));
cb = emit_code(gc, cb, jne(gc, noneed));
cb = emit_code(gc, cb, mov(gc, edx, value(gc, (int)gc)));
cb = emit_code(gc, cb, push(gc, edx));
cb = emit_code(gc, cb, call(gc, mem(gc, value(gc, (int)&(gc)->procs.unlockBuffers), NULL, NULL, 1)));
...
GenerateSetup(__GLcontext *gc, GLboolean spanlet)
{
    ...
    if (!spanlet &&
        ...
        cookedTexture = (texenv == GL_REPLACE) &&
            gc->texture.currentTexture[gc->texture.currentTexUnit]->level[0].pixelBuffer;
    } else {
        cookedTexture = GL_FALSE;
    }
...
cb = emit_code(gc, cb, and(gc, esi, value(gc, 0xffffff80)));
cb = emit_code(gc, cb, and(gc, ebp, value(gc, 0xffffff80)));
...
cb = emit_code(gc, cb, mov(gc, edx, mem(gc, value(gc, (int)&(gc)->procs.lockBuffers), NULL, NULL, 1)));
cb = emit_code(gc, cb, test(gc, edx, edx));
cb = emit_code(gc, cb, je(gc, noneed));
...
cb = emit_code(gc, cb, mov(gc, edx, value(gc, (int)gc)));
cb = emit_code(gc, cb, push(gc, edx));
cb = emit_code(gc, cb, call(gc, mem(gc, value(gc, (int)&(gc)->procs.lockBuffers), NULL, NULL, 1)));
...
cb = emit_code(gc, cb, add(gc, esi, value(gc, (XY_BIAS + 1))));
cb = emit_code(gc, cb, mov(gc, QBASE(ebp,q0), esi));
cb = emit_code(gc, cb, fild(gc, QBASE(ebp,q0)));
cb = emit_code(gc, cb, fsub(gc, mem(gc, vx(window.x), eax, NULL, 1)));
cb = emit_code(gc, cb, fxch(gc, st1));
