                           ...
                           __GLvertex *v1, __GLvertex *v2)
{
    ...
    __GLvertex *ov0, *ov1, *ov2;
    __GLcolor tmpColor;
    __GLtri tr;         /* Must be last auto for assembler access */


    /*
     * Save original vertex pointers in case PolygonMode is POINT or LINE
     */
    ...
    *((int *)&v2->window.y) &= 0xffffff80;
    ...
    /* We can't return here unless PolygonMode(FRONT_AND_BACK, FILL)
     * because we may need to draw a point or line even if area == 0
     */
    /* if (c == 0.0F) return; */
...

    {
        ...
        case __GL_POLYGON_MODE_FILL:
            /* Why the shift?  Can we have a negative zero? */
            if (*(int *)&c << 1) {
                break;
            }
            ...
            if ((modeFlags & __GL_SHADE_SMOOTH) == 0) {
                gc->vertex.provoking->color =
                        &gc->vertex.provoking->colors[__GL_FRONTFACE];
            }
            return;
        case __GL_POLYGON_MODE_POINT:
            if (ov0->boundaryEdge) (*gc->procs.renderPoint)(gc, ov0);
            if (ov1->boundaryEdge) (*gc->procs.renderPoint)(gc, ov1);
            if (ov2->boundaryEdge) (*gc->procs.renderPoint)(gc, ov2);
            v0->color = &v0->colors[__GL_FRONTFACE];
            v1->color = &v1->colors[__GL_FRONTFACE];
            v2->color = &v2->colors[__GL_FRONTFACE];
            if ((modeFlags & __GL_SHADE_SMOOTH) == 0) {
        ...
        case __GL_POLYGON_MODE_LINE:
            if (ov0->boundaryEdge) {
                (*gc->procs.renderLine)(gc, ov0, ov1);
            }
            if (ov1->boundaryEdge) {
                (*gc->procs.renderLine)(gc, ov1, ov2);
            }
            if (ov2->boundaryEdge) {
                (*gc->procs.renderLine)(gc, ov2, ov0);
            }
            v0->color = &v0->colors[__GL_FRONTFACE];
            v1->color = &v1->colors[__GL_FRONTFACE];
            v2->color = &v2->colors[__GL_FRONTFACE];
...
#if __GL_SST
        p0 = v0->color->r / 8.1;
        dp0 = v1->color->r / 8.1 - p0;
        dp1 = v2->color->r / 8.1 - p0;
#else
        p0 = v0->color->r;
        dp0 = v1->color->r - p0;
        dp1 = v2->color->r - p0;
...
#if __GL_SST
            p0 = v0->color->g / 4.1;
            dp0 = v1->color->g / 4.1 - p0;
            dp1 = v2->color->g / 4.1 - p0;
#else
            p0 = v0->color->g;
            dp0 = v1->color->g - p0;
            dp1 = v2->color->g - p0;
...
#if __GL_SST
            p0 = v0->color->b / 8.1;
            dp0 = v1->color->b / 8.1 - p0;
            dp1 = v2->color->b / 8.1 - p0;
#else
            p0 = v0->color->b;
            dp0 = v1->color->b - p0;
            dp1 = v2->color->b - p0;
    ...
    if (modeFlags & __GL_SHADE_TEXTURE)
    {
        ...
        if (
            ...
            p0 = v0->texture[0].x;
            dp0 = v1->texture[0].x - p0;
            dp1 = v2->texture[0].x - p0;
            dpdx = dy0*dp1 - dy1*dp0;
            dpdy = dp0*dx1 - dp1*dx0;
            ...
            p0 = v0->texture[0].y;
            dp0 = v1->texture[0].y - p0;
            dp1 = v2->texture[0].y - p0;
            dpdx = dy0*dp1 - dy1*dp0;
            dpdy = dp0*dx1 - dp1*dx0;
            ...
            p0 = v0->texture[0].w;
            dp0 = v1->texture[0].w - p0;
            dp1 = v2->texture[0].w - p0;
            dpdx = dy0*dp1 - dy1*dp0;
            dpdy = dp0*dx1 - dp1*dx0;
                        ...
                        __GLfloat s, __GLfloat t, __GLfloat qw)
{
    ...
    p1 = (s + tr->fdswdx) * qw1;
    pupx = p1 - p0;
    if (!gc->texture.interpUVScaled[0])
                    ...
                    float xsnap0, float ysnap0)
{
    ...
    v0->rhow = v0->texture[0].w *
        __glFRComputePolygonRho(gc, tr,
                                v0->texture[0].x,
                                v0->texture[0].y,
                                v0->texture[0].w);

    v1->rhow = v1->texture[0].w *
        __glFRComputePolygonRho(gc, tr,
                                v1->texture[0].x,
                                v1->texture[0].y,
                                v1->texture[0].w);

    v2->rhow = v2->texture[0].w *
        __glFRComputePolygonRho(gc, tr,
                                v2->texture[0].x,
                                v2->texture[0].y,
                                v2->texture[0].w);
...
#if AFFINE_RHO
    if (gc->polygon.shader.modeFlags & __GL_SHADE_TEXTURE_PERSP)
#endif
    {
