...
static void applyTexParameter(__GLcontext *gc, __GLtexture *tex)
{
    ...
    if (tex->params.minFilter != GL_NEAREST && tex->params.minFilter != GL_LINEAR) {
        if (tex->level[0].width && tex->level[0].height) { /* XXXshui check a bit instead */
            ...
            grTex.smallLod    = GR_LOD_1;
            grTex.largeLod    = tex->sst.lod;
            grTex.aspectRatio = tex->sst.aspect;
    ...
    if ( gc->grNTexelFx == 1 ) {
        ...
        if ( enTex0 && enTex1 ) { /* both tmus enabled */
            if ( nullTMU0 ) {
                ...
                grTex.format      = tex->sst.grformat;
                grTex.smallLod    = ( tex->sst.allocation == __GL_SST_TEXALLOC_STACK ) ? GR_LOD_1 : tex->sst.lod;
                grTex.largeLod    = tex->sst.lod;
                grTex.aspectRatio = tex->sst.aspect;
                grTexSource(GR_TMU0,
                            (FxU32)tex->sst.texUnit[1].cache->addr,
                            GR_MIPMAPLEVELMASK_BOTH,
                            &grTex);
                nullTMU0 = 0;
            }
            if ( hint != 2 ) {
                grHints( GR_HINT_STWHINT, GR_STWHINT_W_DIFF_TMU1|GR_STWHINT_ST_DIFF_TMU1 );
                hint = 2;
            }
            switch( texEnv0 ) {
            case GL_REPLACE:
            ...
            /* XXX Evil Hack - From Taco W/ Love
               The goal here is to convince glide to fetch the
               s/t data from tmuvtx[0] when runing on a 
               single tmu.  So, what we do is set up TMU1
               in copy mode and set up TMU0 in scale color
               by local alpha mode, where the source texture
               is a mythical 1x1 mipmap at the 0 baseaddress
               with an RGB only texture format.  The hardware
               supplies an alpha of 1.0 where none exists
               thus giving us a safe passthru that convince glide
               that tmu0 is the most downstream, active tmu ( cf
               glidref::grTexSource ).  
             */
            if ( 1 ) {
                GrTexInfo texInfo;
                texInfo.smallLod = GR_LOD_1;
                texInfo.largeLod = GR_LOD_1;
                       ...
                       GLenum type, const GLvoid *buf)
{
    ...
    */
    ...
    if (lod == 0) {
        ...
        switch (major) {
        case 256:
            tex->sst.lod = GR_LOD_256;
            break;
        case 128:
            tex->sst.lod = GR_LOD_128;
            break;
        case 64:
            tex->sst.lod = GR_LOD_64;
            break;
        case 32:
            tex->sst.lod = GR_LOD_32;
            break;
        case 16:
            tex->sst.lod = GR_LOD_16;
            break;
        case 8:
            tex->sst.lod = GR_LOD_8;
            break;
        case 4:
            tex->sst.lod = GR_LOD_4;
            break;
        case 2:
            tex->sst.lod = GR_LOD_2;
            break;
        case 1:
            tex->sst.lod = GR_LOD_1;
            break;
        }
        ...
        switch (aspect) {
        case 64:
            tex->sst.aspect = GR_ASPECT_8x1;
            tex->sst.sScale = 256.0f;
            tex->sst.tScale =  32.0f;
            break;
        case 32:
            tex->sst.aspect = GR_ASPECT_4x1;
            tex->sst.sScale = 256.0f;
            tex->sst.tScale =  64.0f;
            break;
        case 16:
            tex->sst.aspect = GR_ASPECT_2x1;
            tex->sst.sScale = 256.0f;
            tex->sst.tScale = 128.0f;
            break;
        case 8:
            tex->sst.aspect = GR_ASPECT_1x1;
            tex->sst.sScale = 256.0f;
            tex->sst.tScale = 256.0f;
            break;
        case 4:
            tex->sst.aspect = GR_ASPECT_1x2;
            tex->sst.sScale = 128.0f;
            tex->sst.tScale = 256.0f;
            break;
        case 2:
            tex->sst.aspect = GR_ASPECT_1x4;
            tex->sst.sScale =  64.0f;
            tex->sst.tScale = 256.0f;
            break;
        case 1:
            tex->sst.aspect = GR_ASPECT_1x8;
