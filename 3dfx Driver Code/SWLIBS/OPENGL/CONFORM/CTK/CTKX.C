...
static visualRatingSupported = GL_FALSE;
        /* string length of GLX_EXT_visual_rating */
#define VISUAL_RATING_STRING_LENGTH  21 
...
{
    int erb, evb;
    int extensionStringLength, majorVersion, minorVersion;
    char *ptr, *extensionStringEnd;
	...
	if (glXQueryVersion( display, &majorVersion, &minorVersion)) {
		...
		extensionString = glXQueryExtensionsString(display, screen);
	    }
	}
	...
	if (extensionString == NULL) {
	    visualRatingSupported = GL_FALSE;
	} else {
	    ptr = (char *)extensionString;
	    extensionStringEnd = ptr + strlen(ptr);
	    while (ptr < extensionStringEnd) {
		int n = strcspn(ptr, " ");
		if ((VISUAL_RATING_STRING_LENGTH == n) && 
			    (strncmp("GLX_EXT_visual_rating", ptr, n) == 0)) {
		    visualRatingSupported = GL_TRUE;
		}
		ptr += (n + 1);
	    }
	}
...
long tkNewWindow(TK_WindowRec *wind)
{
	...
	temp.screen = screen;
	visual = XGetVisualInfo(display, VisualIDMask|VisualScreenMask,
				&temp, &count);
	if (visual == 0) {
	    return 0;
	}
    ...
    */
    ...
    sh.y = 0;
    sh.width = wind->width;
    sh.height = wind->height;
    sh.min_width = wind->width;
    sh.min_height = wind->height;
    sh.max_width = wind->width;
    sh.max_height = wind->height;
...
static void GetVisuals(TK_VisualIDsRec *ptr)
{
    ...
    int total, useGL, i, rating, caveatValue;

    ptr->count = 0;
    ptr->nonConformantCount = 0;
	...
	if (useGL) {
	    rating = GL_TRUE;
#ifdef GLX_EXT_visual_rating 
	    if (visualRatingSupported == GL_TRUE) {
		glXGetConfig(display, &vInfoList[i], GLX_VISUAL_CAVEAT_EXT, &caveatValue);
		if (caveatValue == GLX_NON_CONFORMANT_VISUAL_EXT) {
		    rating = GL_FALSE;
		}
	    }
#endif
		...
		ptr->nonConformantIDs[ptr->nonConformantCount++] = vInfoList[i].visualid;
	    }
...
long tkDrawFont(char *fontName, long x, long y, char *string, long len)
{
    ...
    */
	...
	values.foreground = visual->green_mask;
    } else {
