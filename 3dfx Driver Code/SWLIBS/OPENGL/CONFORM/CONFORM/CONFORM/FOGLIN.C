...
static long TestRGB(void *data, float ref)
{
    ...
    /*
    ** implementations with extreme visual depths may still 
    ** use a look up table as long as it has 128 entries or more.
    */
    MaxError = (1.5*epsilon.color[ptr->color] > (1.0f/64.0f)) ?  
	       1.5 * epsilon.color[ptr->color] : (1.0f / 64.0f);

    if (ABS(value-ref) > MaxError) {
	/*
	** Only Error Mesg Stuff.
	*/
...
static long TestCI(void *data, float ref)
{
	...
	*/
	...
	MaxError = (2.0 * epsilon.ci > (ptr->numColor / 64.0f)) ? 
				2.0 * epsilon.ci :  (ptr->numColor / 64.0f);  
	if (ABS(value-expected) > MaxError+epsilon.zero) {
