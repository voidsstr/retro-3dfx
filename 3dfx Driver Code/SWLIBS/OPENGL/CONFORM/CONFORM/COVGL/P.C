...
void CallPixelStore(void)
{
	...
	if (enum_PixelStore[i].value == GL_UNPACK_ALIGNMENT ||
		enum_PixelStore[i].value == GL_PACK_ALIGNMENT) {
	    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	    glPixelStoref(GL_UNPACK_ALIGNMENT, 1.0);  
				/* 1 chosen for historical consistency */
	} else {
	    glPixelStorei(enum_PixelStore[i].value, 0);
	    glPixelStoref(enum_PixelStore[i].value, 0.0);
				/* 0 is the default */
	}
	ProbeEnum();
    }
