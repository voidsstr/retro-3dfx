...
NTSTATUS
MapMemDispatch(IN PDEVICE_OBJECT DeviceObject,
               IN PIRP Irp);

VOID
MapMemUnload(IN PDRIVER_OBJECT DriverObject);

static NTSTATUS
MapMemMapTheMemory(IN PDEVICE_OBJECT DeviceObject,
...
extern ULONG __stdcall
GetMSR(MSRInfo* msrInfo);

extern ULONG __stdcall
SetMSR(MSRInfo* msrInfo);
...
--*/
{
  ...
  MapMemKdPrint (("FXPTL.SYS: entering DriverEntry\n"));
  ...
  if (NT_SUCCESS(ntStatus))
  {
    ...
    if (!NT_SUCCESS(ntStatus))
    {
      ...
      MapMemKdPrint (("FXPTL.SYS: IoCreateSymbolicLink failed\n"));
    ...
    else
    {
      ...
      if( P6_IsFixDisabledInRegistry() == 0L )     // OK with user?
        P6FixBrokenIntelChipset();               // Yes - do it.
    }
  }
  else
  {
    MapMemKdPrint (("FXPTL.SYS: IoCreateDevice failed\n"));
  }
...
--*/
{
  ...
  case IRP_MJ_CREATE:
    ...
    MapMemKdPrint (("FXPTL.SYS: IRP_MJ_CREATE\n"));
  ...
  case IRP_MJ_CLOSE:
    ...
    MapMemKdPrint (("FXPTL.SYS: IRP_MJ_CLOSE\n"));
  ...
  case IRP_MJ_DEVICE_CONTROL:
    ...
    case IOCTL_MAPMEM_MAP_USER_PHYSICAL_MEMORY:
      ...
      if (NT_SUCCESS(Irp->IoStatus.Status))
      {
        ...
        MapMemKdPrint (("FXPTL.SYS: memory successfully mapped\n"));
      }
      else
      {
        Irp->IoStatus.Status = STATUS_INVALID_PARAMETER;

        MapMemKdPrint (("FXPTL.SYS: memory map failed :(\n"));
      }
      break;
    ...
    case IOCTL_MAPMEM_UNMAP_USER_PHYSICAL_MEMORY:
      if (inputBufferLength >= sizeof(PVOID))
      {
        ...
        MapMemKdPrint (("FXPTL.SYS: memory successfully unmapped\n"));
      }
      else
      {
        Irp->IoStatus.Status = STATUS_INSUFFICIENT_RESOURCES;

        MapMemKdPrint (("FXPTL.SYS: ZwUnmapViewOfSection failed\n"));
      }
      break;
    ...
    case IOCTL_MAPMEM_GET_MSR:
      if (inputBufferLength != sizeof(MSRInfo)) {
        Irp->IoStatus.Status = STATUS_INVALID_PARAMETER;
        MapMemKdPrint (("FXPTL.SYS: InputBuffer size != sizeof(MSRInfo)\n"));        
      } else if (inputBufferLength != sizeof(MSRInfo)) {
        Irp->IoStatus.Status = STATUS_INVALID_PARAMETER;
        MapMemKdPrint (("FXPTL.SYS: InputBuffer size != sizeof(MSRInfo)\n"));        
      } else if (!hasMSR) {
        Irp->IoStatus.Status = STATUS_NOT_IMPLEMENTED;
        MapMemKdPrint (("FXPTL.SYS: No MSR's\n"));
      } else if (GetMSR(ioBuffer)) {
        Irp->IoStatus.Status = STATUS_SUCCESS;
        Irp->IoStatus.Information = sizeof(MSRInfo);
        MapMemKdPrint (("FXPTL.SYS: Got MSR\n"));
      } else {
        Irp->IoStatus.Status = STATUS_INVALID_PARAMETER;
        MapMemKdPrint (("FXPTL.SYS: Invalid MSR\n"));
      } 
      break;
    ...
    case IOCTL_MAPMEM_SET_MSR:
      if (inputBufferLength != sizeof(MSRInfo)) {
        ...
        Irp->IoStatus.Status = STATUS_NOT_IMPLEMENTED;
        MapMemKdPrint (("FXPTL.SYS: No MSR's\n"));
      } else if (SetMSR(ioBuffer)) {
        Irp->IoStatus.Status = STATUS_SUCCESS;
        Irp->IoStatus.Information = sizeof(MSRInfo);
        MapMemKdPrint (("FXPTL.SYS: Set MSR\n"));
      } else {
        Irp->IoStatus.Status = STATUS_INVALID_PARAMETER;
        MapMemKdPrint (("FXPTL.SYS: Invalid MSR\n"));
      }
    ...
    default:
      ...
      MapMemKdPrint (("FXPTL.SYS: unknown IRP_MJ_DEVICE_CONTROL\n"));
...
--*/
{
  ...
  MapMemKdPrint (("FXPTL.SYS: unloading\n"));
...
--*/
{
  ...
  {
    MapMemKdPrint (("FXPTL.SYS: Insufficient input or output buffer\n"));
  ...
  if (!NT_SUCCESS(ntStatus))
  {
    MapMemKdPrint (("FXPTL.SYS: ZwOpenSection failed\n"));
  ...
  if (!NT_SUCCESS(ntStatus))
  {
    MapMemKdPrint (("FXPTL.SYS: ObReferenceObjectByHandle failed\n"));
  ...
  {
    MapMemKdPrint (("FXPTL.SYS: HalTranslatephysicalAddress failed\n"));
  ...
  {
    MapMemKdPrint (("FXPTL.SYS: mappedLength.LowPart == 0\n"));
  ...
  else
  {
    ...
    if (!NT_SUCCESS(ntStatus))
    {
      MapMemKdPrint (("FXPTL.SYS: ZwMapViewOfSection failed\n"));
  ...
  /* Since we've mapped things we'll probably be checking for msr's soon
   * for setting up write combining.
   */
  hasMSR = CheckForMSR();
