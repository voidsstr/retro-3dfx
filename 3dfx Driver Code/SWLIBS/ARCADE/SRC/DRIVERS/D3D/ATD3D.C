...
static BOOL _atrD3DAfterDeviceCreated(int w, int h, LPDIRECT3DVIEWPORT2* lpViewport,
                               LPVOID lpContext);
...
#ifdef DPRIMAPI
D3DTLVERTEX _atD3DVerts[ATD3D_MAX_VERTS];
WORD _atD3DIdx[ATD3D_MAX_IDX];
int _atd3dNumVerts, _atd3dCurVert;
int _atd3dNumIdx, _atd3dCurIdx;
int _atd3dGeomType;
#endif
...
//    memset(&ctx->d3dExData, 0, sizeof(D3DEXECUTEDATA));
	...
	LastError = ctx->lpD3DDevice->lpVtbl->Execute(ctx->lpD3DDevice, ctx->lpD3DExBuf,
                               ctx->lpD3DViewport, D3DEXECUTE_UNCLIPPED);
...
_atrD3DBeginScene(FxU32 *viewWidth, FxU32 *viewHeight) {
    ...
    w = d3dContext->any.caps.width = d3dappi.szClient.cx ;
    h = d3dContext->any.caps.height = d3dappi.szClient.cy ;
    ...
    *viewHeight = h;
    ...
    rval = d3dapp->lpD3DViewport->lpVtbl->SetViewport2(d3dapp->lpD3DViewport,
                                                       &viewData);
    if (rval != D3D_OK) {
...
_atrD3DEndScene(void) {
	D3DRECT rect;
    D3DCLIPSTATUS status={D3DCLIPSTATUS_EXTENTS2, 0, 2048.0f, 
                                   0.0f, 2048.0f, 0.0f, 0.0f, 0.0f};

#ifdef DPRIMAPI
    _atrD3DFlushGeom();

    if (d3dContext->lpD3DDevice2->lpVtbl->GetClipStatus(d3dContext->lpD3DDevice2, 
                                                 &status) != D3D_OK)
        return FALSE;
    rect.x1 = (LONG)floor((double)status.minx);
    rect.x2 = (LONG)ceil((double)status.maxx);
    rect.y1 = (LONG)floor((double)status.miny);
    rect.y2 = (LONG)ceil((double)status.maxy);
#else
...
#ifdef DPRIMAPI
void
_atrD3DFlushGeom(void) {
    HRESULT ddrval;

	if ( _atd3dGeomType != 0 ) {
        ddrval = d3dContext->lpD3DDevice2->lpVtbl->DrawIndexedPrimitive(d3dContext->lpD3DDevice2, 
                                  _atd3dGeomType, D3DVT_TLVERTEX, 
                                  (LPVOID)_atD3DVerts, _atd3dNumVerts, 
                                  (LPWORD)_atD3DIdx, _atd3dNumIdx, 0);

        // if (ddrval != DD_OK) {
           // atuError(FXFALSE, "FlushGeom: bad call to DrawIndexedPrim");
        // }
	}
...
void
_atrD3DCheckGeom(D3DPRIMITIVETYPE type, int numVerts, int numIdx) {
    if (( type != _atd3dGeomType ) ||
        ((_atd3dNumVerts + numVerts ) >= ATD3D_MAX_VERTS ) ||
        ((_atd3dNumIdx + numIdx ) >= ATD3D_MAX_IDX )) {
        if ( _atd3dGeomType != 0 )
            _atrD3DFlushGeom();
        _atd3dGeomType = type;
    }

    _atd3dCurVert = _atd3dNumVerts; 
    _atd3dCurIdx = _atd3dNumIdx;
    _atd3dNumVerts += numVerts;
    _atd3dNumIdx += numIdx;
}
#endif
...
static _AtrD3DRenderState state[100];
...
_atrD3DRenderStatef(_AtrD3DDriver *ctx, D3DRENDERSTATETYPE type, D3DVALUE v) {
    DWORD arg;
    _AtrD3DRenderState *s = state+type;
#ifndef DPRIMAPI
    EBufferData *ebd = &ctx->ebd;
#endif
    ...
    ctx->lpD3DDevice2->lpVtbl->SetRenderState(ctx->lpD3DDevice2, type, arg);
#endif
}
...
_atrD3DRenderImg(  AtrImg *img, FxU32 screenX, FxU32 screenY ) {
    ...
    if (ddrval!=DD_OK ) {
        ...
        POINT p;
        p.x = p.y = 0;
        ClientToScreen(d3dappi.hwnd, &p);
        GetClientRect(d3dappi.hwnd, &rc);
        GetWindowRect(d3dappi.hwnd, &rc1);
...
_atrD3DDrawPoint( const GrVertex *a ) {
    ...
    _atrD3DCheckGeom(D3DPT_POINTLIST, 1, 1);
    ...
    *idx = _atd3dCurVert;
    ...
    verts[0].color=RGBA_MAKE((int)a->r,(int)a->g,(int)a->b, 0xff);
...
#ifdef notdef
    d3dContext->lpD3DDevice2->lpVtbl->DrawPrimitive(d3dContext->lpD3DDevice2, D3DPT_POINTLIST,
                       D3DVT_TLVERTEX, (LPVOID)verts, 1, 0);
#endif
}
#else
void
...
_atrD3DDrawLine( const GrVertex *a, const GrVertex *b ) {
    ...
    _atrD3DCheckGeom(D3DPT_LINELIST, 2, 2);
    verts = _atD3DVerts + _atd3dCurVert;
    idx = _atD3DIdx+_atd3dCurIdx;
    idx[0] = _atd3dCurVert;
    idx[1] = _atd3dCurVert+1;
    ...
    verts[1].color=RGBA_MAKE((int)b->r,(int)b->g,(int)b->b, 0xff);
...
#ifdef notdef
    d3dContext->lpD3DDevice2->lpVtbl->DrawPrimitive(d3dContext->lpD3DDevice2, D3DPT_LINELIST,
                       D3DVT_TLVERTEX, (LPVOID)verts, 2, 0);
#endif
}
#else
void
...
void _atrD3DDrawTriangle(const GrVertex *a, const GrVertex *b,
    ...
    _atrD3DCheckGeom(D3DPT_TRIANGLELIST, 3, 3);
    verts = _atD3DVerts + _atd3dCurVert;
    idx = _atD3DIdx+_atd3dCurIdx;
    idx[0] = _atd3dCurVert;
    idx[1] = _atd3dCurVert+1;
    idx[2] = _atd3dCurVert+2;
    ...
    verts[2].color=RGBA_MAKE((int)c->r,(int)c->g,(int)c->b, 0xff);
    verts[2].specular=RGB_MAKE(0,0,0);
...
#ifdef notdef
    d3dContext->lpD3DDevice2->lpVtbl->DrawPrimitive(d3dContext->lpD3DDevice2, 
                                             D3DPT_TRIANGLEFAN,
                       D3DVT_TLVERTEX, (LPVOID)verts, 3, 0);
#endif
}
#else
void _atrD3DDrawTriangle(const GrVertex *a, const GrVertex *b,
...
_atrD3DClearCanvas( float r, float g, float b, FxU16 za ) {
    ...
    if ( ( r != d3dContext->bmat.diffuse.r ) ||
        ...
        d3dContext->lpD3DViewport2->lpVtbl->SetBackground(d3dContext->lpD3DViewport2, 
                                                  d3dContext->hBmat);
    }

    dummy.x1 = _atrCurrentCanvas->xMin;
    dummy.y1 = d3dContext->any.caps.height-_atrCurrentCanvas->yMax;
    dummy.x2 = _atrCurrentCanvas->xMax;
    dummy.y2 = d3dContext->any.caps.height-_atrCurrentCanvas->yMin;
...
_atrDepthBufferMode( GrDepthBufferMode_t mode ) {
  ...
  _atrD3DRenderState(d3dContext, D3DRENDERSTATE_ZENABLE, 
...
SetTextureData(_AtrD3DTexEntry *d3dentry ) {
    ...
    rval=IDirect3DTexture2_Load(d3dentry->vmTexture, d3dentry->smTexture);
    CHECK(rval!=DD_OK, 
...
_atrD3DTramAllocate( _AtrTexEntry *entry ) {
    ...
    rval=IDirectDrawSurface_QueryInterface(d3dentry->vmSurf,
                          ...
                          (void *)&d3dentry->vmTexture);

    CHECK(rval!=DD_OK, "Could not Query for video memory texture");

    /* Get the handle for the texture */

    rval=IDirect3DTexture2_GetHandle(d3dentry->vmTexture, 
                                    d3dapp->lpD3DDevice, 
                                    &d3dentry->texHandle);

    if (rval!=DD_OK)
        atuError(FXFALSE, "Could not get a texture handle %s",
...
_atrD3DUpdateMaterial( AtrMaterial *m ) {
	...
	switch( m->texSrc[0] ) {
	  ...
	  case ATR_TEXSRC_NONE:
          /* when using the software renderer and not texturing a value
             of replace causes no output to be displayed */

          _atrD3DRenderState(d3dContext, D3DRENDERSTATE_TEXTUREMAPBLEND, 
                                   D3DTBLEND_MODULATE );
...
_atrD3DInitDriver(_AtrD3DDriver *ctx )  {
    ...
    ctx->lpD3DDevice2 = d3dapp->lpD3DDevice;
    rval=ctx->lpD3DDevice2->lpVtbl->QueryInterface(ctx->lpD3DDevice2,
	                      &IID_IDirect3DDevice, 
                          (void *)&ctx->lpD3DDevice);
    CHECK(rval!=DD_OK, "Could not Query for D3D device");
    ctx->lpD3DViewport2 = d3dapp->lpD3DViewport;
    rval=ctx->lpD3DViewport2->lpVtbl->QueryInterface(ctx->lpD3DViewport2,
	                      &IID_IDirect3DViewport, 
                          (void *)&ctx->lpD3DViewport);
    CHECK(rval!=DD_OK, "Could not Query for D3D device");
    ...
    if (ctx->lpD3DDevice->lpVtbl->CreateExecuteBuffer(ctx->lpD3DDevice, &ctx->debDesc, &ctx->lpD3DExBuf,
                                           NULL) != D3D_OK) {
    ...
    ctx->any.caps.tfxConfig[0].tfxRev = 0;
    ctx->any.caps.tfxConfig[0].tfxMem = ctx->TotalTextureMemory;
    ctx->any.caps.sli = 0;
...
AtrDriverInfo *_atrD3DDriverInfo;
...
_atrD3DInit( AtrDriverInfo *driverInfo ) {
    ...
    /* set snap bias to ensure we never get a negative value for coordinates
 ...
 */
static BOOL
_atrD3DAfterDeviceCreated(int w, int h, LPDIRECT3DVIEWPORT2* lplpViewport, 
                          LPVOID lpContext) {
    ...
    rval = d3dapp->lpD3DDevice->lpVtbl->SetCurrentViewport(d3dapp->lpD3DDevice, 
                                                           lpD3DViewport);
                                                            
    if (rval != D3D_OK) {
        atuError(FXTRUE, "SetCurrentViewport failed.\n%s", 
                 D3DAppErrorToString(rval));
    }
    ...
    viewData.dvMinZ = 0.0f;
    viewData.dvMaxZ = 1.0f;
    rval = lpD3DViewport->lpVtbl->SetViewport2(lpD3DViewport, &viewData);
    if (rval != D3D_OK) {
        atuError(FXTRUE, "SetViewport failed.\n%s", D3DAppErrorToString(rval));
    }
    ...
    *lplpViewport = lpD3DViewport;
    ...
    d3dContext->lpD3DViewport2->lpVtbl->SetBackground(d3dContext->lpD3DViewport2, d3dContext->hBmat);
    ...
    d3dapp->lpD3DDevice->lpVtbl->BeginScene(d3dapp->lpD3DDevice);
    ...
    d3dapp->lpD3DDevice->lpVtbl->EndScene(d3dapp->lpD3DDevice);
...
_atrD3DResize(AtrContext ctx, int w, int h) {
    ...
    if ((w > szBuffers.cx || h > szBuffers.cy) ||
         ...
         * Create new ones
         */
        if(!D3DAppICreateBuffers(d3dapp->hwnd, w, h, D3DAPP_BOGUS, 
                                     FALSE, 
                                     d3dapp->ThisDriver.bIsHardware)) {
            atuError(FXTRUE, "Could not create buffers\n");
        }
...
/*-------------------------------------------------------------------
  Function: _atrD3DMove
  Date: 3/14/97
  Implementor(s): mlwp
  Library: AT Render
  Description:
    Move's all the buffers and re-creates device if necessary.
  Arguments:
    ctx - the rendering context
    x   - new x
    y   - new y
...
_atrD3DPause(FxBool flag) {
    ...
    while ( tmp = (_AtrD3DTexEntry *)_inCache[0] ) {
        RELEASE(tmp->vmTexture); tmp->vmTexture = NULL;
        RELEASE(tmp->vmSurf); tmp->vmSurf = NULL;
        tmp->texHandle=(unsigned)HANDLE_MISS;
        _atrTexPunt( _inCache[0] );
    }
}
...
_atrD3DInitDispatchTable(_AtrD3DDriver *ctx) {
    ...
    ctx->any.TransformVertices = atr_d3d_transform_verts_asm;
