...
typedef struct _DeviceChoice DeviceChoice;

typedef struct _DeviceChoice {
    char *driverDesc;
	char *driverName;
	FxBool isPrimary;
    GUID guid;
...
DeviceChoice *deviceChoices = NULL;
extern AtrDriverInfo *_atrD3DDriverInfo;

BOOL isHardwareDevice( LPDIRECTDRAW lpDD ) {
    ...
    if (! (DriverCaps.dwCaps & DDCAPS_3D))
		return FALSE;
	/* Hack altert */
	return ( DriverCaps.dwVidMemTotal >= 2*1024*1024 );
}
                                      ...
                                      LPSTR lpDriverName, LPVOID lpContext)
{
     ...
     */
	dc = (DeviceChoice *)atuMemMalloc(sizeof(DeviceChoice));
	if ( dc == NULL ) {
		atuError(FXTRUE, "Could not allocate space for device description");
	}
	dc->driverDesc = atuStrDup(lpDriverDesc);
	dc->driverName = atuStrDup(lpDriverName);
	if ( lpGUID ) {
	    dc->guid = *lpGUID;
		dc->isPrimary = FXFALSE;
	} else dc->isPrimary = FXTRUE;
	dc->next = NULL;
	if ( deviceChoices == NULL )
		deviceChoices = dc;
	else {
		DeviceChoice *p ;
		for ( p = deviceChoices; p->next!= NULL; p = p->next ) 
			;
		p->next = dc;
	}
	lpDD->lpVtbl->Release(lpDD);
...
D3DAppICreateDD(DWORD flags) {
    ...
    if ( _atGUID != (void *)1 ) { 
        d3dappi.bIsPrimary = (_atGUID == 0 );
        LastError = DirectDrawCreate(_atGUID, &lpDD, NULL);
        if (LastError != DD_OK) {
    ...
    } else if (!(flags & D3DAPP_ONLYDDEMULATION)) {
        DirectDrawEnumerate(D3DAppIDDEnumCallback, &lpDD);

	    if ( deviceChoices == NULL ) {
		    atuError(FXTRUE, "Could not find D3D hardware");
            return FALSE;
        }

        if (atuStringCompare( _atrD3DDriverInfo->driverName, "D3DS")) {
            for ( pChoice = deviceChoices; pChoice; pChoice = pChoice->next ) {
                if ( !pChoice->isPrimary )
                    break;
            }
            if ( pChoice == NULL ) 
		        atuError(FXFALSE, "Could not find secondary D3D hardware");
        }

        if ( pChoice == NULL ) 
            pChoice = deviceChoices;

	    if ( pChoice->isPrimary ) {
	        if (FAILED(DirectDrawCreate(NULL, &d3dappi.lpDD, NULL))) {
		        atuError(FXTRUE, "Could not open DD hardware\n");
                return FALSE;
            }
	    } else {
            if (FAILED(DirectDrawCreate(&pChoice->guid, &d3dappi.lpDD, NULL))) {
		        atuError(FXTRUE, "Could not open DD hardware\n");
                return FALSE;
            }
	    }

	    d3dappi.bIsPrimary = pChoice->isPrimary;
    } else {
    ...
    /* Setup a DirectDraw2 object, We need this to determine texture 
       memory size */

    rval=d3dappi.lpDD->lpVtbl->QueryInterface(d3dappi.lpDD, &IID_IDirectDraw2, 
                                           (LPVOID *)&d3dappi.lpDD2);

    if(rval!=DD_OK) {
        D3DAppISetErrorString("Could not get IDirectDraw2",
                              D3DAppErrorToString(LastError));
        return FALSE;
    }
