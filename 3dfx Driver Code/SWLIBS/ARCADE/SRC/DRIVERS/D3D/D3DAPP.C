                          ...
                          D3DAppInfo** D3DApp)
{
    ...
    *D3DApp = &d3dappi;
    ...
    if ( flags & D3DAPP_FULLSCREEN )
         mode = PickDisplayMode(); 
    else mode = D3DAPP_YOUDECIDE ;
    if ( mode == -1 )
    ...
    if (mode == D3DAPP_USEWINDOW) {
        ...
        if (!D3DAppICreateBuffers(hwnd, w, h, D3DAPP_BOGUS, FALSE,
        ...
        if (!D3DAppICreateBuffers(hwnd, w, h, d3dappi.Mode[mode].bpp, TRUE,
                                     d3dappi.Driver[driver].bIsHardware)) {
        }
    }
    /*
