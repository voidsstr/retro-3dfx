...
FxU32 _atrD3DTransformVertices( AtrDstVertex *dest,
                                AtrVertex *src,
                                AtrVertex *end ) {
    FxU32 allIn = 0;
    FxU32 allOut = 0xff;
    float   temp;
    FxI32   tempx,tempy,tempw,tempfar;
    FxU32   cc;
    ...
    while( src < end ) {
        ...
        temp=dest->oow = src->x * _atrRenderCache->lcsToCCS.data[2]  +
                         src->y * _atrRenderCache->lcsToCCS.data[6]  +
                         src->z * _atrRenderCache->lcsToCCS.data[10] +
                         _atrRenderCache->lcsToCCS.data[14];

        dest->ooz = (dest->oow-_atrRenderCache->nearClip)*one_over_far_minus_near;
        ...
        temp = 1.0f / temp;
        tempw=*((FxI32 *)(&dest->oow));
        tempx=(*((FxI32 *)(&dest->x)))&0x7FFFFFFF;
        tempy=(*((FxI32 *)(&dest->y)))&0x7FFFFFFF;
        tempfar=*((FxI32 *)(&_atrRenderCache->farClip));

        if (( tempx < 0.0f ) || ( tempy < 0.0f )) {
            atuError(FXTRUE, "negative coords\n");
        }

        if(tempw<tempx)
        {
           cc+=2-((*((FxU32 *)(&dest->x)))>>31);
        }
        cc<<=2;
        if(tempw<tempy)
        {
           cc+=2-((*((FxU32 *)(&dest->y)))>>31);
        }
        cc<<=1;
        if(tempw>tempfar)
        {
...
#ifndef AT_PENTIUM_ASSEMBLY
#ifdef DTRIAPI
static D3DTRIANGLE tri[1000];

void RenderNode( _AtrTriSetNode *n ) {
    FxU32   *connectivity;
    FxU16    numTri = 0;
    FxU32 i;
    D3DTLVERTEX verts[100];
    HRESULT LastError;
    ...
    for ( i = 0; i <  n->numVertices; i++) {
        ...
        verts[i].color=RGBA_MAKE((int)a->r,(int)a->g,(int)a->b, (int)a->a);
        verts[i].specular=0;
        verts[i].tu=a->s0;
        verts[i].tv=a->t0;
    }
    ...
    for ( connectivity = n->connectivity; *connectivity; connectivity++ ) {
        tri[numTri].v1=(FxU16)((((*connectivity)>>16)&0xFF));
        tri[numTri].v2=(FxU16)((((*connectivity)>>8)&0xFF));
        tri[numTri].v3=(FxU16)(((*connectivity)&0xFF));
        tri[numTri].wFlags= D3DTRIFLAG_EDGEENABLETRIANGLE;
        numTri++;
    }

	LastError = d3dContext->lpDev->lpVtbl->DrawTriangle(d3dContext->lpDev, 
                                                d3dContext->lpView, 
                                         verts, n->numVertices, tri, numTri, 
...
void RenderNode( _AtrTriSetNode *n ) {
    ...
    _atrD3DCheckBufferFull(d3dContext,  n->numVertices, 4, 
                             sizeof(D3DPROCESSVERTICES) + numTri*sizeof(D3DTRIANGLE));
    ...
    _atrD3DProcessVertices(d3dContext, 
                D3DPROCESSVERTICES_COPY | D3DPROCESSVERTICES_UPDATEEXTENTS, n->numVertices);
    ...
    _atrD3DInstruction(d3dContext, D3DOP_TRIANGLE, sizeof(D3DTRIANGLE), numTri);

    for ( connectivity = n->connectivity; *connectivity; connectivity++ ) {
        ((D3DTRIANGLE *)ebd->lpInstPointer)->v1=vnum+(FxU16)((((*connectivity)>>16)&0xFF));
        ((D3DTRIANGLE *)ebd->lpInstPointer)->v2=vnum+(FxU16)((((*connectivity)>>8)&0xFF));
        ((D3DTRIANGLE *)ebd->lpInstPointer)->v3=vnum+(FxU16)(((*connectivity)&0xFF));
...
void RenderNodeLM( _AtrTriSetNode *n ) {
    ...
    /* ensure we have enough space in buffer in the worst case we need space 
       for the vertices, and instructions for PROCESSVERTICES, and EXIT and
       for each triangle a QWORD ALIGN, a D3DSTATE and a TRIANGLE */ 

	
    _atrD3DCheckBufferFull(d3dContext, 3*numTri, 2+3*numTri, 
                             sizeof(D3DPROCESSVERTICES) + 
                             numTri*(sizeof(D3DTRIANGLE)+sizeof(D3DSTATE)));
...
void atrD3DRenderTriSet( AtrTriSet *t ) {
    ...
    /* For Each Chunk */
    one_over_far_minus_near=1.0f/(_atrRenderCache->farClip - _atrRenderCache->nearClip);
    while( n ) {
...
#       endif
        ...
        flags = DRV_FUNC(TransformVertices)( dest, src, end );
