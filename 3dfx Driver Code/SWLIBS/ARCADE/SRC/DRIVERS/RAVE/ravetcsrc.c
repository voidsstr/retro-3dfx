/*
** Copyright (c) 1996, Apple Computer Corp.
** All Rights Reserved.
**
** This is UNPUBLISHED PROPRIETARY SOURCE CODE of Apple Computer Corp.;
...
**
** $Revision: 4$ 
** $Date: 10/11/00 7:35:36 PM$ 
**
*/

/*************************************************************************
 * ravetcsrc.c : this file was patterned after d3dtcsrc.c, and contains  *
 *		a number of texture vertex callbacks needed by                     *
 *		atrRaveUpdateMaterial. These are mostly NOPs but may be adjusted   *
 *		for special effects at a later date.                               *
 *************************************************************************/
...
/*-------------------------------------------------------------------
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC0_2DTC0(AtrDstVertex *dest,           
                      AtrVertex *src,
                      AtrVertex *end ) {
    while( src < end ) {                        
...
/*-------------------------------------------------------------------
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC0_TC0( AtrDstVertex *dest,           
                     AtrVertex *src,
                     AtrVertex *end ) {
    while( src < end ) {                        
        if ( dest->flags ) {
...
/*-------------------------------------------------------------------
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC1_TC0( AtrDstVertex *dest,           
                    AtrVertex *src,
                    AtrVertex *end ) {
    while( src < end ) {                        
        if ( dest->flags ) {
            ...
            dest->s1 = src->s0  * dest->oow;
            dest->t1 = src->t0  * dest->oow;
            dest->oow0 = dest->oow;
        }
        dest++;
        src++;
...
/*-------------------------------------------------------------------
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC0_TC1( AtrDstVertex *dest, 
                    AtrVertex *src,
                    AtrVertex *end ) {
    while( src < end ) {
        if ( dest->flags ) {
            ...
            dest->s0   = src->s1  * dest->oow;
            dest->t0   = src->t1  * dest->oow;
            dest->oow0 = dest->oow;
        }   
        src++;
        dest++;
...
/*-------------------------------------------------------------------
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC1_TC1( AtrDstVertex *dest, 
                    AtrVertex *src,
                    AtrVertex *end ) {
    while( src < end ) {
        if ( dest->flags ) {
...
/*-------------------------------------------------------------------
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC0_TC2( AtrDstVertex *dest, 
                    AtrVertex *src,
                    AtrVertex *end ) {
    while( src < end ) {
        if ( dest->flags ) {
            ...
            dest->s0   = src->s2  * dest->oow;
            dest->t0   = src->t2  * dest->oow;
            dest->oow0 = dest->oow;
        }
        src++;
        dest++;
...
/*-------------------------------------------------------------------
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC1_TC2( AtrDstVertex *dest, 
                    AtrVertex *src,
                    AtrVertex *end ) {
    while( src < end ) {
        if ( dest->flags ) {
            ...
            dest->s1   = src->s2  * dest->oow;
            dest->t1   = src->t2  * dest->oow;
            dest->oow1 = dest->oow;
        }
        src++;
        dest++;
...
/*-------------------------------------------------------------------
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC0_EMAP( AtrDstVertex *dest, 
                      AtrVertex *src,
                      AtrVertex *end ) {
...
//tScale=1.0;
    ...
    while( src < end ) {
        ...
        oom = half / atmOOSqrt( r[0] * r[0] + 
                                  r[1] * r[1] +
                                  r[2] * r[2] );
        dest->s0 = (r[0] *  oom + half)*dest->oow0;
        dest->t0 = (r[1] * -oom + half)*dest->oow0;
        dest++;
        src++;
    }
    return;
...
/*-------------------------------------------------------------------
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC1_EMAP( AtrDstVertex *dest, 
                      AtrVertex *src,
                      AtrVertex *end ) {
    AtmVector3 normal, u, r, tmp;
    AtrXform lcsToVCS, wcsToVCS;
...
//tScale=1.0;
    ...
    while( src < end ) {
        ...
        oom = half / atmOOSqrt( r[0] * r[0] + 
                                  r[1] * r[1] +
                                  r[2] * r[2] );
        dest->s1 = (r[0] *  oom + half)*dest->oow0;
        dest->t1 = (r[1] * -oom + half)*dest->oow0;
        dest++;
        src++;
    }
    return;
...
/*-------------------------------------------------------------------
  Function: _atrRAV_TCSRC0_PROJECTED
  Date: 7/4/96
  Implementor(s): jdt
  Library: AT Render
  Description:
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC0_PROJECTED(AtrDstVertex *dest, 
                          AtrVertex    *src,
                          AtrVertex    *end ) {
...
/*-------------------------------------------------------------------
  Function: _atrRAV_TCSRC1_PROJECTED
  Date: 7/4/96
  Implementor(s): jdt
  Library: AT Render
  Description:
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC1_PROJECTED(AtrDstVertex *dest, 
                          AtrVertex    *src,
                          AtrVertex    *end ) {
    _AtrLightNode *n = _atrLightHead;
    FxU32 tag        = _atrRenderMaterial->projectedTag;
...
/*-------------------------------------------------------------------
  Function: _atrRAV_TCSRC0_PLANAR
  Date: 7/22/96
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC0_PLANAR(AtrDstVertex *dest, 
                       AtrVertex    *src,
                       AtrVertex    *end ) {
...
/*-------------------------------------------------------------------
  Function: _atrRAV_TCSRC1_PLANAR
  Date: 7/22/96
  Implementor(s): jdt, da
  Library: AT Render
  Description:
  ...
  Return:
    none
  -------------------------------------------------------------------*/
void _atrRAV_TCSRC1_PLANAR(AtrDstVertex *dest, 
                       AtrVertex    *src,
                       AtrVertex    *end ) {
    AtrXform *mTW = _atrCurrentXform;
    float s, t;
