...
int countKeyframeObjects( TDSFile *tdsFile ) {
    int retval = 0;
    int i;
    for( i = 0; i < 65536; i++ ) {
...
int countMeshes( tds_face **faces, 
                 int numObjs, 
                 int *faceCountArray, 
                 int numMaterials ) {
...
void translate3DSMaterialIntoATBMaterial( AtrMaterial *atbMat, 
    ...
    if ( tdsMat->has_name ) {
        ...
        if ( tdsMat->has_reflmap ) {
            printf( "%s has a reflection map and "
                    "reflection maps aren't yet supported.\n", 
                    tdsMat->name );
        }
...
char *findTDSObjectName( int object, TDSFile *tdsFile ) {
    ...
    for( index = 0; index < 65536; index++ ) {
        if ( tdsFile->key_node_to_object_index[index] == object ) {
            if ( tdsFile->key_nodes[index] && 
                 (tdsFile->key_nodes[index]->type == TDS_OBJECT_NODE_TAG) ) {
                if ( *(tdsFile->
                       key_nodes[index]->
                       nodes.object_node.instance_name) != 0 ) {
                    return ( tdsFile->
                             key_nodes[index]->
                             nodes.object_node.instance_name );
                } else {
...
int main( int argc, char *argv[] ) {
    FILE *infile;
    FILE *outfile;
    int index, object, rendertarget, mesh, frame;
    int numMeshes, numMaterials, numTransforms, numKFOs, numRenderTargets;
    int numFrames, firstFrame, lastFrame;
    ...
    MeshThingy   *meshThingyArray;

    Scene scene;

    if ( argc != 3 )  {
        puts( "aextract infile.3ds outfile.foo\n" );
        return -1;
    }
    ...
    */
    ...
    meshThingyArray = calloc( sizeof( MeshThingy ), numMeshes );
            ...
            for ( face = 0; face < tdsFile.num_faces_in_object[object]; face++ ) {
                if ( tdsFile.faces[object][face].material_index == material ) {
...
#endif
                    atrMeshBeginTriangle( meshArray + mesh );
                    atrMeshVertex( meshArray + mesh, 
                                   v1[0], v1[1], v1[2], 
                                   t1[0], t1[1] );
                    atrMeshVertex( meshArray + mesh, 
                                   v2[0], v2[1], v2[2], 
                    ...
                    atrMeshVertex( meshArray + mesh, 
                                   v3[0], v3[1], v3[2], 
                                   t3[0], t3[1] );
                    atrMeshEndTriangle( meshArray + mesh );
                }
            }
            if ( meshStarted ) {
                atrMeshCalcFaceNormals( meshArray + mesh );
                atrMeshCalcVertexNormals( meshArray + mesh );
                meshThingyArray[mesh].materialIndex = material;
                meshThingyArray[mesh].objectIndex = object;
                meshThingyArray[mesh].meshIndex = mesh;
                meshThingyArray[mesh].name = 
                    calloc( sizeof(char),
                            strlen( findTDSObjectName( object, &tdsFile ) ) +
                            strlen( tdsFile.materials[material].name ) + 2 );
                strcpy( meshThingyArray[mesh].name,
                        findTDSObjectName( object, &tdsFile ) );
                strcat( meshThingyArray[mesh].name,
                        "." );
                strcat( meshThingyArray[mesh].name,
    ...
    /* Get animation */
    numKFOs = countKeyframeObjects( &tdsFile );
    object = 0;
    numRenderTargets = 0;
    numFrames = tdsFile.animation_length;
    animationArray = atrAnimationAllocate( numKFOs );
    firstFrame = tdsFile.first_frame;
    lastFrame = tdsFile.last_frame;
    if ( numFrames > 0 )
    for ( index = 0; index < 65536; index++ ) {
        if ( tdsFile.key_nodes[index] && 
             (tdsFile.key_nodes[index]->type == TDS_OBJECT_NODE_TAG) ) {
            if ( object >= numKFOs ) {
                printf( "problem calculating number of keyframe objects\n" );
                return -1;
            }
            animationArray[object].totalCells = numFrames;
            animationArray[object].animationTime = (float)numFrames;
            animationArray[object].infiniteLoop = FXTRUE;
            animationArray[object].cellList = 
               atrTransformAllocate( numFrames );
            for ( frame = firstFrame; frame < lastFrame; frame++ ) {
                translate3DSMatrixIntoAtbTransform(
                   animationArray[object].cellList + frame,
                   tdsFile.world_matrices[index] + frame );
            }
            for( mesh = 0; mesh < numMeshes; mesh++ ) {
                if ( meshThingyArray[mesh].objectIndex == (FxU32)
                     tdsFile.key_node_to_object_index[index] ) {
                    numRenderTargets++;
                }
            }
            object++;
    ...
    renderTargetArray = calloc( sizeof( RenderTarget ), numRenderTargets );
    object = 0;
    rendertarget = 0;
    for( index = 0; index < 65536; index++ ) {
        if ( tdsFile.key_nodes[index] && 
            ...
            for( mesh = 0; mesh < numMeshes; mesh++ ) {
                if ( meshThingyArray[mesh].objectIndex == (FxU32)
                     tdsFile.key_node_to_object_index[index] ) {
                    printf( "Creating rendertarget: %s\n", meshThingyArray[mesh].name );
                    renderTargetArray[rendertarget].name =
                        meshThingyArray[mesh].name;
                    renderTargetArray[rendertarget].meshIndex = 
                        meshThingyArray[mesh].meshIndex;
                    renderTargetArray[rendertarget].materialIndex = 
                        meshThingyArray[mesh].materialIndex;
                    renderTargetArray[rendertarget].animationIndex = object;
                    renderTargetArray[rendertarget].transformIndex = 0;
                    rendertarget++;
                }
            }
            object++;
