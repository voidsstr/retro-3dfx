...
AtmVector3  lightPosition  = {  30.0f, 5.0f,  0.0f }, 
            ...
            up             = {  0.0f,  1.0f,   0.0f };
AtrColor    camWin   = { 0.2f, 0.0f, 0.0f },
...
void currentWindow( Window *w, AtrColor *c ) {
    ...
    ATM_VEC3_SCALE(tmp, cameraShift, cameraPosition);
    atrXformPointAt( &w->camera->lcsToWCS, cameraTarget, tmp, up );
...
int AppMain( int argc, char **argv ) {
    ...
    atrMaterialSetup(projMat, ATR_MAT_TEX_PROJECTED );
    projMat->texture[0] = projTex;
    projMat->texSClamp[0] = ATR_TEXCLAMP_CLAMP;
    projMat->texTClamp[0] = ATR_TEXCLAMP_CLAMP;
...

    {
        ...
        lightPov = createWindow( 480, 351, 608, 477, &tmp );
        lightPov->camera->fovRadians = 90.0f * ATM_DEGREE;

        atrXformPointAt( &tmp, cameraTarget, cameraPosition, up );
        cameraPov = createWindow( 60, 100, 340, 300, &tmp );

        textWindow = createWindow( 480, 128, 637, 350, &tmp );
        imgWindow = createWindow( 480, 2, 608, 128, &tmp );
    }
