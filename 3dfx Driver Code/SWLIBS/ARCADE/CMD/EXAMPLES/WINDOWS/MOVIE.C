...
AppRenderScene( FxU32 displayWidth, FxU32 displayHeight) {
   ...
   /*--------------------------------------------------------
    ...
    frame = movie->curImage+1;
    if ( frame >= movie->numImages )
        frame = 0;

    atsTextureFrame( movie, frame );
...
AppInit(void) {
   ...
   /*--------------------------------------------------------
	...
	if (!atrImgCreateFromFile( img, name )) {
		atuError(FXTRUE, "Can't find image file %s", name);
	}
	tex = atrTexNewHandle( 0 );
    atrTexAssociate( tex, img);
	cubeMaterial->texture[0] = tex;
#else
    movie = (AtsTexture *)atsTextureMovie("D:\\3dfx\\movies\\fight\\%04d.3df", 0, 72);
	cubeMaterial->texture[0] = movie->handle;
#endif
