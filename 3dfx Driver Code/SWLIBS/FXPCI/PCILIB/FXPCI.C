...
struct {
  FxU32 address;
  FxU32 device_bus_func_number ;
  FxU32 mapped;
} linearAddressMapList[MAX_PCI_DEVICES];

FxU32 find_mapped_address(FxU32 device_bus_func_number) {
  FxU32 i;

  for (i=0;i<MAX_PCI_DEVICES;i++) {
     if ((linearAddressMapList[i].mapped) && 
         (linearAddressMapList[i].device_bus_func_number == 
              device_bus_func_number)) {
         return (linearAddressMapList[i].address);
     }
  }
  return (0);
}
void set_mapped_address(FxU32 device_bus_func_number,FxU32 value) {
  FxU32 i;
  ...
  for (i=0;i<MAX_PCI_DEVICES;i++) {
     if ((linearAddressMapList[i].mapped) && 
         (linearAddressMapList[i].device_bus_func_number == 
              device_bus_func_number)) {
         if (value == 0) {
            linearAddressMapList[i].mapped = 0;
         }
         linearAddressMapList[i].address = value;
         return;
     }
  }
  ...
  for (i=0;i<MAX_PCI_DEVICES;i++) {
     if (linearAddressMapList[i].mapped == 0) {
        linearAddressMapList[i].mapped = 1;
        linearAddressMapList[i].address = value;
...
#       include <conio.h>
#       if (_MSC_VER <= 1200)  /* MS VC 2.0=900, 4.0=1000, 4.1=101, 4.2=1020, 5.0=1100, 6.0=1200 */
...
/* PRIVATE DATA */
static PciRegister baseAddresses[4];
...
{
  const char*
    retVal = pciError[pciErrorCode].string;
...
pciMapPhysicalDeviceToLinear(FxU32* linear_addr, 
                             ...
                             FxU32* length) 
{ 
#define FN_NAME "pciMapPhysicalDeviceToLinear"
  if ( !libraryInitialized ) {
    pciErrorCode = PCI_ERR_NOTOPEN;
    return FXFALSE;
  }
  ...
  *linear_addr = DpmiMapPhysicalToLinear( physical_addr, *length );
...
#elif defined( MAPPL_MAPMEM_MINIPORT )
  {
    ...
    if ( !DeviceIoControl( hMapmemFile,
                          ...
                          &pmi, sizeof(pmi),
                          linear_addr, sizeof(linear_addr),
                          &cbReturned, NULL ) ) {
...
pciUnmapPhysical( FxU32 linear_addr, FxU32 length ) 
{
  ...
  for (i = 0; i < MAX_PCI_DEVICES; i++) {
    if (find_mapped_address(i) == linear_addr) {
      set_mapped_address(i,0L);
...
pciMapCardMulti(FxU32 vendorID, FxU32 deviceID,
                FxI32 length, FxU32 *devNum,
                FxU32 cardNum, FxU32 addressNum)
{
#define FN_NAME "pciMapCardMulti"
  ...
  if (find_mapped_address(*devNum) != 0L) { 
    return (FxU32 *) find_mapped_address(*devNum);
  }
  ...
  set_mapped_address(*devNum,virtAddress);
