...
REM -----------------------------------------------------------------------------
REM Q3D: Added %COMP32%\..\Common\MSDev98\Bin;
PATH=%COMP32%\bin;%COMP32%\..\Common\MSDev98\Bin;%PATH%
REM

REM -----------------------------------------------------------------------------
REM # Setup PATH to other tools
REM -----------------------------------------------------------------------------

REM
REM setup path for MSVC
REM
set MSDevDir=%DEVTOOLS%\watcom
rem set MSVCDir=%DEVTOOLS%\msvc42

set path=%MSDevDir%\binnt;%MSDevDir%\binw;%path%
set include=%MSDevDir%\h;%include%
set lib=%MSDevDir%\lib386\dos;%lib%

REM
REM setup different LIB paths for free and checked
REM
if "%1%"=="" goto free
if "%1%"=="free" goto free
...
REM setup DDK path
REM
rem set BASEDIR=%DEVTOOLS%\w2kddk
rem set PATH=%BASEDIR%\bin;%path%
rem set LIB=%BASEDIR%\lib%BUILD_ALT_DIR%;%lib%
rem set INCLUDE=%BASEDIR%\inc;%include%
...
REM C_AUTOSTRIP=0 use autostripping logic for triangle primitives
set C_AUTOSTRIP=0

REM VB_SUPP=0 include Vertex Buffer ddi support
set VB_SUPP=0
...
REM HP=H5 use for Napalm specific hw and driver names
set HP=H5
...
REM WCS=0 Windows C Simulator
set WCS=0

REM SA=1 For SLI/AA Support
set SA=1

REM RA=1 For Read Abort Error Workaround --> Disabled Now
set RA=
