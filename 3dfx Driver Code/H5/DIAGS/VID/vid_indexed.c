...
main (int argc, char **argv)
{
    ...
    sst = SST_BEGIN(argc,argv);
	    ...
	    if ( diago.ytiled )
		 ...
		 SET(sstio->vidDesktopOverlayStride,stride<<SST_OVERLAY_STRIDE_SHIFT); 
	      ...
	      tmp = GET(sstio->vidProcCfg);
			if ( diago.ytiled )
			SET(sstio->vidProcCfg,tmp | SST_DESKTOP_PIXEL_PAL8|SST_DESKTOP_TILED_EN);
			else
			SET(sstio->vidProcCfg,tmp | SST_DESKTOP_PIXEL_PAL8);
	 ...
	 tmp |= SST_OVERLAY_CLUT_SELECT; // turn on clut
	 ...
	 for(x=0; x<640; x++)
	   {
			tmp = ((x*y)>>7)& 0x0ff; 
			if (tmp>255) tmp=255;
			if((x%11==0)) tmp |= 0xff; // white stripes
			if((y%9==0)) tmp ^= 0xffffff; // invert data
			CSIM_PIXEL_WR(CSIM_BUF_DESKTOP, x, y, tmp);
	   }
	 }
 
        // setup clut
        for(x=0; x<512; x++) clut[x]=(x-256)*(1+256+65536); // straight ramp
        for(x=0; x<256; x++) clut[x]=(33*(x&0xe0))<<11 | (65*(x& 0x18))<<5 
                                     | (x&0x7)*33;
; // 323 format
        loadClut(clut, sstio);
	...
	SET(sstio->hwCurPatAddr
			  , 0x100000 + GET(sst->colBufferAddr)); // one meg into it.
        SET(sstio->hwCurLoc, ((65-8) <<SST_CURSOR_X_SHIFT) 
                           | ((266+65) << SST_CURSOR_Y_SHIFT)); 
