...
**
** $Revision: 2$
** $Date: 10/11/00 8:19:53 PM$
*/
/* This test is a very simple test of the video backend, using the vidutils.c
file
*/
/* This test tries uvyv color space conversion, with bilinear filtering*/
...
main (int argc, char **argv)
{
    ...
    sst = SST_BEGIN(argc,argv);
	 sstio = (SstIORegs *)SST_IO_ADDRESS(sst);
	    ...
	    if ( diago.ytiled )
	       SET(sstio->vidProcCfg,SST_OVERLAY_PIXEL_UYVY422|SST_OVERLAY_TILED_EN);
		 else
		 // use dithered 565 mode
		    SET(sstio->vidProcCfg,SST_OVERLAY_PIXEL_UYVY422);
