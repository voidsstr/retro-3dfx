...
main (int argc, char **argv)
{
    ...
    sst = SST_BEGIN(argc,argv);
	    ...
	    if ( diago.ytiled )
		 ...
		 SET(sstio->vidDesktopOverlayStride,stride<<SST_OVERLAY_STRIDE_SHIFT); 
	      ...
	      tmp = GET(sstio->vidProcCfg);
			if ( diago.ytiled )
			SET(sstio->vidProcCfg,tmp | SST_DESKTOP_PIXEL_RGB32|SST_DESKTOP_TILED_EN);
			else
			SET(sstio->vidProcCfg,tmp | SST_DESKTOP_PIXEL_RGB32);
			SET(sstio->vidDesktopStartAddr,GET(sst->colBufferAddr));
			SET(sst->colBufferStride, width*4);
			stride = GET(sst->colBufferStride);
			stride &= ~SST_BUFFER_MEMORY_TYPE;
	 ...
	 for(y=0;y<(int)img.any.height; y++)
		 for(x=0; x<(int)img.any.width; x++)
	 {
		 tmp = img.any.data[0+(4*(x+y*img.any.width))];
		 tmp |= (img.any.data[1+(4*(x+y*img.any.width))] & 0xff)<<8; 
		 tmp |= (img.any.data[2+(4*(x+y*img.any.width))] & 0xff)<<16;
