...
main (int argc, char **argv)
{
    ...
    sst = SST_BEGIN(argc,argv);
	 sstio = (SstIORegs *)SST_IO_ADDRESS(sst);
	    ...
	    if ( diago.ytiled )
	       SET(sstio->vidProcCfg,SST_OVERLAY_PIXEL_YUV411|SST_OVERLAY_TILED_EN);
		 else
		 // use dithered 565 mode
		    SET(sstio->vidProcCfg,SST_OVERLAY_PIXEL_YUV411);
	 ...
	 for(y=0;y<(int)img.any.height; y++)
      for(x=0; x<(int)img.any.width; x+=4)
	     {
		 for(j=0; j<4; j++) {
		 ...
		 CSIM_PIXEL_WR(CSIM_BUF_OVERLAY, x+2,y,tmp);
	 }
