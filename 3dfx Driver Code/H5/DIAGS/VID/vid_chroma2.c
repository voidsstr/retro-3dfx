...
main (int argc, char **argv)
{
    ...
    sst = SST_BEGIN(argc,argv);
	 ...
	 tmp |= SST_CHROMA_EN;    // chroma enabled, 
	 tmp |= SST_CHROMA_INVERT;    // chroma enabled,  inversion
	 ...
	 SET(sstio->vidScreenSize,tmp);    
         ...
         SET(sstio->vidChromaMax, 0x007bef);
