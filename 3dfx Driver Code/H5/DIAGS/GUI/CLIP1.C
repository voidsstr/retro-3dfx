...
//    fxHalPutenv("SST_FBI_MEM=4");
  ...
  while (DIAG_STARTPASS())			// for each pass
  {
	...
	maxWidth = ((128 / 8) / pix_to_bytepp[dfIndex]) * widthFactor;
      ...
      for (clipX = clipXinitial; clipX <= (maxWidth + 1); clipX++) 	
      {
      for (clipYindex = clipYInitialIndex; clipYindex < 2; clipYindex++)
      {
	  if (clipYindex == 1)
	      clipY = iRandom(maxWidth - 1) + 1;
	  
	  for (clipWidthIndex = clipWidthIndexInitial;
	       clipWidthIndex <= 4;
	       clipWidthIndex++)
	  {
	      if (clipWidthIndex == 2)
		  clipWidth = maxWidth / 2;
	      else if (clipWidthIndex == 3)
		  clipWidth = maxWidth;
	      else if (clipWidthIndex == 4)
		  clipWidth = maxWidth + 1;
	      else
		  clipWidth = clipWidthIndex;
	      
          for (clipHeightIndex = clipHeightInitialIndex;
	       clipHeightIndex < 3; clipHeightIndex++)
	  {
	      clipHeight = clipHeightList[clipHeightIndex];
	      if (clipHeightIndex == 2)
		  clipHeight = iRandom(diago.tsize - 1) + 1;

	      clipX2 = clipX;

	      if (iRandom(1) == 1)
	      {
		  SET(sstg->clip0min, (clipY << 16) | clipX2);
	      ...
	      {
		  SET(sstg->clip1min, (clipY << 16) | clipX2);
	  ...
	  for (widthIndex = widthIndexInitial; widthIndex < 5; widthIndex++)
	  {
	      // for now, only do non-zero area blits
	      widthIndex = 4;
	      
	      if (widthIndex == 2)
		  width = maxWidth / 2;
	      else if (widthIndex == 3)
		  width = maxWidth;
	      else if (widthIndex == 4)
		  width = maxWidth + 1;
	      else
		  width = widthIndex;
	      
	 for (heightIndex = heightIndexInitial; heightIndex < 4; heightIndex++)
	 {
	     // for now, only do non-zeor area blits
	     heightIndex = 3;
	     
	     height = heightList[heightIndex];
	     if (heightIndex == 3)
		 height = iRandom((diago.tsize/2) - 1) + 1;
	     
	for (dstXquadrant = dstXquadInitial; dstXquadrant <= 2; dstXquadrant++)
	for (dstYquadrant = dstYquadInitial; dstYquadrant <= 1; dstYquadrant++)
	{
	    if (dstXquadrant == 2)
	    {
		// fully cover clip rect, + 1 pixel on every boundary
		dstX = clipX2 - 1;
		dstY = clipY - 1;
		width2 = clipWidth + 2;
		height2 = clipHeight + 2;
	    }
	    else
	    {
		width2 = width;
		...
		dstX = clipX2 + (width2 *
				 ((dstXquadrant == 2) ? 1 : dstXquadrant));
		if (dstXquadrant == 1)
		    dstX = clipX2 + clipWidth;

		dstY = clipY + (height2 * dstYquadrant);
		if (dstYquadrant == 1)
		    dstY = clipY + clipHeight;
	    }

	    srcX = iRandom(diago.xmaxscreen - (width2 + 4));
	    if (srcX < 0)
		srcX = 0;
	    srcY = iRandom(diago.ymaxscreen - (height2 + 4));
	    if (srcY < 0)
		srcY = 0;

	    my_do_blt(sstg, srcX, srcY, dstX, dstY, width2, height2, cmdops);

	    if (dstXquadrant == 2)
		break;	// only one iteration to do here
