...
main (int argc, char **argv)
{
  ...
  while (DIAG_STARTPASS())			// for each pass
  {
	...
	for (xLeft = -3; xLeft <= lastX[dfIndex]; xLeft++)
	{
	    /* for negative x, just test a few starting points: x= -128 bits,
	     * x = -64bits, and x = -1 pixel
	     */
	    if (xLeft == -3)
		x = -(long)(maxWidth / widthFactor);
	    else if (xLeft == -2)
		x = -(long)(maxWidth / widthFactor) / 2;
	    else
		x = xLeft;

	    for (width = 0; width <= maxWidth + 1; width++)
	    {
