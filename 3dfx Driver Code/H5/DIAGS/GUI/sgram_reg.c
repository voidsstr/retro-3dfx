...
main (int argc, char **argv)
{
    ...
    GDBG_INFO(0,"HACK -- waiting for idle before reconfiguring sgram\n");
    sst_idle_really(sst);
    halStore32(&(sstg->commandEx),0x0);
    sstgCheckForIdle(sst, cmdXops);

    halStore32(&(sstg->colorFore),0x37);
    {
      int csimSave = diago.halInfo->csimio;
      diago.halInfo->csimio = 0;
      halStore32(&(sstg->command),0x10d);
      diago.halInfo->csimio = csimSave;
    }
    GDBG_INFO(0,"HACK -- waiting for idle after reconfiguring sgram\n");
    sst_idle_really(sst);
