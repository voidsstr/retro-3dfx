...
#define ROP_USES_DST(x)     (((x)^((x)<<1)) & 0xFE)     /* test whether rop uses the dst data */
		...
		FxU32 dst_colorkey = mysstg.commandEx & SSTG_EN_DST_COLORKEY_EX;
		FxU32 src_colorkey = mysstg.commandEx & SSTG_EN_SRC_COLORKEY_EX;
		FxU8 rop0 = (FxU8) ((mysstg.command&SSTG_ROP0)>>SSTG_ROP0_SHIFT);
		FxU8 rop2 = (FxU8) ((mysstg.rop>>8)&0xFF);
		
		if ( dst_colorkey || ROP_USES_DST(rop0) || (src_colorkey && ROP_USES_DST(rop2)) ) {
		  
		  FxU32 clipselect = mysstg.command & SSTG_CLIPSELECT;
		  long xd, yd;
		  unpackXY(mysstg.dstXY,&xd,&yd);  // get dst start posn
		  
		  if ( ( clipselect==0 && (yd<(signed)HIWORD(mysstg.clip0min) || yd>(signed)HIWORD(mysstg.clip0max)) ) || 
		       ( clipselect!=0 && (yd<(signed)HIWORD(mysstg.clip1min) || yd>(signed)HIWORD(mysstg.clip1max)) ) ) {
