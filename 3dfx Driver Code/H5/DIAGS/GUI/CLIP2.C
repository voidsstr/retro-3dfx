...
main(int argc, char **argv)
{
    ...
    FxU32 cfore,cback;
    char *newArgv[1024], widthArg[]="-w2048";
    int i;
    ...
    for(i=0; i<argc; i++)
      newArgv[i]=argv[i];
    newArgv[argc++]=widthArg;
    newArgv[argc]=NULL;
    argv=newArgv;

    gdbg_printf("INFO: forcing -w2048 option!\n");
  ...
  while (DIAG_STARTPASS())			// for each pass
  {
	...
	height = 7;
      ...
      for (res = resInitial; res < Nresolutions; res++)
      {
	  if ( diago.ytiled )
	    stride = CEIL(resXlist[res] * pix_to_bytepp[dfIndex],SST_TILE_WIDTH);
	  else
	    stride = resXlist[res] * pix_to_bytepp[dfIndex];
	  SET(sstg->dstFormat, dstFormat | stride);
	  SET(sstg->srcFormat, dstFormat | stride);

       for (clipReg = clipRegInitial; clipReg < 2; clipReg++)
       {
	   if (clipReg)
	   {
	       SET(sstg->clip1min, 0);
	       SET(sstg->clip1max, (resYlist[res] << 16) | resXlist[res]);
	       cmdops |= SSTG_CLIPSELECT;
	       gdbg_info(2, "cliprect 1 enabled and set to [0, 0], ");
	       gdbg_info_more(2, "(%d,%d)\n", resXlist[res], resYlist[res]);
	   }
	   else
	   {
	       SET(sstg->clip0min, 0);
	       SET(sstg->clip0max, (resYlist[res] << 16) | resXlist[res]);
	       cmdops &= ~SSTG_CLIPSELECT;
	       gdbg_info(2, "cliprect 0 enabled and set to [0, 0], ");
	       gdbg_info_more(2, "(%d,%d)\n", resXlist[res], resYlist[res]);
	   }

	   for (centerPt = centerPtInitial; centerPt < 4; centerPt++)
	   {
	       if (centerPt % 2)
		   xCenter = resXlist[res];
	       else
		   xCenter = 0;
	       if (centerPt < 2)
		   yCenter = 0;
	       else
		   yCenter = resYlist[res];

	       gdbg_info(3, "center point: (%d,%d)\n", xCenter, yCenter);

	       for (pos = posInitial; pos < 5; pos++)
	       {
		   if (pos == 4)
		   {
		       xd = xCenter - width/2;
		   ...
		   xs = (resXlist[res]/2) - iRandom(maxWidth);
		   ys = (resYlist[res]/2) - iRandom(height);

		   gdbg_info(4, "dest xy: (%d,%d)\n", xd, yd);
