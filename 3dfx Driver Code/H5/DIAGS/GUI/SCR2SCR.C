...
//    fxHalPutenv("SST_FBI_MEM=4");
  ...
  while (DIAG_STARTPASS())			// for each pass
  {
	...
	width = (maxWidth * 2) + iRandom(maxWidth);
	height = maxHeight - iRandom(maxHeight/2);

	srcX = iRandom(diago.xmaxscreen - width) + width - 1;
	if (srcX < 0)
	    srcX = 0;
	else if ((srcX + (int)width) >= diago.xmaxscreen)
	    srcX -= width;
	
	srcY = iRandom(diago.ymaxscreen -
		       (2 * maxHeight)) + (2 * maxHeight) - 1;
	if (srcY < 0)
	    srcY = 0;
	else if ((srcY + (int)height) >= diago.ymaxscreen)
	    srcY -= height;
	
	for (dstXindex = -2; dstXindex <= 2; dstXindex++)
	for (dstYindex = -2; dstYindex <= 2; dstYindex++)
	{
	    // note that we are testing an in-place copy!
	    if (dstXindex == -3)
		dstX = srcX - width;
	    else if (dstXindex == -2)
		dstX = srcX - (width/2);
	    ...
	    my_do_blt(sstg, srcX, srcY, dstX, dstY, width, height, cmdops);
