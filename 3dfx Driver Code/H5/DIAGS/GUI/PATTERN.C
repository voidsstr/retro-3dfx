...
main (int argc, char **argv)
{
  ...
  while (DIAG_STARTPASS())			// for each pass
  {
    ...
    for (patIndex = patIndexInitial; patIndex < 2; patIndex++)
    {
	if (patIndex == 0)
	{
	    // mono pattern
	...
	for (patx = 0; patx <= 7; patx++)
	for (paty = 0; paty <= 7; paty++)
	{
	    destX = iRandom(diago.xmaxscreen - 9);
	    destY = iRandom(diago.ymaxscreen - 9);

//XCOVER[destX % 8][patx] += 1;
//YCOVER[destY % 8][paty] += 1;
	    
	    cmdops &= ~(SSTG_X_PATOFFSET | SSTG_Y_PATOFFSET);
	    cmdops |= patx << SSTG_X_PATOFFSET_SHIFT;
	    cmdops |= paty << SSTG_Y_PATOFFSET_SHIFT;
	    cmdops |= (FxU32)0xf0 << SSTG_ROP0_SHIFT;	//patcopy

	    mydorect(sst, sstg, destX, destY, width, height, cmdops,
