...
int main(int argc, char **argv)
{
  ...
  while(DIAG_STARTPASS()) 
      ...
      for(j=0; j<nColors; j++)
	{
	  ...
	  for(i=0; i<256; i++)
	    {
	      downloadPaletteEntry(sst, i, r, g, b);
	      fxHalIdle(sst);
	    }
	  
	  for(yOffset=0; yOffset < 400; yOffset += 100)
	    {		  	      
	      for(i=0; i<256; i++)
		{	  
		  if(singleIndex >= 0)
...
void downloadTexture(SstRegs *sst, FxU32 paletteEntry)
{
  ...
  //Calculate the offset to the 8x8 lod
  lodOffset = 256*256 + 128*128 + 64*64 + 32*32 + 16*16;
...
void setup(SstRegs *sst)
{
  ...
  //Force lod to 8x8 size
  SET(sst->tLOD, ((5<<2) << SST_LODMIN_SHIFT) | ((5<<2) << SST_LODMAX_SHIFT));
