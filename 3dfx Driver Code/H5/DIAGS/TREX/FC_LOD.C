...
void do_1pix_tri_1q (
		SstRegs *sst,
		int *tri_cnt,
		volatile long *tri_regs[4],	// register set for the group
...
//	};
	...
	if (diago.rgb == 16)
	    SET(sst->fbzMode, SST_RGBWRMASK | SST_ENALPHABUFFER | drawbufferRandom());
	else
...
//
	...
	for (cnt = 0; cnt < 200; cnt++) {
		...
		//   2 ^ (4.0 - lod_no_w) = w
		SETF(sst->Fw, flt_clmp_w((float)pow(2.0, (4.0 - lod_no_w))));
	...
	for (cnt = 0; cnt < 400; cnt++) {
		...
		if ((tri_cnt & 7) == 0) {
			...
			lod_flt = 4.0f * ((float) (iRandom(0x7fffffff))) 
							  / ((float) (0x40000000));
