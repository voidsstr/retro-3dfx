...
**
** $Header: gtexdl.c, 11, 10/11/00 8:27:25 PM, Brent$
** $Log: 
          ...
          if (format != GR_TEXFMT_ARGB_CMP_FXT1) {
            ...
            if (levelSize < SST_TEXTURE_ALIGN) {
              ...
              while(maxLod < GR_LOD_LOG2_256) {
                ...
                texOffset += levelSize;
#else
				// There were two bugs in this bit of code, these are fixed here
                levelSize = (_grMipMapHostSize[aspectIndex][maxLod]
                             * formatMult)>>3; // Bug 1: the bits to bytes convertion was ommitted
                if (levelSize >= SST_TEXTURE_ALIGN) break;

				// Bug 2: no check was made on the Even/Odd mask to see if the mip-map affects
				//        this TMU
