...
**
** $Header: gtex.c, 38, 10/11/00 8:27:21 PM, Brent$
** $Log: 
...
#include "q3dglide.h" /* For definition of GR_MIPMAP_PERFORMANCE_TRILINEAR_Q3D */

/* This function implements the Performance trilinear feature. This works in 4SP FSAA by adjusting the lodBias on each chip
   to give an effective 2bit blending between mipmap levels. The texture must otherwise be set up as a normal bilinear texture.
   This means that only one TMU is used. This gives us two major advantages:

   1) With no multi-texturing, 2 pixel per clock mode can be used.
   2) With multi-texturing, trilinear can be used along with multitexture.

   This funtion MUST be called after each time the tLOD register has been written.if performance trilinear is enabled for that TMU.
   If it is not enabled, then the tLOD register write will have been broadcast to all chips and will have removed the lodBias adjustments
   set up by this routine, hence disabling performance trilinear if it had previously been enabled.
   */
void SetPerformanceTrilinearQ3D(GrChipID_t tmu, FxU32 tLOD_base)
{
#define FN_NAME "SetPerformanceTrilinearQ3D"

	FxU32 tLod, chipMask,numPasses;
    FxI32 oldLodBias,newLodBias;
	unsigned int i;

  	static FxI32 chipLodBias[] =
				// 4.2 format for tLod register
				//   0.00, 0.5, 0.250, 0.75 - works for 2 chip, 4 chip, and 8 chip
				{
                    0x00<<SST_LODBIAS_SHIFT, 0x02<<SST_LODBIAS_SHIFT, 0x01<<SST_LODBIAS_SHIFT, 0x03<<SST_LODBIAS_SHIFT
				};


  	GR_BEGIN_NOFIFOCHECK("SetPerformanceTrilinearQ3D", 88);
    if((gc->grPixelSample >= 4) /*&& (!gc->state.mode2ppc || (tmu == gc->state.mode2ppcTMU))*/)
    {
        oldLodBias = tLOD_base & SST_LODBIAS;
        tLOD_base &= ~(SST_LODBIAS);

        /* Sign extend it. */
  	    oldLodBias = ((oldLodBias << (32-6-SST_LODBIAS_SHIFT)) >> (32-6-SST_LODBIAS_SHIFT));

        if(gc->chipCount == 8)
            chipMask = 0x11; /* we can modify 2 chips at a time */
        else
            chipMask = 0x01;
        ...
        if(gc->chipCount >= 4)
            ...
            numPasses = gc->chipCount;

        /* don't need to modify chips 0 and 4 as the lodBias adjustment is 0 */
	    for (i = 1; i < numPasses; i++)	
	    {
		    newLodBias = oldLodBias + chipLodBias[i];

  		    if(newLodBias > (0x1f<<SST_LODBIAS_SHIFT)) newLodBias = (0x1f<<SST_LODBIAS_SHIFT);
  		    /*if(newLodBias < (-0x20<<SST_LODBIAS_SHIFT)) newLodBias = (-0x20<<SST_LODBIAS_SHIFT); This check is not required as we always add a positve value */
  		    /* Mask it back off. */
  		    newLodBias &= SST_LODBIAS;
	  	    tLod = tLOD_base | newLodBias;

  		    {
    		    SstRegs* tmuHw = SST_TMU(hw, tmu);
    		    _grChipMask( chipMask << i );
...
#ifdef Q3D
  // Hack to allow textures loaded using GR_MIPMAPLEVELMASK_BOTH to a 'single TMU' whilst
  // using UMA texture extension (grEnable(GR_TEXTURE_UMA_EXT)) to use single pass trilinear.
  // This will only work if GR_COMBINE_FACTOR_LOD_FRACTION is used as the blending factor on
  // TMU 0. Using GR_COMBINE_FACTOR_ONE_MINUS_LOD_FRACTION will give incorrect results.
  if((texturemode & SST_TRILINEAR) && 
...
#define FN_NAME "grTexMipMapMode"
  ...
  /*--------------------------------------------------------------
    ...
    --------------------------------------------------------------*/
#if Q3D_FC_PERFORMANCE_TRILINEAR
  if(_GlideRoot.environment.forcePerformanceTrilinear)
  {
      if(mmMode == GR_MIPMAP_NEAREST_DITHER || mmMode == GR_MIPMAP_NEAREST)
          mmMode = GR_MIPMAP_PERFORMANCE_TRILINEAR_Q3D;
  }
#endif
  switch (mmMode) {
...
#if Q3D_FC_PERFORMANCE_TRILINEAR
  case GR_MIPMAP_PERFORMANCE_TRILINEAR_Q3D:
#endif
...
#if Q3D_FC_PERFORMANCE_TRILINEAR
  if(mmMode == GR_MIPMAP_PERFORMANCE_TRILINEAR_Q3D)
  {
      gc->state.per_tmu[tmu].performanceTrilinear = FXTRUE;
      mmMode = GR_MIPMAP_NEAREST; /* As far as any other state checks go this is the case */
  }
  else
      gc->state.per_tmu[tmu].performanceTrilinear = FXFALSE;
#endif
