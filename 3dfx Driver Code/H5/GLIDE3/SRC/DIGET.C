...
**
** $Header: diget.c, 22, 10/11/00 8:26:32 PM, Brent$
** $Log: 
...
#define FN_NAME "grGet"
  ...
  case GR_MEMORY_TMU:
    if (plength == 4) {
      ...
      {
          if (gc->open)
            *params = (gc->tmu_state[0].total_mem * _GlideRoot.GCs[_GlideRoot.current_sst].num_tmu);
          else
            *params = 2 << 20;
      }
      else
...
#if Q3D_FC_PERFORMANCE_TRILINEAR
      if(_GlideRoot.environment.forcePerformanceTrilinear)
            *params = 1; /* We tell the app there is only 1 TMU to force it to not use full tri-linear,
                      hopefully it will revert to bilinear, if it reverts to 2 pass trilinear
                      then this hack won't work. */
      else
#endif
