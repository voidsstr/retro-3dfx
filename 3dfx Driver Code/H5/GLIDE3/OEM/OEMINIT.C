...
#define kOEMSwapControlTestSize 0x10UL

/* This function, along with fxoemSwapControlWrite(), gives the OEM
 * DLL an opportunity to inform Glide that it has data that it wants
 * to write to the hardware during an application call to
 * grBufferSwap() immediately after Glide has setup any necessary
 * hardware swap data. Both of these functions must be present in the
 * OEM DLL for the swap control functionality to be enabled from
 * within Glide.
 *
 * The OemInitInfo structure will
 * contain information to disambiguate the currently selected board from
 * any others currently in use, and will be one that the OEM DLL has been
 * notified of being mapped via the fxoemInitMapBoard() entrypoint.  
 *
 * A zero return value indicates thats the OEM DLL has no data to
 * write, and any other value is the number of bytes that it would
 * like to write to the hardware. The return value should include the
 * size of any command fifo packet headers necessary for the correct
 * interpretation of the data by the hardware.  
 *
 * NB: This test implementation always writes a bunch of nop packets to
 * make sure it doesn't kill the hw and that the handshake works.
 */
FX_EXPORT FxU32 FX_CSTYLE 
fxoemSwapControlP(OemInitInfo* oemInfo)
{
#define FN_NAME "fxoemSwapControlP"
  DumpOemInitInfo(FN_NAME, oemInfo);

  return kOEMSwapControlTestSize;
#undef FN_NAME
}

/* The OemInitInfo structure will contain information to disambiguate
 * the currently selected board from any others currently in use, and
 * will be one that the OEM DLL has been notified of being mapped via
 * the fxoemInitMapBoard() entrypoint. This will be the same parameter
 * block passed to the fxoemSwapControlP() entrypoint.
 *
 * When using this entrypoint, it is up to the oem dll to disambiguate
 * between the different command transport mechanisms on the various
 * 3dfx hw. This can be done by examining the fields of the
 * OemInitInfo structure that is passed to the callbacks.
 *
 * On hw w/ direct writes the writeBuffer parameter is actually a
 * pointer to the base of the memory mapped register set. It is up to
 * teh oem dll writers to write valid data.
 *
 * On hw w/ command fifos the writeBuffer parameter is a pointer to a
 * buffer at least as large as allocated by the return value from the
 * fxoemSwapControlP() entrypoint. 
 * 
 * The data written to the buffer is not guaranteed to be written to
 * the hardware until the completion of the call to this entrypoint.
 * 
 * A zero return value indicates that the OEM DLL changed its
 * anthropomorphic mind and decided not write any data after all. A
 * non-zero return value indicates the number of bytes written to the
 * writeBuffer, and must match the value returned from the previous
 * call to the fxoemSwapControlP() entrypoint.  
 */
FX_EXPORT FxU32 FX_CSTYLE 
fxoemSwapControlWrite(OemInitInfo* oemInfo,
                      FxU32* writeBuffer)
{
#define FN_NAME "fxoemSwapControlWrite"
  FxU32 i;

  DumpOemInitInfo(FN_NAME, oemInfo);
  
  /* Write to the hw that we know about */
  switch(oemInfo->deviceID) {
  case 1: /* sst1 */
    /* Write to somewhere 'safe' (nopCMD) */
    for(i = 0; i < (kOEMSwapControlTestSize >> 0x02UL); i++) {
      writeBuffer[0x48UL] = 0x00UL;
    }
    break;

  case 2: /* voodoo2 */
    /* Writes nops to the command stream. This is not
     * the same as the nopCMD's that are written above.
     */
    for(i = 0; i < (kOEMSwapControlTestSize >> 0x02UL); i++) {
      *writeBuffer++ = 0x00UL;
    }
  ...
  default:
    MyDebugPrintf(outFile, FN_NAME": Unknown device(0x%X).\n", oemInfo->deviceID);
    break;
  }
