...
static char *test_name = "ext-fogcoord";
static char *test_description = "Test per vertex fog functionality";

#define MAX_FOG_ENTRY 256

// local data
typedef struct {
  GrVertex verts[4];  // Temporary vertex storage
  GrFog_t table[MAX_FOG_ENTRY];
} test_data;
...
int do_conform(conform_state *state)
{
  ...
  int odd;
  char *ext;
  FxI32 fog_entries = MAX_FOG_ENTRY + 10;

  ext = (char *)grGetString(GR_EXTENSION);

  if(!strstr(ext,"FOGCOORD")) {
    log_perror(state,"Accelerator does not support FOGCOORD extension");
    return(0);
  }

  // sanity check fog table size
  grGet(GR_FOG_TABLE_ENTRIES, sizeof(fog_entries), &fog_entries);

  if (fog_entries > MAX_FOG_ENTRY) {
    log_perror(state, "test needs more fog table space");
    return TEST_FAIL;
  }
  ...
  grConstantColorValue(rgbacolor(state,0xffffffff));
  ...
  guFogGenerateExp(local_ptr->table, 0.5f);
  
  grFogTable(local_ptr->table);
  
  // green fog
  grFogColorValue(rgbacolor(state, 0x00ff00ff));
  
  for(depth = 0.7f; depth < 4.71f; depth += 0.5f) {
    for(odd = 0; odd < 3; odd++) {
      switch(odd) {
      case 1:
	local_ptr->verts[0].fog = 0.2f;
	local_ptr->verts[1].fog = depth;
	local_ptr->verts[2].fog = depth;
	local_ptr->verts[3].fog = 0.2f;
	break;
      case 2:
	local_ptr->verts[0].fog = depth;
	local_ptr->verts[1].fog = depth;
	local_ptr->verts[2].fog = 0.2f;
	local_ptr->verts[3].fog = 0.2f;
	break;
      default:
	local_ptr->verts[0].fog = 0.2f;
	local_ptr->verts[1].fog = 0.2f;
	local_ptr->verts[2].fog = depth;
	local_ptr->verts[3].fog = depth;
	break;
      }
