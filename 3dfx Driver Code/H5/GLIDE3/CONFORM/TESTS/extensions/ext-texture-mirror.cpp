...
static char *test_name = "ext-texture-mirror";
static char *test_description = "Test mirroring of single level textures";
...
int init_conform(conform_state *state, int local_argc, char **local_argv)
{
  ...
  // this is manditory
  frame_name("ext-texture-mirror",state);
...
int do_conform(conform_state *state)
{
  ...
  if(!strstr(ext,"TEXMIRROR")) {
    log_perror(state,"Accelerator does not support TEXMIRROR extension");
    return(0);
  }
  ...
  for(type = 0; type < 4 ; type++) {
	...
	case 2:
	  grTexClampMode(local_ptr->curtmu, 
			 GR_TEXTURECLAMP_WRAP, GR_TEXTURECLAMP_MIRROR_EXT);
	  break;
	default:
	  grTexClampMode(local_ptr->curtmu, 
			 GR_TEXTURECLAMP_MIRROR_EXT, GR_TEXTURECLAMP_WRAP);
	  break;
	}
		 ...
		 FxU16 color1, FxU16 color2, FxU16 color3)
{
  ...
  base = (float)(1<<((7-level)+7));

  // setup vertex data for all TMUs
  for(i = 0; i < GRVERTEX_NUM_TMU; i++) {
    local_ptr->verts[0].tmuvtx[i].sow = 128.0f - base;
    local_ptr->verts[0].tmuvtx[i].tow = 128.0f - base;
    local_ptr->verts[0].oow = oow;  
    local_ptr->verts[0].tmuvtx[i].oow = oow;
    
    local_ptr->verts[1].tmuvtx[i].sow = 128.0f + base;
    local_ptr->verts[1].tmuvtx[i].tow = 128.0f - base;
    local_ptr->verts[1].oow = oow;  
    local_ptr->verts[1].tmuvtx[i].oow = oow;
    
    local_ptr->verts[2].tmuvtx[i].sow = 128.0f + base;
    local_ptr->verts[2].tmuvtx[i].tow = 128.0f + base;
    local_ptr->verts[2].oow = oow;  
    local_ptr->verts[2].tmuvtx[i].oow = oow;
    
    local_ptr->verts[3].tmuvtx[i].sow = 128.0f - base;
    local_ptr->verts[3].tmuvtx[i].tow = 128.0f + base;
