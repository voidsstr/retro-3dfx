...
static char *test_name = "ext-palette";
static char *test_description = "Test palette 6666 format";
...
void texture_gen(int alpha, int partial, int aspect, test_data *local_ptr);
void byte_texture(FxU8 *tmp, FxU8 *shadow, int height, int width,
		  FxU8 red, FxU8 grn);
void texture_gen_palette(int alpha, int partial, FxU8 *shadow, 
			 FxU8 *red, FxU8 *grn);
...
int do_conform(conform_state *state)
{
  ...
  for(alpha = 0; alpha < 2; alpha++) {
    for(partial = local_ptr->partial_start; 
	partial < (local_ptr->partial_end+1);
	partial++) {
      for(aspect = 0; aspect < 7; aspect++) { 
	grBufferClear(clearcolor,0x0000,0x00000000);
	
	local_ptr->info.data = buffer;
	texture_gen(alpha, partial, aspect, local_ptr);
...
void texture_gen(int alpha, int partial, int aspect, test_data *local_ptr)
{
  ...
  texture_gen_palette(alpha, partial, shadow, &red, &grn); 
			 ...
			 FxU8 *red, FxU8 *grn)
{
  ...
  for(i=0;i<64;i++) {
    ...
    if(alpha) {
      palette[shadow[pos++]] = 
	(((FxU32)i)<<18)|(((FxU32)i)<<10)|(((FxU32)i)<<2)|0xfc000000;
    } else {
      palette[shadow[pos++]] = 
	(((FxU32)i)<<18)|(((FxU32)i)<<10)|(((FxU32)i)<<2)|0xfc000000;
    }
  ...
  for(i=0;i<64;i++) {
    ...
    if(alpha) {
      palette[shadow[pos++]] = (((FxU32)i)<<18)|0x7c000000;
    } else {
      palette[shadow[pos++]] = (((FxU32)i)<<18)|0xfc000000;
    }
  }
  ...
  for(i=0;i<64;i++) {
    ...
    if(alpha) {
      palette[shadow[pos++]] = (((FxU32)i)<<10)|0x3c000000;
    } else {
      palette[shadow[pos++]] = (((FxU32)i)<<10)|0xfc000000;
    }
  }
  ...
  for(i=0;i<64;i++) {
    ...
    shadow[pos] = tmp;
    if(alpha) {
      palette[shadow[pos++]] = (((FxU32)i)<<2)|0xcc000000;
    } else {
      palette[shadow[pos++]] = (((FxU32)i)<<2)|0xfc000000;
    }
  }
  ...
  if(partial) {
	...
	if(!entry_full[i]) {
      ...
      grTexDownloadTablePartial(GR_TEXTABLE_PALETTE_6666_EXT,
				palette,start,end);
    ...
    // download full table
    grTexDownloadTable(GR_TEXTABLE_PALETTE_6666_EXT,palette);
  }
}
