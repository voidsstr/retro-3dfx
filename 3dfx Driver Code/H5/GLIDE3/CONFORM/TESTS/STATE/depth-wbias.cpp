...
static char *test_name = "depth-wbias";
static char *test_description = "Test W buffer bias";
...
int do_conform(conform_state *state)
{
  ...
  emu_draw_at_bias(state, &local_ptr->verts[0], 0, 
                   GR_DEPTHBUFFER_WBUFFER, GR_CMP_GREATER, RGBA8888_RED);

  if (!log_status(state, check_frame(state), "wbuffer bias 0 check")) {
      return(TEST_FAIL);
  }
  ...
  emu_draw_at_bias(state, &local_ptr->verts[4], 1,
                   GR_DEPTHBUFFER_WBUFFER, GR_CMP_GREATER, RGBA8888_GREEN);

  if (!log_status(state, check_frame(state), "wbuffer bias 1 check")) {
      return(TEST_FAIL);
  }
  ...
  emu_draw_at_bias(state, &local_ptr->verts[8], 2,
                   GR_DEPTHBUFFER_WBUFFER, GR_CMP_GREATER, RGBA8888_BLUE);

  if (!log_status(state, check_frame(state), "wbuffer bias 2 check")) {
      return(TEST_FAIL);
  }
  ...
  draw_at_bias(state, &local_ptr->verts[0], 0);
  emu_draw_at_bias(state, &local_ptr->verts[0], 0,
                   GR_DEPTHBUFFER_WBUFFER, GR_CMP_EQUAL, RGBA8888_WHITE);
  ...
  if (!log_status(state, check_frame(state), "wbuffer compare to bias 0 check")) {
      return(TEST_FAIL);
  }
  ...
  if (!log_status(state, check_frame(state), "wbuffer compare to bias 1 check")) {
      return(TEST_FAIL);
  }
  ...
  if (!log_status(state, check_frame(state), "wbuffer compare to bias 2 check")) {
      return(TEST_FAIL);
  }
...
#ifdef ORIGIN_LL
  // always use LL origin
  ystart = state->conform_height - yloc - height;
#else // ORIGIN_UL, default
  ystart = yloc;
#endif // NOT ORIGIN_LL
  yend = ystart + height;
