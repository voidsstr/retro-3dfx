...
static char *test_name = "color-constant";
static char *test_description = "Test constant color";
...
int init_conform(conform_state *state, int local_argc, char **local_argv)
{
  ...
  // this is manditory
  frame_name("color-constant",state);
...
int do_conform(conform_state *state)
{
  ...
  for(alpha = 0; alpha < 2; alpha++) {
    ...
    if(alpha) {
      grColorCombine(GR_COMBINE_FUNCTION_BLEND_LOCAL,
    ...
    for(y = 0; y < 64; y++) {
      for(x = 0; x < 64; x++) {
	// Init vertex data
	local_ptr->verts[0].x = 22.0f + (float)x;
	local_ptr->verts[0].y = 22.0f + (float)y;
	local_ptr->verts[1].x = 22.9f + (float)x;
	local_ptr->verts[1].y = 22.0f + (float)y;
	local_ptr->verts[2].x = 22.9f + (float)x;
	local_ptr->verts[2].y = 22.9f + (float)y;
	local_ptr->verts[3].x = 22.0f + (float)x;
	local_ptr->verts[3].y = 22.9f + (float)y;
