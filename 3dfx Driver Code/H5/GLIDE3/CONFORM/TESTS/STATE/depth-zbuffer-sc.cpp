...
static char *test_name = "depth-zbuffer-sc";
...
// Z depths we'll use for testing
...
int do_conform(conform_state *state)
{
  ...
  for(mode = 0; mode < NUM_MODES; mode++) { 
    for (depth = 0; depth < 3; depth++) {
      ...
      switch (depth) {
        case 1: // ==
          //box2_depth =  1.0f/(float)(depthrange[0]  * 0.5f);
          box2_depth =  depthrange[0]/MID;
          break;
        case 2: // <<
          //box2_depth =  1.0f/(float)(depthrange[0]  * 0.75f);
          box2_depth =  depthrange[0]/FARTHEST;
          break;
        default: // << 
          //box2_depth =  1.0f/(float)(depthrange[0]  * 0.25f);
          box2_depth =  depthrange[0]/NEAREST;
          break;
      }
                      ...
                      FxBool depthmask)
{
  ...
  // We'll use RGBA8888 format for our color buffer
  FxU32 *color_buffer = 
                       ...
                       FxU32 *expected_ptr, FxU32 *screen_ptr)
{
  ...
  if (!expected_ptr || !screen_ptr) {
    ...
    log_message(state, "FAIL: can't compare NULL image data!");
    status = TEST_FAIL;
