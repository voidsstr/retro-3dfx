...
static char *test_name = "texture-load-format";
static char *test_description = "Test texture formats";
...
void texture_gen(int aspect, int format, test_data *local_ptr);
void texture_info(int aspect, int format, test_data *local_ptr);
void word_texture(FxU16 *tmp, int height, int width, 
		  FxU16 red, FxU16 grn, FxU16 blu_a, FxU16 blu);
void byte_texture(FxU8 *tmp, int height, int width, 
		  FxU8 red, FxU8 grn, FxU8 blu_a, FxU8 blu);

#define NUM_FORMAT 11
...
int init_conform(conform_state *state, int local_argc, char **local_argv)
{
  ...
  // this is manditory
  frame_name("texture-load-format",state);
...
int do_conform(conform_state *state)
{
  ...
  for(format = local_ptr->start; format < (local_ptr->end+1); format++) {
    for(aspect = 0; aspect < 7; aspect++) { 
      ...
      // Precalculate info for grTexCalcMemRequired
      texture_info(aspect, format, local_ptr);

      // get texture buffer
      if((buffer = (FxU16 *)malloc(
		   grTexCalcMemRequired(local_ptr->info.smallLodLog2,
					local_ptr->info.largeLodLog2,
					local_ptr->info.aspectRatioLog2,
					local_ptr->info.format))) == NULL) {
	// fail
	log_message(state,"Could not allocate memory buffer for texture map");
      }

      local_ptr->info.data = buffer;
      texture_gen(aspect, format, local_ptr);
...
void help_conform(conform_state *state)
{
  ...
  fprintf(stderr,"          1   GR_TEXFMT_8BIT\n");
  fprintf(stderr,"          2   GR_TEXFMT_RGB_332\n");
  fprintf(stderr,"          3   GR_TEXFMT_RGB_565\n");
  fprintf(stderr,"          4   GR_TEXFMT_16BIT\n");
  fprintf(stderr,"          5   GR_TEXFMT_ARGB_8332\n");
  fprintf(stderr,"          6   GR_TEXFMT_ARGB_1555\n");
  fprintf(stderr,"          7   GR_TEXFMT_ARGB_4444\n");
  fprintf(stderr,"          8   GR_TEXFMT_INTENSITY_8\n");
  fprintf(stderr,"          9   GR_TEXFMT_ALPHA_INTENSITY_44\n");
  fprintf(stderr,"         10   GR_TEXFMT_ALPHA_INTENSITY_88\n");
  fprintf(stderr,"         11   GR_TEXFMT_ALPHA_8\n");
}
...
void texture_gen(int aspect, int format, test_data *local_ptr)
{
  ...
  switch(format) {
  case 0:
    local_ptr->info.format = GR_TEXFMT_8BIT;
    byte_texture((FxU8 *)local_ptr->info.data,
		 height, width,
		 0xe0, 0x1c, 0x03, 0x03);
    break;
  case 1:
    local_ptr->info.format = GR_TEXFMT_RGB_332;
    byte_texture((FxU8 *)local_ptr->info.data,
		 height, width,
		 0xe0, 0x1c, 0x03, 0x03);
    break;
  case 2:
    ...
    word_texture((FxU16 *)local_ptr->info.data,
		 height, width,
		 0xf800, 0x07e0, 0x001f, 0x001f);
    break;
  case 3:
    local_ptr->info.format = GR_TEXFMT_16BIT;
    word_texture((FxU16 *)local_ptr->info.data,
		 height, width,
		 0xffe0, 0xff1c, 0xff03, 0x0003);
    break;
  case 4:
    local_ptr->info.format = GR_TEXFMT_ARGB_8332;
    word_texture((FxU16 *)local_ptr->info.data,
		 height, width,
		 0xffe0, 0xff1c, 0xff03, 0x0003);
    break;
  ...
  case 6:
    ...
    break;
  case 7:
    local_ptr->info.format = GR_TEXFMT_INTENSITY_8;
    byte_texture((FxU8 *)local_ptr->info.data,
		 height, width,
		 0xff, 0x7f, 0x3f, 0x3f);
    break;
  case 8:
    local_ptr->info.format = GR_TEXFMT_ALPHA_INTENSITY_44;
    byte_texture((FxU8 *)local_ptr->info.data,
		 height, width,
		 0xff, 0xf8, 0xf3, 0x03);
    break;
  case 9:
    ...
    word_texture((FxU16 *)local_ptr->info.data,
		 height, width,
		 0xffff, 0xff7f, 0xff3f, 0x003f);
    break;
  case 10:
    local_ptr->info.format = GR_TEXFMT_ALPHA_8;
    byte_texture((FxU8 *)local_ptr->info.data,
		 height, width,
		 0xff, 0xff, 0xff, 0x00);
    break;
