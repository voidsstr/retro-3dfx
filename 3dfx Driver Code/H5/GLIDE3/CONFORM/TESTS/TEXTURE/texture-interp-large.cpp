...
static char *test_name = "texture-interp-large";
static char *test_description = 
  "test texture interpolation across various triangles, using large texture coords";
...
// Could be this:
// #define MAX_TEXTURE_COORD (int)(32767.0f * 0.7f)
// But this produces more easily distinguishable results
#define MAX_TEXTURE_COORD (10 * 256)  // 10 times texture size, though this really
                                      // means 20 times, since our texture coords
                                      // can be from - to + MAX_TEXTURE_COORD
...
// The combinations of rotations that we'll do. Uncomment these for more
...
typedef struct {
  ...
  float tex_left;      // texture coords, left 
  float tex_right;     // texture coords, right
  float tex_top;       // texture coords, top
  float tex_bottom;    // texture coords, bottom
...
void xform_verts(test_data *local_ptr,
  ...
  for(int i = 0; i < num_verts; i++) {
    ...
    // If applicable, adjust texture coords before applying scale and offset.
    // The idea is that our triangle will be rotated on top of the texture.
    if (local_ptr->do_adj_tex_coords) {
      float tex_half_width  = (local_ptr->tex_right - local_ptr->tex_left) / 2.0f;
      float tex_half_height = (local_ptr->tex_bottom - local_ptr->tex_top) / 2.0f;
      float tex_center_x = local_ptr->tex_right - tex_half_width;
      float tex_center_y = local_ptr->tex_bottom - tex_half_height;

      out_verts[i].tmu[0].s = 
                    ((tex_half_width * out_verts[i].x) + tex_center_x)/divisor; 
      out_verts[i].tmu[0].t =  
                    ((tex_half_height * out_verts[i].y) + tex_center_y)/divisor; 
    }
...
FxBool
gen_texture(conform_state *state, test_data *local_ptr)
{
  GrTexInfo txinfo;
  ...
  FxU16 colors[] = {
    ARGB_1555(1, 255, 0, 0), // red
    ARGB_1555(1, 0, 255, 0), // green
    ARGB_1555(1, 0, 0, 255), // blue
  };

#define NUM_COLORS (sizeof(colors) / sizeof(FxU16))

  // Generate a texture. Give it a cyan border on the top/left
  // edges, and a yellow one on the bottom/right. Iterate through
  // our color array in the middle. 
  FxU16 *data_ptr = &texture_data[0];
  for (int y = 0; y < y_size; y++) {
    for (int x = 0; x < x_size; x++) {
      if (x == 0 || y == 0) { // top and left edges
        *data_ptr++ = ARGB_1555(1, 0, 255, 255);
      } else if (x == x_size-1 || y == y_size-1) { // bottom & right
        *data_ptr++ = ARGB_1555(1, 255, 255, 0);
      } else {
        *data_ptr++ = colors[c++ % NUM_COLORS];
      }
    }
  }
...
void init_tri_verts(test_data *local_ptr) {
  ...
  */
  ...
  // Pick random texture coords
  float tex_left   = local_ptr->tex_left   = -(float)nrand(MAX_TEXTURE_COORD);
  float tex_right  = local_ptr->tex_right  =  (float)nrand(MAX_TEXTURE_COORD);
  float tex_top    = local_ptr->tex_top    = -(float)nrand(MAX_TEXTURE_COORD);
  float tex_bottom = local_ptr->tex_bottom =  (float)nrand(MAX_TEXTURE_COORD);

  verts[0].x = -1.0f; verts[0].y = -1.0f;       
  verts[0].tmu[0].s = tex_left/divisor;    // LL
  verts[0].tmu[0].t = tex_top/divisor;
  verts[0].tmu[0].q = 1.0f;

  verts[1].x = -1.0f; verts[1].y =  1.0f;       
  verts[1].tmu[0].s = tex_left/divisor;    // UL
  verts[1].tmu[0].t = tex_bottom/divisor;
  verts[1].tmu[0].q = 1.0f;

  verts[2].x =  0.0f; verts[2].y =  0.0f;       
  verts[2].tmu[0].s = ((tex_left + tex_right)/2.0f)/divisor; // Center
  verts[2].tmu[0].t = ((tex_top + tex_bottom)/2.0f)/divisor;
  verts[2].tmu[0].q = 1.0f;

  verts[3].x =  1.0f; verts[3].y =  1.0f;
  verts[3].tmu[0].s = tex_right/divisor;   // UR
  verts[3].tmu[0].t = tex_bottom/divisor;
  verts[3].tmu[0].q = 1.0f;

  verts[4].x =  1.0f; verts[4].y = -1.0f;       // LR
  verts[4].tmu[0].s = tex_right/divisor; 
  verts[4].tmu[0].t = tex_top/divisor;
...
int init_conform(conform_state *state, int local_argc, char **local_argv)
{
  ...
  local_ptr->cur_rep = 0;
  local_ptr->debug = 0;
  local_ptr->texture_lod = GR_LOD_LOG2_4; // default to 4x4 texture
  ...
  while(--local_argc) {
    if(!_stricmp(*local_argv,"-nr") || // number of rotations
      ...
      --local_argc;
    } else if(!_stricmp(*local_argv,"-lod") || // set LOD of texture
      ...
      switch(lod) {
        ...
        case 256: local_ptr->texture_lod = GR_LOD_LOG2_256; 
                  ...
                  break;
        default: {
          sprintf(tmp_str,"invalid lod: %d - defaulting to 4", lod);
          log_message(state, tmp_str);
          local_ptr->texture_lod = GR_LOD_LOG2_4; 
          local_ptr->texture_wh = 4;
          break;
        }
      }
