...
static char *test_name = "texture-interp";
static char *test_description = 
  "test texture interpolation across various triangles";
...
#define NUM_COLORS (sizeof(colors) / sizeof(FxU16))
  ...
  int col_mult_x = 255/x_size;
  int col_mult_y = 255/y_size;

  // Generate a texture. Give it a cyan border on the bottom/left
  // edges, and a yellow one on the top/right. Iterate through
  // our color array in the middle. 
  FxU16 *data_ptr = &texture_data[0];
  for (int y = 0; y < y_size; y++) {
    int c = y % NUM_COLORS;
    for (int x = 0; x < x_size; x++) {
      if (x == 0 || y == 0) { // bottom and left edges
        *data_ptr++ = ARGB_1555(1, 0, 255, 255);
      } else if (x == x_size-1 || y == y_size-1) { // top & right
...
int init_conform(conform_state *state, int local_argc, char **local_argv)
{
  ...
  while(--local_argc) {
    if(!_stricmp(*local_argv,"-nr") || // number of rotations
      ...
      --local_argc;
    } else if(!_stricmp(*local_argv,"-lod") || // select a LOD for the texture
