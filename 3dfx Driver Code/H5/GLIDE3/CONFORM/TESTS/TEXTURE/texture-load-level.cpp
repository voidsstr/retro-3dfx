...
static char *test_name = "texture-load-level";
static char *test_description = "Test loading of MIP mapped texture levels";
...
int init_conform(conform_state *state, int local_argc, char **local_argv)
{
  ...
  // this is manditory
  frame_name("texture-load-level",state);
...
int do_conform(conform_state *state)
{
  ...
  for(aspect = 0; aspect < 7; aspect++) { 
	...
	// get a random base address the first time thru for all levels
	if(getbase) {
	...
	// load it
	grTexDownloadMipMapLevel(local_ptr->curtmu, baseaddr, curLod,
