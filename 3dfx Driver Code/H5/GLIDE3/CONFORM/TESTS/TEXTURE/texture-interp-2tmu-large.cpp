...
static char *test_name = "texture-interp-2tmu-large";
static char *test_description = 
  "Test 2 tmu texture interpolation across various odd-shaped triangles. Apply multiples of the texture across the quad.";
...
void xform_verts(test_data *local_ptr,
  ...
  for(int i = 0; i < num_verts; i++) {
    ...
    if (local_ptr->do_adj_tex_coords) {
      ...
      out_verts[i].tmu[0].s = out_verts[i].tmu[1].s = 
                    ((tex_half_width * out_verts[i].x) + tex_center_x)/divisor; 

      out_verts[i].tmu[0].t = out_verts[i].tmu[1].t =  
...
gen_texture(conform_state *state, test_data *local_ptr, GrChipID_t tmu)
{
  ...
  if (tmu == GR_TMU0) {
    ...
    // blue 2-pixel wide cross in the center. The rest of the
    // texture is white.
    ...
    // There is a 1-pixel wide green X in the center of the texture, from
...
@html
 ...
 Set the texture coords so that they apply a multiple of the texture
 to the quad we'll draw.
@end
--------------------------------------------------------------------*/
void init_tri_verts(test_data *local_ptr) {
  ...
  */
  ...
  // We'll pick a multiple of the texture to apply on either side
  // of 0. Max number of repetitions is 32767/256 == 127.
  float tex_left   = local_ptr->tex_left   = -((nrand(6)+1) * 256.0f);
  float tex_right  = local_ptr->tex_right  =   (nrand(6)+1) * 256.0f;
  float tex_top    = local_ptr->tex_top    = -((nrand(6)+1) * 256.0f);
  float tex_bottom = local_ptr->tex_bottom =   (nrand(6)+1) * 256.0f;

  if (local_ptr->debug) {
    fprintf(stderr, "l,r,t,b = %f, %f, %f, %f\n", 
  ...
  // match TMU1's coords to TMU0's coords
  ...
  for (i = 0; i < local_ptr->num_verts; i++) {
    ...
    verts[i].tmu[0].a = verts[i].tmu[1].a = alpha;
    verts[i].q = 1.0f;
...
int init_conform(conform_state *state, int local_argc, char **local_argv)
{
  ...
  while(--local_argc) {
    if(!_stricmp(*local_argv,"-nr") || // number of rotations
      ...
      --local_argc;
    } else if(!_stricmp(*local_argv,"-lod") || // set LOD of textures
