...
static char *test_name = "texture-combine-multi";
static char *test_description = "Test texture combine with multiple TMUs";
...
void texture_gen1(int format, FxU16 *buffer);
...
int init_conform(conform_state *state, int local_argc, char **local_argv)
{
  ...
  // this is manditory
  frame_name("texture-combine-multi",state);
...
int do_conform(conform_state *state)
{
  ...
  if((buffer1 = (FxU16 *)malloc(32*32*2)) == NULL) {
    // fail
    log_message(state,"Could not allocate memory buffer for texture map");
  }
...
void texture_gen1(int format, FxU16 *buffer)
{
  ...
  for(pos = 0; pos < 32*32; pos++) {
    // flip X & Y
    flippos = ((pos>>5)&0x01f)|((pos<<5)&0x3e0);
    if(flippos < 256) {
      // grey ramp
      red = flippos;
      grn = flippos;
