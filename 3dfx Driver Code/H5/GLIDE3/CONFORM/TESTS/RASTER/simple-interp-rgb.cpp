...
static char *test_name = "simple-interp-rgb";
static char *test_description = 
  "Incrementally rotate solid RGB interpolated primitives";

// TODO: Bump NUM_ROTS_PER_PRIM up when self-checking is in!
...
typedef struct {
  ...
  int cur_rep;         // the current pass through
...
void rot_verts_z(Vertex *out_verts, Vertex *in_verts, 
    ...
    for(int i = 0; i < num_verts; i++) {
      ...
      out_verts[i].tmu[0].r = (float)nrand(255)/color_divisor;
      out_verts[i].tmu[0].g = (float)nrand(255)/color_divisor;
      out_verts[i].tmu[0].b = (float)nrand(255)/color_divisor; 
   }
}
  ...
  switch (local_ptr->cur_prim) {
    ...
    case PRIM_POINTS:
      for (v = 0; v < num_verts; v++) {
        ...
        add_vertex_data(state, &xformed_verts[v], 1); 
      }
      break;
    case PRIM_VA_LINES: 
    case PRIM_VA_LINE_STRIP: 
