...
static char *test_name = "simple-interp-alpha";
static char *test_description = 
 "Draw prims fading from opaque to transparent, against opaque lines";
...
void init_verts(test_data *td) {
  ...
  // triangle
  verts[0].x = -1.0f; verts[0].y =  0.0f; verts[0].a = 255.0f;
  verts[1].x =  1.0f; verts[1].y =  1.0f; verts[1].a = 0.0f;
  verts[2].x =  1.0f; verts[2].y = -1.0f; verts[2].a = 0.0f;

  for (i = 0; i < NUM_TRI_VERTS; i++) {
    verts[i].r = verts[i].g = verts[i].b = rgba;
  }

  // lines
  verts =  td->line_verts;
  verts[0].x = -1.0f; verts[0].y = -0.8f; verts[0].a = 0.0f;
  verts[1].x =  1.0f; verts[1].y = -0.8f; verts[1].a = 255.0f;
  verts[2].x = -1.0f; verts[2].y =  0.8f; verts[2].a = 0.0f;
  verts[3].x =  1.0f; verts[3].y =  0.8f; verts[3].a = 255.0f;
  ...
  // vertex array
  verts =  td->va_verts;
  verts[0].x = -1.0f; verts[0].y = -0.6f; verts[0].a = 255.0f;
  verts[1].x =  1.0f; verts[1].y = -0.3f; verts[1].a = 0.0f;
  verts[2].x = -1.0f; verts[2].y =  0.0f; verts[2].a = 255.0f;
  verts[3].x =  1.0f; verts[3].y =  0.4f; verts[3].a = 0.0f;
  verts[4].x = -1.0f; verts[4].y =  0.7f; verts[4].a = 255.0f;
  verts[5].x =  1.0f; verts[5].y =  0.9f; verts[5].a = 0.0f;
  ...
  verts[0].x = -1.0f; verts[0].y = -0.1f; verts[0].a = 0.0f; 
  verts[1].x =  0.0f; verts[1].y = -0.8f; verts[1].a = 255.0f/2.0f;
  verts[2].x =  1.0f; verts[2].y =  0.2f; verts[2].a = 255.0f;
  verts[3].x =  0.0f; verts[3].y =  0.7f; verts[3].a = 255.0f/2.0f;
...
int do_conform(conform_state *state)
{
  ...
  /* 
   * First, draw some vertical constant color, fully opaque lines
   * for reference.
   */
  grConstantColorValue(rgbacolor(state, 0x0000ffff));  // blue
  GrVertex line_verts[2];
  memset(line_verts, 0, sizeof(line_verts)); // zero them out
  ...
  line_verts[0].x = (float)state->conform_x + state->conform_width;
  line_verts[0].y = 0.0f;
  line_verts[0].a = 255.0f; 

  line_verts[1].x = (float)state->conform_x + state->conform_width;
  line_verts[1].y = (float)line_y_end;
  line_verts[1].a = 255.0f; 
  ...
  /*
   * Now, draw our prims with interpolated alpha.
   */
  ...
  for (int row = 0; row < local_ptr->rows; row++) {
    ...
    switch (row) { // one per primitive
      case PRIM_TRIANGLES:
        ...
        add_vertex_data(state, &xformed_verts[0], 3);
        break;
      case PRIM_LINES:
        ...
        break;
      case PRIM_VA_LINES:
      case PRIM_VA_LINE_STRIP: //break; // BROKEN? WALENDO
      ...
      case PRIM_VA_TRIANGLE_FAN: {
        ...
        add_command_data(state, "grDrawVertexArrayContiguous()", 4, 
                         mode, NUM_VA_VERTS, NULL, sizeof(GrVertex));
