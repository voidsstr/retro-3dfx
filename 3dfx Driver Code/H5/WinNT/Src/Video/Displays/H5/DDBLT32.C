/* $Header: ddblt32.c, 42, 10/11/00 8:57:01 PM, Brent$ */
/*
...
DdBlt( LPDDHAL_BLTDATA pbd )
{
...
#endif
    else if (2 == _DD(ddAANumberSamples))
      ...
      (dst_dwCaps & DDSCAPS_TEXTURE) && (dst_dwCaps & DDSCAPS_VIDEOMEMORY))
  {
    ...
    #endif
    pbd->ddRVal = TEXTURELOAD(ppdev, pbd->lpDDSrcSurface, &pbd->rSrc, pbd->lpDDDestSurface, &pbd->rDest);
                    ...
                    DWORD           dstPixelFormat)
{
  ...
  long            i, j, *scaneline, num_dwWrite;
  int             errorterm;
  int             twodeltaX;
  int             twodeltaY;
  char            *srcOffset, *src;
