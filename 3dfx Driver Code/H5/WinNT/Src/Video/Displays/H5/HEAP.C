...
PDEV*   ppdev)
{
    ...
    if (ppdev->heap.pohFreeList == NULL)
    {
        ...
        poha = ENGALLOCMEM(FL_ZERO_MEMORY, OH_ALLOC_SIZE, ALLOC_TAG, &ppdev->heap.pohaChain);
        if (poha == NULL)
            return(NULL);
...
#ifdef MEMCHECK
        if (NULL != ppdev->heap.pohaChain)
          UPDATE_BLOCK_DATA(ppdev->heap.pohaChain,&poha->pohaNext);
#endif
...
OH*     poh)
{
    ...
    DISPDBG((10, "Freeing %li x %li at (%li, %li)",
...
FLOH    floh)           // Allocation flags
{
...
#if defined( _MSC_VER ) && !defined( DBG )
#if _MSC_VER >= 1200
        // Eliminate issue with VC 6.0 optimizer

        __asm nop               // Force optimizer to sync
#endif
#endif
    ...
    DISPDBG((10, "   Allocated (%li x %li) at (%li, %li) with flags %li",
...
DSURF*  pdsurf)
{
    ...
    UPDATE_BLOCK_DATA(pdsurf, &poh->pdsurf);
...
ULONG   iFormat)
{
    ...
    // Don't allocate if glide is running.

    if ( glideState[ 0 ].glideGDIFlags & GLDATA_GDIFLAGS_HWC_EXCLUSIVE )
        return 0;
    ...
    if (poh != NULL)
    {
        pdsurf = ENGALLOCMEM(0, sizeof(DSURF), ALLOC_TAG, &poh->pdsurf);
        if (pdsurf != NULL)
        {
...
PDEV*   ppdev)
{
    ...
    while (poha != NULL)
    {
        ...
        if (NULL != pohaNext)
        {
          ppdev->heap.pohaChain = pohaNext;
          UPDATE_BLOCK_DATA(pohaNext, &ppdev->heap.pohaChain);
        }
        UPDATE_BLOCK_DATA(poha, 0);
#endif
        ENGFREEMEM(poha);
        poha = pohaNext;
    }
