...
**
** $Header: hwcext.c, 24, 10/11/00 8:56:47 PM, Brent$
** $Log: 
**  24   3dfx      1.19.2.1.1.110/11/00 Brent           Forced check in to enforce
**       branching.
**  23   3dfx      1.19.2.1.1.007/18/00 Adam Briggs     implemented reference count
**       for glide contexts
**  22   3dfx      1.19.2.1    06/07/00 Adam Briggs     merged hwcSliAARequest from
**       win2k
**  21   3dfx      1.19.2.0    05/04/00 Russ Lind       report correct numUnits to
**       glide
...
#endif
extern ULONG ulCmdFifoDisabled;
...
GLIDESTATE * hwcGetGlideStateStructureForProcess( HANDLE hProcess )
{
	...
	DISPDBG((GLIDE_DBG_LEVEL, "NO GLIDE State Structure Found!"));
...
GLIDESTATE * hwcAllocateGlideStateStructureForProcess( HANDLE hProcess )
{
	ULONG dwIndex;
       ...
       dwIndex++)
  {
	...
	for( dwIndex = 0x00; dwIndex < _MAX_GLIDE_PROCESSES; dwIndex++ )
	{
		...
		{	
			...
			memset( &glideState[ dwIndex ], 0x00, sizeof( GLIDESTATE ) );
#endif
	...
	DISPDBG(( GLIDE_DBG_LEVEL, "No More Glide Process State Structures!!!! ERROR ERROR" ));
...
VOID hwcReleaseGlideStateStructureForProcess( HANDLE hProcess )
{
	ULONG dwIndex;
	...
	DISPDBG((GLIDE_DBG_LEVEL, "No Glide State Structure Found For Current Process...ERROR ERROR ERROR" ));
...
hwcGetLinearAddr(hwcExtRequest_t *req, hwcExtResult_t *res, PDEV *ppdev)
{
  ...
  DISPDBG((GLIDE_DBG_LEVEL, "hwcGetLinearAddr"));
  DISPDBG((GLIDE_DBG_LEVEL, "devNum=%08lx", req->optData.linearAddrReq.devNum));
  DISPDBG((GLIDE_DBG_LEVEL, "handle=%08lx", req->optData.linearAddrReq.pHandle));

  // get the glide state structure for the process
  if( ( psGlideState = hwcGetGlideStateStructureForProcess( ( HANDLE )req->optData.linearAddrReq.pHandle ) ) == ( GLIDESTATE * )NULL )
  {
	DISPDBG((GLIDE_DBG_LEVEL, "No State Structure Found For Current Process...Allocating One!"));

	// allocate a GLIDE state structure for the current process
	if( ( psGlideState = hwcAllocateGlideStateStructureForProcess( (HANDLE)(req->optData.linearAddrReq.pHandle) ) )
		== NULL )
	{
		DISPDBG((GLIDE_DBG_LEVEL, "No State Structure Found For Current Process!"));
 		res->resStatus = 0;		
		return res->resStatus;
	}

    if (miscGlideMapMemoryBases(ppdev, (HANDLE)(req->optData.linearAddrReq.pHandle)))
    {
      res->optData.linearAddressRes.numBaseAddrs = 3;
      res->optData.linearAddressRes.baseAddresses[0] = (FxU32) psGlideState->glideRegBase;
      res->optData.linearAddressRes.baseAddresses[1] = (FxU32) psGlideState->glideScreenBase;
      res->optData.linearAddressRes.baseAddresses[2] = (FxU32) psGlideState->glideIOBase;
  ...
  {
      // If memory has already been mapped for this process, return the old mapping
      res->optData.linearAddressRes.numBaseAddrs = 3;
      res->optData.linearAddressRes.baseAddresses[0] = (FxU32) psGlideState->glideRegBase;
      res->optData.linearAddressRes.baseAddresses[1] = (FxU32) psGlideState->glideScreenBase;
      res->optData.linearAddressRes.baseAddresses[2] = (FxU32) psGlideState->glideIOBase;
...
// START Alt-Tab Changes
  ...
  if( ( psGlideState = hwcGetGlideStateStructureForProcess( ( HANDLE )req->optData.executeFifoReq.procId ) ) )
  {
    // check to see if the current glide process structure is undefined
    ...
    {	
      switch(req->optData.executeFifoReq.fifoType)
      {
        ...
        case HWCEXT_FIFO_FB:
        {
          ...
          ppdev->flStatus |= STAT_FIFO_IN_USE;
...
#ifdef ENABLE_V3_W2K_GLIDE_CHANGES
  pGS = hwcGetGlideStateStructureForProcess((HANDLE)(req->optData.releaseContextReq.procID)) ;

  /* only unmap the memory if we are really done with it */
  if ((pGS) &&
     (pGS->glideReferenceCount == 1))
    miscGlideUnmapMemoryBases(ppdev, (HANDLE)(req->optData.releaseContextReq.procID));

  // release the GLIDE state structure for the current process
  hwcReleaseGlideStateStructureForProcess( (HANDLE)(req->optData.releaseContextReq.procID) );
#else
  pGS = hwcGetGlideStateStructureForProcess((HANDLE)(req->optData.linearAddrReq.pHandle)) ;

  /* only unmap the memory if we are really done with it */
  if ((pGS) &&
     (pGS->glideReferenceCount == 1))
    miscGlideUnmapMemoryBases(ppdev, (HANDLE)(req->optData.linearAddrReq.pHandle));

  // release the GLIDE state structure for the current process
  hwcReleaseGlideStateStructureForProcess( (HANDLE)(req->optData.linearAddrReq.pHandle) );
#endif
...
hwcSetEsclusive(hwcExtRequest_t *req, hwcExtResult_t *res, PDEV *ppdev)
{
  ...
  DUMP_H3_REGS(ppdev, GLIDE_DBG_LEVEL);
...
hwcLinearMapOffset(hwcExtRequest_t* req, hwcExtResult_t*  res, PDEV *ppdev)
{
...
#if (_WIN32_WINNT >= 0x0500) && defined (AGP_CMDFIFO)
  {
	...
	if (!res->resStatus)
	{
		TDFX_LINEARADDR_INFO	Addresses;
		TDFX_LINEARADDR_INFO	tempData;
		ULONG					ReturnedDataLength;

		/* Copy the data because we aren't supposed to whack the
		 * request data from the client.
		 */
		tempData.ulAddress    = (PULONG) req->optData.mapInfoReq.remapAddr;
		tempData.ulLinearBase = NULL;

		DISPDBG((GLIDE_DBG_LEVEL, "ulAddress    = 0x%08lx", tempData.ulAddress));

		if (EngDeviceIoControl(ppdev->hDriver,
			IOCTL_3DFX_LINEARBASE_INFO,
			&tempData,  // input buffer
			sizeof(tempData),
			&Addresses,
			sizeof(Addresses),
			&ReturnedDataLength))
		{
			DISPDBG((GLIDE_DBG_LEVEL, "hwcLinearMapOffset - IOCTL_3DFX_LINEARBASE_INFO failed"));
			Addresses.ulLinearBase = NULL;
		}
		else
		{
			res->optData.mapInfoRes.linAddrOffset = (FxU32) req->optData.mapInfoReq.remapAddr
				                                  - (FxU32) Addresses.ulLinearBase;
			DISPDBG((GLIDE_DBG_LEVEL, "ulLinearBase = 0x%08lx", Addresses.ulLinearBase));
			DISPDBG((GLIDE_DBG_LEVEL, "linAddrOffset= 0x%08lx", res->optData.mapInfoRes.linAddrOffset));

			// check to see that the offset is within the 32Meg range, if not then the address
			// passed in must be invalid.. probably a system memory surface gone undetected.
			if( res->optData.mapInfoRes.linAddrOffset > 0x2000000 )
			{
				// zero out the offset to return an error
				res->optData.mapInfoRes.linAddrOffset = ( ULONG )NULL;
		...
		res->resStatus = (Addresses.ulLinearBase != NULL);
	}
...
hwcDownloadGamma(hwcExtRequest_t *req, hwcExtResult_t *res, PDEV *ppdev)
{
  ...
  memcpy(ppdev->TransientGammaTable, (PVOID)req->optData.gamma.gammaPtr, (req->optData.gamma.numGammaEntries * sizeof(ULONG)));
...
hwcUnMapMemory(hwcExtRequest_t *req, hwcExtResult_t *res, PDEV *ppdev)
{
  ...
  pGS = hwcGetGlideStateStructureForProcess((HANDLE)(req->optData.unmapMemoryReq.procHandle));

  if (pGS == NULL)
  {
	miscGlideUnmapMemoryBases(ppdev, (HANDLE)(req->optData.unmapMemoryReq.procHandle));

	// release the GLIDE state structure for the current process
	hwcReleaseGlideStateStructureForProcess( (HANDLE)(req->optData.unmapMemoryReq.procHandle) );

	for( ulIndex = 0x00; ulIndex < _MAX_CONTEXT_INFO; ulIndex++ )	
	{
		// check to make sure that the process handle doesn't already exist
		if( sContextInfo[ ulIndex ].ulProcessID == ( ULONG )req->optData.
			unmapMemoryReq.procHandle )
		{
			sContextInfo[ ulIndex ].ulProcessID = 0x00;
			sContextInfo[ ulIndex ].pulContextDWORD = NULL;
			sContextInfo[ ulIndex ].ppdev = NULL;
   ...
   else
  {
    ...
    // release the GLIDE state structure for the current process
    // (or at least decrement the reference count)
    hwcReleaseGlideStateStructureForProcess((HANDLE)(req->optData.unmapMemoryReq.procHandle));
  }
...
hwcContextDWORDNT(hwcExtRequest_t *req, hwcExtResult_t *res, PDEV *ppdev)
{
	...
	{
        // Call the miniport via an IOCTL to allocated the NON-cahced memory
        if (EngDeviceIoControl(ppdev->hDriver,
                               ...
                               &ReturnedDataLength))
        {
            DISPDBG((GLIDE_DBG_LEVEL, "ShareContextDWORD - Failed ALLOCATE_NONCACHED_MEMORY"));
			return 1;
        }
	}
	...
	for( ulIndex = 0x00; ulIndex < _MAX_CONTEXT_INFO; ulIndex++ )	
	{
		// check to make sure that the process handle doesn't already exist
		if( sContextInfo[ ulIndex ].ulProcessID == ( ULONG )req->optData.
			contextDwordNTReq.procId )
		{
			// return the index into the LFB that the DWORD exists at
			res->optData.contextDwordNTRes.dwordOffset = ( ULONG )
				sContextInfo[ ulIndex ].pulContextDWORD;
			return 1;
		}
	}
	...
	for( ulIndex = 0x00; ulIndex < _MAX_CONTEXT_INFO; ulIndex++ )	
	{
		...
		if( sContextInfo[ ulIndex ].pulContextDWORD == NULL )
		{
	        ...
	        if (EngDeviceIoControl(ppdev->hDriver,
							       ...
							       &ReturnedDataLength))
			{
				DISPDBG((GLIDE_DBG_LEVEL, "ShareContextDWORD - Failed IOCTL_MAP_MEMORY_TO_CURRENT_PROCESS"));
				return 1;
			}

			sContextInfo[ ulIndex ].pulContextDWORD = ( ULONG * )pulProcessFlag;
			sContextInfo[ ulIndex ].ulProcessID = ( ULONG )
				req->optData.contextDwordNTReq.procId;
...
Return:         LONG    1 is always returned.
----------------------------------------------------------------------*/
static LONG hwcPCIOP(hwcExtRequest_t *req, hwcExtResult_t *res, PDEV *ppdev)
{
#ifdef SLI_AA
  HWPCIOP HwPCIOp;
  DWORD   numBytes;
...
Return:         LONG    1 is always returned.
----------------------------------------------------------------------*/
static LONG hwcGetSlaveRegs(hwcExtRequest_t *req, hwcExtResult_t *res, PDEV *ppdev)
{
#ifdef SLI_AA
  int         i;
  int         unitNumber;
  ...
  // what's the process handle here????
  psGlideState = hwcGetGlideStateStructureForProcess((HANDLE)-1);
  if (NULL == psGlideState)
  {
    DISPDBG((0, "No State Structure Found For Current Process...ERROR ERROR ERROR"));

    // HACK
    // we'll attempt to see if req->optData.linearAddrReq.pHandle is still a valid processID
    // glide needs to give us some kind of indication what glide state struct we should use for this request!
    // As of the 02-14-00 daily build, it just so happens that the processID is still in the
    // linearAddrReq portion of req
    psGlideState = hwcGetGlideStateStructureForProcess((HANDLE)req->optData.linearAddrReq.pHandle);
    if (NULL == psGlideState)
    {
      DISPDBG((0, "No GlideState struct found for process id %lXh either!!!",
               req->optData.linearAddrReq.pHandle));
      return FALSE;
    }
    DISPDBG((GLIDE_DBG_LEVEL, "  using process id = %lXh", req->optData.linearAddrReq.pHandle));
  }
  ...
  for (i = 0; i < HWCEXT_MAX_SLAVE_REGS; i++)
    res->optData.slaveRegRes.Regs[i] =
              (ULONG)psGlideState->glideSlaveRegBase[(unitNumber * HWINFO_SST_MAX_CHIP_INDEX) + i];
