	    ...
	    if (((M0 | dM | N0 | dN) & (F - 1)) == 0)
            {
                ...
                // TGL - Use ppdev->pfnLineToTrivial

                vLineToTrivial(ppdev,
                              (M0 >> 4) + ppdev->xOffset,
                              (N0 >> 4) + ppdev->yOffset,
                              (dM >> 4) + ppdev->xOffset,
                              (dN >> 4) + ppdev->yOffset,
...
// H3 doesn't support sub-pixel hardware lines.
...
// Look after styling initialization:
    ...
    apfn = &gapfnStrip_H3[NUM_STRIP_DRAW_STYLES *
                            ((fl & FL_STYLE_MASK) >> FL_STYLE_SHIFT)];

//////////////////////////////////////////////////////////////////////
// Banshee specific initialization:
...
// Get the device ready:
    ...
    SET(3, pjH3Base, colorFore, pbo->iSolidColor);

    GWH_END_2D_PACKET( 3 );

    GWH_EPILOG;     // Save replication by putting it here.
