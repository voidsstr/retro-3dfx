/* $Header: ddovl32.c, 32, 10/11/00 8:57:10 PM, Brent$ */
/*
...
DWORD __stdcall SetOverlayPosition32( LPDDHAL_SETOVERLAYPOSITIONDATA psopd )
{
  ...
  wOvlScrEndY = min(rOvlDst.bottom, wDesktopH-1);
#else
  wOvlScrEndX = (WORD)min(rOvlDst.right, wDesktopW-1);
  wOvlScrEndY = (WORD)min(rOvlDst.bottom, wDesktopH-1);
#endif
...
//#endif
             ...
             (_FF(ddVisibleOverlaySurf) == overlayBaseAddr))
    {
     ...
     if(!IS_NAPALM)     //NAPALM seems don't have this problem
      ...
      wOvlScrEndY = min(rVidDst.bottom, wDesktopH-1);
#else
      wOvlScrEndX = (WORD)min(rVidDst.right, wDesktopW-1);
      wOvlScrEndY = (WORD)min(rVidDst.bottom, wDesktopH-1);
#endif
      GETOVERLAYADDR( wOvlScrEndX, wOvlScrEndY, vidOverlayEndAddr);
      ...
      vidOverlayDvdy = Get_ScalingFactor(dwSrcH, dwDstH, 20);
...
#if (USE_NT5_DDMEMMGR) || !defined(WINNT)
      WAIT_ON_VSYNC();      //WIN2K need this call too.
                            //for NT4.0  this call causes problem in PRS12967
#endif
                ...
                )
{
  ...
  DWORD  bWidth, tileFlag;
      ...
      if(_DD(overlaySurfaceCnt) <= 1)      //only create two shrink surfaces
      {
        FXSURFACEDATA* txtrSurfaceData;
            ...
            shrinkSurfaceData = (FXSURFACEDATA*) ENGALLOCMEM(FL_ZERO_MEMORY,sizeof(FXSURFACEDATA),'3HxD',&psurf_gbl->dwReserved1);
        #else
          ...
          {
    #ifdef MEMCHECK
            ENGFREEMEM(shrinkSurfaceData);
    #else
            DXFREE(shrinkSurfaceData);
    #endif
            return 0;
          }
          ...
          if(IS_NAPALM && (_DD(overlaySurfaceCnt) == 0)&& _DD(f32ShrinkOvl))
          {
  ...
  #ifdef MEMCHECK
                txtrSurfaceData = (FXSURFACEDATA*) ENGALLOCMEM(FL_ZERO_MEMORY,sizeof(FXSURFACEDATA),'3HxD',&psurf_gbl->dwReserved1);
  #else
                ...
                pcsd->ddRVal = memMgr_allocSurface(ppdev,
                 ...
                 {
    #ifdef MEMCHECK
                    ENGFREEMEM(txtrkSurfaceData);
    #else
                    DXFREE(txtrSurfaceData);
    #endif
