...
CreateBlockInfo ( BYTE *pbNew, ULONG sizeNew, const char *pszFileName, ULONG ulLineNo, DWORD dwData )
{
  ...
  if (NULL != pbi)
  {
    ...
    DISPDBG((0, "CreateBlockInfo: pb=%08lXh, pbi=%08lXh, dwData=%08lX [%s(%d)]",
             pbi->pb, pbi, pbi->dwData, pbi->szFileName, pbi->ulLineNo));
  }
...
FreeBlockInfo ( BYTE * pbToFree )
{
  ...
  DISPDBG((0, "FreeBlockInfo: pb=%08lXh, pbi=%08lXh, dwData=%08lX [%s(%d)]",
           pbi->pb, pbi, pbi->dwData, pbi->szFileName, pbi->ulLineNo));
...
CheckMemoryRefs ( void )
{
  ...
  for (pbi = pbiHead; pbi != NULL; pbi = pbi->pbiNext)
  {
    ...
    if (! pbi->fReferenced)
      DISPDBG((0, "Lost mem block!  allocated at %s(%d) pbi=0x%08lX", pbi->szFileName, pbi->ulLineNo,pbi));
...
NoteActiveRefs ( void )
{
  ...
  if (0 != i)
    DISPDBG((0, "NoteActiveRefs: %d blocks with dwData = 0", i));
}
