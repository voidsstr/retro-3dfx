...
#if defined ( GTF_TIMINGS )

//----------------------------------------------------------------------
// Function name:  ds_GetMonitorEdid
//
// Description:    Obtain the monitor EDID block.
//
// Information:    
//
...
//							 FALSE if failure
//----------------------------------------------------------------------
BOOLEAN ds_GetMonitorEdid( PHW_DEVICE_EXTENSION HwDeviceExtension, FxU8 bAddress, FxU8 *ddcData)
{
   PH3_MEMBASE0 sstIO = (PH3_MEMBASE0)HwDeviceExtension->MappedAddress[SST_IO_REGS_INDEX];
   I2CKEY key;
...
retry:
   ...
   retstat = 0;
   retstat |= !i2c_start(HwDeviceExtension, key);
   retstat |= !i2c_sendbyte(HwDeviceExtension, key, (FxU8)(bAddress | I2C_WRITESLAVE));
   retstat |= !i2c_sendbyte(HwDeviceExtension, key, 0);
   retstat |= !i2c_stop(HwDeviceExtension, key);
   retstat |= !i2c_start(HwDeviceExtension, key);
   retstat |= !i2c_sendbyte(HwDeviceExtension, key, (FxU8)(bAddress | I2C_READSLAVE));
   ...
   {
      i2c_readbyte(HwDeviceExtension, key, &ddcData[i], i != (nb - 1));
...
bailout:
   i2c_stop(HwDeviceExtension, key);
...
int i;
	...
	if ( !ds_GetMonitorEdid( HwDeviceExtension, 0xA2, EdidBlock ) )
	{
		if ( !ds_GetMonitorEdid( HwDeviceExtension, 0xA0, EdidBlock ) )
			return FALSE;
	}
