                       ...
                       PVIDEO_REQUEST_PACKET pRequestPacket)
{
  ...
  //
  // XXX XXX XXX
  // if the search order gets rearranged, the following lines with the notation
  // XXX XXX XXX must change
  //

  static const H3_SEARCH_TYPE H3SearchOrder[] =
  {
    { (PWCHAR)L"\\Registry\\User\\CurrentUser\\", (PWCHAR)L"\\Glide" },
    { (PWCHAR)L"\\Registry\\User\\CurrentUser\\", (PWCHAR)L"\\D3D"   },
    { (PWCHAR)L"\\Registry\\User\\CurrentUser\\", (PWCHAR)NULL       },
    { (PWCHAR)L"\\Registry\\Machine\\",           (PWCHAR)L"\\Glide" },
  ...
  static const WCHAR tdfxDevNodeKey[] = L"SYSTEM\\CurrentControlSet\\Services\\3Dfx\\Device0";
  ...
  else if ((0 == memcmp(pRequestPacket->InputBuffer, "FX",     2)) ||
           (0 == memcmp(pRequestPacket->InputBuffer, "Gl",     2)) ||
           (0 == memcmp(pRequestPacket->InputBuffer, "SSTH3_", 6)))
  {
    VideoDebugPrint((2, "using 3dfx node\n"));
    bICDCheck = FALSE;
    pswzKey   = (PWCHAR)tdfxDevNodeKey;
       ...
       pSearchLoc++)
  {
    ...
    RtlAppendUnicodeToString(&ucKeyName, pSearchLoc->pRoot);
...
H3ZwSetRegistryValue(PHW_DEVICE_EXTENSION hwDeviceExtension,
                     PVIDEO_REQUEST_PACKET pRequestPacket)
{
  static const WCHAR tdfxDevNodeKey[] = L"\\Registry\\Machine\\SYSTEM\\CurrentControlSet\\Services\\3Dfx\\Device0";
  ...
  if (! ((0 == memcmp(pRequestPacket->InputBuffer, "FX",     2)) ||
         (0 == memcmp(pRequestPacket->InputBuffer, "Gl",     2)) ||
         (0 == memcmp(pRequestPacket->InputBuffer, "SSTH3_", 6))))
  {
    return ERROR_DEV_NOT_EXIST;
                       ...
                       PVIDEO_REQUEST_PACKET pRequestPacket )
{
      ...
      (0 == memcmp(pRequestPacket->InputBuffer, "GlideGammaTable", 15)))
  {
    // fall thru to default driver location for "GlideGammaTable"
           ...
           (0 == memcmp(pRequestPacket->InputBuffer, "FX",     2)) ||
           (0 == memcmp(pRequestPacket->InputBuffer, "Gl",     2)) ||
           (0 == memcmp(pRequestPacket->InputBuffer, "SSTH3_", 6)))
  {
                   ...
                   PVIDEO_REQUEST_PACKET pRequestPacket)
{
      ...
      (0 == memcmp(pRequestPacket->InputBuffer, "GlideGammaTable", 15)))
  {
    // fall thru to default driver location for "GlideGammaTable"
           ...
           (0 == memcmp(pTDFXValueInfo->ValueName, "FX",     2)) ||
           (0 == memcmp(pTDFXValueInfo->ValueName, "Gl",     2)) ||
...
{
#if H4VEL
  static const WCHAR  FileName[] = L"\\DosDevices\\C:\\3dfxvsn.log";
#else
  static const WCHAR  FileName[] = L"\\DosDevices\\C:\\3dfxvsn.log";
#endif
  HANDLE              hFile;
...
int	i;
	...
	// Win2k appears to be broken here and does not copy new INF registry entries to a non zero \Device,
	// so the second card of the same type would reside at \Device1, but installation registry based
	// entries are not copied to this devnode. So then we will hardcode our stuff in \Device0, the
	// same method we use for NT 4.
