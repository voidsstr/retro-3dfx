                    ...
                    PUCHAR               pBios)
{
  ...
  HwDeviceExtension->pciInit0 &= ~SST_PCI_LOWTHRESH;
  if (1 == HwDeviceExtension->numUnits)
    HwDeviceExtension->pciInit0 |= (DEFAULT_PCI_LOWTHRESH_VAL << SST_PCI_LOWTHRESH_SHIFT);
  else
    HwDeviceExtension->pciInit0 |= (DEFAULT_2CHIP_PCI_LOWTHRESH_VAL << SST_PCI_LOWTHRESH_SHIFT);
...
H3MapAccessRanges(PHW_DEVICE_EXTENSION HwDeviceExtension)
{
  ...
  // map frame buffer
  VideoDebugPrint((1, "MemBase1 AccessRange\n"));
  VideoDebugPrint((1, "  RangeStart=%lX%08lXh  Length=%08lXh  InIoSpace=%lXh\n",
  ...
  VideoDebugPrint((1, "MemBase1 Mapped Range\n"));
#if (_WIN32_WINNT >= 0x0500)
  VideoDebugPrint((1, "  PhysAddr=%lX%08lXh -> MappedAddress=%08lX  Length=%08lXh\n",
                   HwDeviceExtension->AccessRanges[MEMBASE_ONE].RangeStart.HighPart,
                   HwDeviceExtension->AccessRanges[MEMBASE_ONE].RangeStart.LowPart,
  ...
  VideoDebugPrint((1, "IOBase AccessRange\n"));
  VideoDebugPrint((1, "  RangeStart=%lX%08lXh  Length=%08lXh  InIoSpace=%lXh\n",
  ...
  VideoDebugPrint((1, "IOBase Mapped Range -\n"));
  VideoDebugPrint((1, "  PhysAddr=%lX%08lXh -> MappedAddress=%08lX  Length=%08lXh\n",
                   HwDeviceExtension->AccessRanges[IOBASE_ZERO].RangeStart.HighPart,
                   HwDeviceExtension->AccessRanges[IOBASE_ZERO].RangeStart.LowPart,
                   HwDeviceExtension->MappedAddress[SST_IO_INDEX],
  ...
  VideoDebugPrint((1, "MemBase0 AccessRange\n"));
  VideoDebugPrint((1, "  RangeStart=%lX%08lXh  Length=%08lXh  InIoSpace=%lXh\n",
                   ...
                   HwDeviceExtension->AccessRanges[MEMBASE_ZERO].RangeInIoSpace));
  VideoDebugPrint((1, "MemBase0 Mapped Range\n"));
  VideoDebugPrint((1, "  PhysAddr=%lX%08lXh -> MappedAddress=%08lX  Length=%08lXh\n",
...

{
  ...
  if (IS_NAPALM)
  {
    // memory mapped regs = 64MB
    hwDeviceExtension->AccessRanges[0].RangeLength = 64 * 1024 * 1024;
    // frame buffer = 64MB
    hwDeviceExtension->AccessRanges[1].RangeLength = 64 * 1024 * 1024;
  }
#endif
                            ...
                            PUCHAR                pIO)
{
  ...
  // pciInit0
  RegisterMap->pciInit0 = HwDeviceExtension->pciInit0;
...
// START ALT-TAB CHANGES
	case IOCTL_ALLOCATE_NONCACHED_MEMORY:
		...
		VideoDebugPrint((1, "ALLOCATE_NONCACHED_MEMORY\n"));			
		{
			...
			pMemory = MmAllocateContiguousMemory( ( ULONG )*( ULONG *)RequestPacket->InputBuffer,
				sPhysicalAddress );			
...
// END ALT-TAB CHANGES
    ...
    case IOCTL_QUERY_MONITOR:
      VideoDebugPrint((1, "QUERY_MONITOR\n"));
        ...
        RequestPacket->StatusBlock->Information =
										 sizeof(TDFX_QUERY_MONITOR_INFO);
		if (RequestPacket->OutputBufferLength <
				  RequestPacket->StatusBlock->Information)
			{
				status = ERROR_INSUFFICIENT_BUFFER;
