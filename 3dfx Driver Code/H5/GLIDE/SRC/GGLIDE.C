...
**
** $Header: gglide.c, 24, 10/11/00 8:21:35 PM, Brent$
** $Log: 
**  24   3dfx      1.17.1.3.1.110/11/00 Brent           Forced check in to enforce
**       branching.
**  23   3dfx      1.17.1.3.1.007/24/00 troy thornton   fixed a problem when
**       clearing the front buffer in FSAA
**  22   3dfx      1.17.1.3    06/18/00 Stephane Huaulme fixed toggle and screen
**       shot key for the Macintosh
**  21   3dfx      1.17.1.2    05/25/00 Kenneth Dyke    Fixed bogus file time
**       stamps.
**  20   3dfx      1.17.1.1    05/24/00 Kenneth Dyke    Added MacOS support for AA
**       toggle & screenshots.
**  19   3dfx      1.17.1.0    05/09/00 Kenneth Dyke    Fix to make sure dither
**       rotation isn't trashed after grGlideSetState.
**  18   3dfx      1.17        04/13/00 Kenneth Dyke    Added support for new-style
**       two-sample AA mode.
**  17   3dfx      1.16        04/10/00 Kenneth Dyke    Added magical screenshot
**       hotkey.
**  16   3dfx      1.15        04/05/00 Adam Briggs     I can't believe the guy who
**       wrote the 2D buffer clear didn't implement code for the swapped y-origin
**       case!  Anyway, it's fixed now.
**  15   3dfx      1.14        04/03/00 Adam Briggs     Added a nop after a
**       bufferswap
**  14   3dfx      1.13        03/14/00 Adam Briggs     implemented cheesey
**       cliprect workaround for AA artifacts seen in Unreal.
**  13   3dfx      1.12        03/13/00 Adam Briggs     made grBufferClear stop
**       clobbering the dither rotation, shifted out depth values for 24bit w
**       buffering and removed A0 AA swap buffer workaround.
**  12   3dfx      1.11        03/06/00 Kenneth Dyke    Added AA toggle key hack.
**  11   3dfx      1.10        03/01/00 Kenneth Dyke    Disable dither rotation for
**       fastfills.
**  10   3dfx      1.9         02/28/00 Kenneth Dyke    4-sample AA dither rotation
**       matrix stuff.
**  9    3dfx      1.8         02/23/00 Kenneth Dyke    Fixed up and enabled wax
**       and triangle buffer clear code.
**  8    3dfx      1.7         02/23/00 Kenneth Dyke    Fix Y origin subtraction
**       for Glide2.x.
** 
**  7    3dfx      1.6         02/08/00 James Hunter    Descent 3 Alt+Tab fix -
**       this was somehow lost, possibly during the Napalm bringup stages.
**  6    3dfx      1.5         01/31/00 Adam Briggs     changed the IS_NAPALM macro
**       to cooperate with the display driver version of the same
**  5    3dfx      1.4         01/31/00 Adam Briggs     Changed all device ID magic
**       numbers to use those defined in fxhal.h & added IS_NAPALM macro to test
**       against device ID range
**  4    3dfx      1.3         01/28/00 Adam Briggs     glide3->glide2 merge from
**       napalm bringup
**  3    3dfx      1.2         12/20/99 James Hunter    Ported Descent 3 Alt+Tab
**       PRS #9156 fix from Voodoo 3 glide
**  2    3dfx      1.1         11/05/99 Anthony tai     added
**       FX_GLIDE_SWAPPENDINGCOUNT. Default=1. Range 0-3
**  1    3dfx      1.0         09/11/99 StarTeam VTS Administrator 
** $
** 
** 204   9/03/99 1:12p Atai
** fixed fifo pointer check
** 
** 203   7/18/99 11:12a Atai
** added 2 pixel per clock
** 
** 202   6/25/99 1:16p Atai
** fixed with 2 buffers per chip
** 
** 201   6/25/99 11:05a Atai
** fixed grBufferNumPending
** 
** 200   6/14/99 10:23a Stb_gkincade
** Improved code comments for image quality code in grDitherMode()
** 
** 199   5/13/99 2:45p Stb_gkincade
** Force dither matrix to 2x2 and Added user support for turning on/off
** dither substraction
** 
** 198   4/29/99 10:14a Atai
** sync grDitherMode with glide3x
** 
** 197   4/05/99 10:35p Atai
** added code to check context. exit if alt-tab happens
** 
** 196   3/31/99 9:02p Dow
** loosing context is fatal to glide2x
** 
** 195   3/26/99 12:01p Atai
** restore render mode for now
** 
...
** Made HW work after CSIM effage
** 
...
void
_grTriFill(GrColor_t color, FxU32 depth)
{
...
//
  ...
  //
  {
    ...
    bufferMode = fbzMode & SST_ENDEPTHBUFFER;
    //_grSwizzleColor(&color);
    ...
    {
      REG_GROUP_SET(hw, fbzMode, (fbzMode & ~(SST_WBUFFER | SST_ZFUNC | SST_ENCHROMAKEY | SST_ENZBIAS | SST_DEPTH_FLOAT_SEL)) | (GR_CMP_ALWAYS << SST_ZFUNC_SHIFT)) ;
    }
    REG_GROUP_END();
...
#define FN_NAME "grBufferClear"
  /* NEW MACRO - Fixes Alt+Tab program with Descent 3 (PRS #9156) */
  GR_BEGIN_NOFIFOCHECK_NO_CONTEXT_RETURN("grBufferClear", 86);
...

  {
    ...
    if ((fbzMode & SST_ZAWRMASK) != 0) {
      if ((fbzMode & SST_ENALPHABUFFER) != 0) {
        ...
        /* AJB W BUFFER HACK:
         * We are kinda screwed when w-buffering in 32bit mode
         * since glide2x has no mechanism for returning the
         * farthest w value to an app.
         * Fake it out here.
         */
        if ((fbzMode & SST_WBUFFER) &&
           (gc->grPixelSize == 4))
          dwDepth = depth << 8 | 0xff ;

        zacolor &= ~SST_ZACOLOR_DEPTH;
        zacolor |= (((FxU32) dwDepth) << SST_ZACOLOR_DEPTH_SHIFT);
      }
    }
    ...
    if (!doneP) {
      ...
      if (gc->sliCount > 1)      // This code can only possibly help in Sli
      {
            ...
            _GlideRoot.environment.waxon)
        {
          ...
          if (doColorP)
          {
            ...
            _grBufferClear2D(gc->buffers0[gc->curBuffer],
                             clipLeft, clipBottom, clipRight, clipTop, color, mask, FXTRUE, 
                             gc->strideInTiles | SST_BUFFER_MEMORY_TILED) ;

            // If there is an AA buffer, clear that too.
            if (gc->enableSecondaryBuffer)
              _grBufferClear2D(gc->buffers1[gc->curBuffer],
                               clipLeft, clipBottom, clipRight, clipTop, color, mask, FXTRUE, 
                               gc->strideInTiles | SST_BUFFER_MEMORY_TILED);
          }
          ...
          if (doAuxP)
          {
            ...
            _grBufferClear2D(gc->buffers0[gc->grColBuf],
                             clipLeft, clipBottom, clipRight, clipTop, dwDepth, mask, FXTRUE, 
                             gc->strideInTiles | SST_BUFFER_MEMORY_TILED) ;
...
#endif // end of hot wax session 
      {
        ...
        {
          /* Make sure dither rotation is disabled */
          GR_SET_EXPECTED_SIZE(sizeof(FxU32), 1);
          GR_SET(BROADCAST_ID, hw, renderMode, gc->state.fbi_config.renderMode & ~SST_RM_DITHER_ROTATION);
          GR_CHECK_SIZE();
        }
#endif
...
#define GETBLUE(a)  (a & 0x1f)
            ...
            if (gc->grPixelSize == 2)
            {
              ...
              red = GETRED(dwDepth) << 3;
              green = GETGREEN(dwDepth) << 2;
              blue = GETBLUE(dwDepth) << 3;
        ...
        if (IS_NAPALM(gc->bInfo->pciInfo.deviceID))
        {
          /* Make sure dither rotation is restored */
          GR_SET_EXPECTED_SIZE(sizeof(FxU32), 1);
...
#define FN_NAME "_grClipNormalizeAndGenerateRegValues"
  ...
  /* AJB AA garbage workaround */
  if (((gc->grPixelSample > 1) &&
     (_GlideRoot.windowsInit == FXTRUE)) &&
...
#define FN_NAME "grDepthBiasLevel"
   ...
   * Fake it out here.
   */
  if ((gc->state.fbi_config.fbzMode & SST_WBUFFER) &&
     (gc->grPixelSize == 4))
    dwLevel = level << 8 | 0xff ;

  zacolor = gc->state.fbi_config.zaColor;
  zacolor = (zacolor & ~SST_ZACOLOR_DEPTH) | (dwLevel & SST_ZACOLOR_DEPTH);
...
#define FN_NAME "grDitherMode"
   ...
   * Checking if user selected dither subtraction.
   *    Default Selection: Dither Subtraction Disabled
   ******************************************************************/   
...
switch (mode) {
  ...
  case GR_DITHER_4x4:     /* To ditherSub or not ditherSub,that is the question */
    fbzMode |= (SST_ENDITHER | SST_DITHER2x2 | SST_ENDITHERSUBTRACT);
...
#define FN_NAME "grFogMode"
  ...
  */
  ...
  /* Do the write in code below so it's reusable other places. */
  _grUpdateFogMode();
...
GR_DDFUNC(_grUpdateFogMode, void, (void))
{
#define FN_NAME "_grUpdateFogMode"
  FxU32 fogmode;
  GR_BEGIN_NOFIFOCHECK("grUpdateFogMode", 85);

  fogmode = gc->state.fbi_config.fogMode;
  
  if(gc->grPixelSample == 2) {
    FxU32 fogMode = fogmode;
...
#ifdef GLIDE_INIT_HWC
        if ((gc->bInfo->pciInfo.deviceID == SST_DEVICE_ID_H3) &&
            (gc->bInfo->devRev < 3))
...
#if ((GLIDE_PLATFORM & GLIDE_SST_SIM) && defined(GSIM))
    gsim_winclose();
#endif
...
#define FN_NAME "grGlideSetState"
  ...
  /* Need to re-write fogMode since otherwise the dither rotation stuff
   * will get whacked. */
  _grUpdateFogMode();
