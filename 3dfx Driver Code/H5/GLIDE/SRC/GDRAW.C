 ...
 **
 ** $Header: gdraw.c, 4, 10/11/00 8:21:32 PM, Brent$
 ** $Log: 
 **  4    3dfx      1.0.1.2     10/11/00 Brent           Forced check in to enforce
 **       branching.
 **  3    3dfx      1.0.1.1     08/14/00 Jonny Cochrane  Adjust glide2x grDrawPoint
 **       coords at sub pixel level so points look as good in none, 2x, 4x FSAA. PRS
 **       15201.
 ** 
 **  2    3dfx      1.0.1.0     07/10/00 troy thornton   changed grDrawPoints to
 **       draw points in FSAA
 **  1    3dfx      1.0         09/11/99 StarTeam VTS Administrator 
 ** $
** 
** 100   7/08/99 6:49p Kcd
** Use external grDrawTriangle code since CodeWarrior is lame.
** 
** 99    6/18/99 1:15p Kcd
** PowerPC PCI Bump & Grind stuff.
** 
** 98    6/03/99 6:23p Kcd
** Use slower (but more "compatible") version of SETF.
** 
** 97    5/07/99 1:24p Hanson
** Fix for failing to declare grDrawTriangle as naked.  Creates guarantee
** that the nothing is pushed onto the stack before jumping to the
** assembly function.
** 
...
#define FP_TRUNC_BIAS(__fpVal, __fpBias) \
                   ...
                   0x02, sizeof(FxU32) << 1, SSTCP_PKT3_BDDDDD);
  {
    ...
    /* Upper right corner was originally y -= (0x01UL << (21UL - kNumMantissaBits)).
 	   Now adjust coords at sub pixel level so points look as good in none, 2x, 4x FSAA	
	   Fixes PRS 15201 - Stars disappear at night with 4x FSAA 					
	*/
	y-=0x1e;
    ...
    /* Upper left corner was originally x -= (0x01UL << (21UL - kNumMantissaBits)).
 	   Now adjust coords at sub pixel level so points look as good in none, 2x, 4x FSAA	
	   Fixes PRS 15201 - Stars disappear at night with 4x FSAA 					
	*/
	x-=0x10;
 ...
 ** grDrawTriangle
 */
/* code does jump to assembly function */
/* naked declaration is necessary to guarantee there are */
/* no variables pushed onto the stack */
#if defined(GLIDE_USE_C_TRISETUP) || defined(__WATCOMC__) || defined(GLIDE_DEBUG) || defined(macintosh)
#define NAKED_FN
#else
#define NAKED_FN __declspec( naked )
#endif

#if defined(GLIDE_DEBUG) || !defined(__POWERPC__) || !defined(__MWERKS__)
#ifdef GLIDE3
GR_ENTRY(grDrawTriangle, void, (void *a, void *b, void *c))
#else
NAKED_FN GR_ENTRY(grDrawTriangle, void, (const GrVertex *a, const GrVertex *b, const GrVertex *c))
#endif
{
    ...
    while (count > 0) {
      ...
      for (k = 0; k < vcount; k++) {
         ...
         */
        {
          ...
          /* draw a square triangle strip the size of the pixel. 1x1*/
          
          /* The approach here is to split the strip into two packets, one
           * that sends just the coordinates of the last two points of the
           * triangle w/ no other parameter information, and then the
           * centered point w/ all of the parameter information. The first
           * packet is has a BDD command, but is incomplete, so nothing
          ...
          fy -= _GlideRoot.pool.f1;
          DA_SETF(fx);
          dataElem = 0;
          DA_SETF(fy);
          
          fx -= _GlideRoot.pool.f1;
          fy += _GlideRoot.pool.f1;
          DA_SETF(fx);
          DA_SETF(fy);
