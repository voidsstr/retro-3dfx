...
**
** $Header: gtex.c, 9, 10/11/00 8:21:55 PM, Brent$
** $Log: 
**  9    3dfx      1.7.1.0     10/11/00 Brent           Forced check in to enforce
**       branching.
**  8    3dfx      1.7         03/16/00 Kenneth Dyke    User settable LOD bias
**       offset.
**  7    3dfx      1.6         02/03/00 Kenneth Dyke    Fix signed/unsigned
**       comparison.
**  6    3dfx      1.5         02/03/00 Kenneth Dyke    Improved 2PPC support.
**  5    3dfx      1.4         02/03/00 Adam Briggs     support for forcing 2ppc
**       off
**  4    3dfx      1.3         01/31/00 Adam Briggs     changed the IS_NAPALM macro
**       to cooperate with the display driver version of the same
**  3    3dfx      1.2         01/31/00 Adam Briggs     Changed all device ID magic
...
** $
** 
** 53    7/30/99 3:54p Atai
** make csim happy
** 
** 52    7/27/99 7:14p Atai
** fixed 12 nopCMD size
** 
** 51    7/27/99 2:41p Atai
** added 12 nopCMD going from 2ppc to 1ppc
** 
** 50    7/18/99 11:12a Atai
...
void
_grTex2ppc(GrChipID_t tmu, FxBool enable)
{
#define FN_NAME "_grTex2ppc"
  ...
  */
  ...
  wasEnabled = (combineMode & SST_CM_ENABLE_TWO_PIXELS_PER_CLOCK) ? FXTRUE : FXFALSE;
  
  if((FxU32)enable == wasEnabled)
    return;
    
  /* enable = (gc->do2ppc ? enable : 0) ; */

  GDBG_INFO(gc->myLevel, FN_NAME ": changing 2ppc mode to: %s\n", enable ? "FXTRUE" : "FXFALSE");

  band = _GlideRoot.environment.band2ppc;
  tmu0cMode = gc->state.tmu_config[0].combineMode;
  tmu1cMode = gc->state.tmu_config[1].combineMode;
  chipField = (FifoChipField)(0x02UL << upstreamTmu);
  tmuHw = SST_TMU(hw, upstreamTmu);
  ...
  renderMode |= (((enable) ? band : 0) << SST_RM_TWO_PIXELS_PER_CLOCK_BAND_SELECTION_SHIFT);
  ...
  */
  ...
  REG_GROUP_END();
  /*
  ** set upstream tmu state the same as downstream
  */
  if (enable)
    upstreamTmu = tmu;
  ...
  {
    REG_GROUP_SET(tmuHw, textureMode, gc->state.tmu_config[upstreamTmu].textureMode);
    REG_GROUP_SET(tmuHw, tLOD, gc->state.tmu_config[upstreamTmu].tLOD);
    REG_GROUP_SET(tmuHw, tDetail, gc->state.tmu_config[upstreamTmu].tDetail);
    REG_GROUP_SET(tmuHw, texBaseAddr, gc->state.tmu_config[upstreamTmu].texBaseAddr);
    
    REG_GROUP_SET(tmuHw, texBaseAddr1, gc->state.tmu_config[upstreamTmu].texBaseAddr_1);
    REG_GROUP_SET(tmuHw, texBaseAddr2, gc->state.tmu_config[upstreamTmu].texBaseAddr_2);
    REG_GROUP_SET(tmuHw, texBaseAddr38, gc->state.tmu_config[upstreamTmu].texBaseAddr_3_8);
  }
  REG_GROUP_END();
...
#define FN_NAME "grTexClampMode"
  ...
  if((tmu > 0) && IS_NAPALM(gc->bInfo->pciInfo.deviceID) && gc->do2ppc)
    _grTex2ppc(tmu, FXFALSE);
...
#define FN_NAME "grTexCombine"
  ...
  /* Rampage will not support this in one vta pass so test for this now */
   ...
   */
  {
    ...
    if (upstreamTmu < gc->num_tmu) {
      if (localColorP && localAlphaP) {
        ...
        /* Make sure we disable 2PPC before we write any registers to
           an upstream TMU */
        if(IS_NAPALM(gc->bInfo->pciInfo.deviceID) && gc->do2ppc)
          _grTex2ppc(tmu, FXFALSE);
    }
...
#define FN_NAME "grTexNCCTable"
  ...
  GR_BEGIN_NOFIFOCHECK("grTexNCCTable",88);
