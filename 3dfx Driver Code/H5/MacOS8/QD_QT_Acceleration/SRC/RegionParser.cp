...
void RegionParser(NQDDrawVars *dv)
{
	...
	parserParams.blitFunction = (RegionBlitProcInternal) drawVars.refCon;

	h3Info *board = FindH3Info(drawVars.dstPixMap.baseAddr);

	dv->refCon = (long) board;
	
	GetArbitration(board);
...
/**********************************************************************************
	...
	Entry point for the region-parsing code. This version of the function is designed to
	be called from direct QuickDraw patches.
*/
...
{
	UInt32		numComplexRgns = 0;
	NQDRegion	**rgns[3];

	// Determine how many regions are complex
	if (rgn1)
	{
		if (!RPFastSectRect(minRect, rgn1[0][0].rgnBBox, minRect))
			return;
		if (rgn1[0][0].rgnSize > 10)
			rgns[numComplexRgns++] = (NQDRegion **) rgn1;	
	}
	if (rgn2)
	{
		if (!RPFastSectRect(minRect, rgn2[0][0].rgnBBox, minRect))
			return;
		if (rgn2[0][0].rgnSize > 10)
			rgns[numComplexRgns++] = (NQDRegion **) rgn2;	
	}
	if (rgn3)
	{
		if (!RPFastSectRect(minRect, rgn3[0][0].rgnBBox, minRect))
			return;
		if (rgn3[0][0].rgnSize > 10)
			rgns[numComplexRgns++] = (NQDRegion **) rgn3;	
	}
	
	// Call the approriate region function
	if (numComplexRgns == 0)
		parserParams.blitFunction(parserParams.blitDataPtr, minRect);
	else if (parserParams.parseLeftToRight)
	{
		if (numComplexRgns == 1)
			ParseOneRgnProc(parserParams, rgns[0], minRect);
		else if (numComplexRgns == 2)
			ParseTwoRgnProc(parserParams, rgns[0], rgns[1], minRect);
		else if (numComplexRgns == 3)
			ParseThreeRgnProc(parserParams, rgns[0], rgns[1], rgns[2], minRect);
	} else
	{
		if (numComplexRgns == 1)
			ParseOneRgnProcR2L(parserParams, rgns[0], minRect);
		else if (numComplexRgns == 2)
			ParseTwoRgnProcR2L(parserParams, rgns[0], rgns[1], minRect);
		else if (numComplexRgns == 3)
			ParseThreeRgnProcR2L(parserParams, rgns[0], rgns[1], rgns[2], minRect);
	}
}
