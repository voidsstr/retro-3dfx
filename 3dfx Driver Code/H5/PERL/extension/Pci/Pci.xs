...
void __stdcall pciPrintDeviceList(void);
...
unsigned long * __stdcall pciMapCardMultiFunc (unsigned long, unsigned long, unsigned long, unsigned long *, unsigned long, unsigned long);
...
int __stdcall pciFindCardMultiFunc(unsigned long, unsigned long, unsigned long *, unsigned long *, unsigned long);
#ifdef __cplusplus
}
#endif
...
unsigned long
mapCardMultiFunction(vendor, devid, size, base, function)
	...
	PROTOTYPE: $$$$$
	CODE:
	board = (unsigned long)pciMapCardMultiFunc(vendor, devid, size, &devnum, function, base); 
	RETVAL = board;
	sizeofcard = size;
	configmech = 1;
	OUTPUT:
...
mapCard(vendor, devid, size, base, card)
	...
	PROTOTYPE: $$$$$
	CODE:
	board = (unsigned long)pciMapCardMulti(vendor, devid, size, &devnum, card, base); 
	RETVAL = board;
	sizeofcard = size;
	configmech = 1;
	OUTPUT:
...
int
findMultiFunction(vendor, devid, card)
	unsigned long vendor
	unsigned long devid
	unsigned long card
	...
	flag = pciFindCardMultiFunc(vendor, devid, &devnum, &function, card);
