...
$vendor = $opt_v ? $opt_v : $DEFAULT_VENDOR;
$gclk = $opt_f ? $opt_f : $DEFAULT_FREQ;
$loop = $opt_l ? "-l" : "";

# Only set SSTH3_DRAMINIT0 if the user has specified the -d option. Otherwise,
# it gets set by the init code as a function of vendor name.

$ENV{SSTH3_DRAMINIT0} = $opt_d if $opt_d;

if ((@ARGV != 0) || ($opt_h)) {
    print "Usage: perl boardtest_qual.pl [-l] [-f <grx_freq>] [-d <hex_draminit0_value] [-v <sgram_vendor_name>]\n";
    print "  -h show this usage message\n";
    print "  -l will cause the script to loop forever\n";
    print "  If -d is not specified, dramInit0 will be set to a default\n";
    print "  value or to the value of env var SSTH3_DRAMINIT0 if it is set\n";
    die "  draminit0 must be specified as a hex value, e.g. 0xc17a9e9\n";
}
...
if ( $agp ) {
    ...
    if ( &Execute2("rundiags $loop -f $testlist","Command Stream '.*' FAILED") ) {
