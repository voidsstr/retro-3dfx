/* $Header: ddsurf.c, 83, 10/16/00 6:30:04 AM, Leo Galway$ */
/*
...
#if defined(SLI_AA) && !defined(WINNT)
extern  VOID GetProcessFileName(LPSTR lpBuf, DWORD dwSize);
#endif
...
DdCreateSurface( LPDDHAL_CREATESURFACEDATA pcsd )
{
  ...
  if ((_FF(ddExclusiveMode) == TRUE) &&
	  ...
	  (dwCaps & DDSCAPS_3DDEVICE)    &&
	  ((dwCaps & DDSCAPS_PRIMARYSURFACE) || (dwCaps & DDSCAPS_FLIP)))
  {

#if !defined(WINNT)
// Fix for PRS 15685 and 15694: Display corruption when running Re-Volt in 1600x1200x32 using Fastest Performace
// SLI/AA setting. Re-Volt creates a Z-Bufffer before creating the flip chain. In 1600x1200x32 the V5 will attemp to
// setup triple buffering, htough fail on account of memory space, when attempting to create a Third Buffer. 
// The Primary surface and Back Buffer are then destroyed and re-created (without requiring a third buffer). If
// Compute_SLIAA_Config has been initialized during the surface creation process before the surface creation runs out of memory,
// the hwPtr and lfbPtr are incorrect for the primary surface and lead to corrution of the game screen (and also corruption of the 
// desktop, if the game can be exited).
// This fix (hack alert!!!) checks the current process name. If the process is not Re-Volt or is Re-Volt using a primary surface
// width, height and bpp of less than 1600x1200x32 respectively, or the process is Re-Volt and the number of current surfaces is 2,
// then the code should flow through the usual code path at this stage of the driver. Otherwise the Config_SLIAA_Config should be
// ignored. lgalway@3dfx.com
 
	char  szBuffer[MAX_PATH];
	unsigned char szProcess[MAX_PATH] = {0xad, 0xba, 0xa9, 0xb0, 0xb3, 0xab, 0xd1, 0xba, 0xa7, 0xba, 0x00};

  	GetProcessFileName(szBuffer, sizeof(szBuffer));

	if ((strcmp(szBuffer, szProcess) != 0) || 
	   ((strcmp(szBuffer, szProcess) == 0) && ((_FF(hres)<0x640) || (_FF(vres)<0x4B0) || (_FF(bpp)<0x20))) ||
	   ((strcmp(szBuffer, szProcess) == 0) && ((int)pcsd->dwSCnt == 2)))
	{
...
#endif
#ifdef STEREO
	  		if( _FF(ddStereoHeapFactor) && (dwCaps & DDSCAPS_COMPLEX) && (dwCaps & DDSCAPS_FLIP)  )	
//	  		if( _FF(ddStereoHeapFactor) && (pcsd->dwSCnt > 1) )	
      			Compute_SLIAA_Config(ppdev, 2); 
	  		else
		  		Compute_SLIAA_Config(ppdev, pcsd->dwSCnt);	 
#else
   	 		Compute_SLIAA_Config(ppdev, pcsd->dwSCnt); 	 
#endif
    	}
  ...
  for( i=0;i<(int)pcsd->dwSCnt;i++ )
  {
...
#if defined(WINNT) && defined(MEMCHECK)
      surfaceData = (FXSURFACEDATA*) ENGALLOCMEM(FL_ZERO_MEMORY,sizeof(FXSURFACEDATA),'3HxD',&psurf_gbl->dwReserved1);
#else
    ...
    else
    {
      ...
      // We've found a few cases where the primary surface is recreated
      // in a new mode, but the width and height in the lpGbl pointer
      // has not changed to the new display mode.  So we update
      // the global structure ourselves.  Fixes PRS 15242
      if(_FF(hres) != psurf_gbl->wWidth)
      {
        Msg(ppdev, 0, "Updating global surface width for primary surface");
        psurf_gbl->wWidth = _FF(hres);
      }
      if(_FF(vres) != psurf_gbl->wHeight)
      {
        Msg(ppdev, 0, "Updating global surface height for primary surface");
        psurf_gbl->wHeight = _FF(vres);
      }
    }
...
CleanUp:
          ...
          (FOURCC_YV12 == psurf_gbl->ddpfSurface.dwFourCC))
      {
        ...
        (LPVOID)psurf_gbl->fpVidMem = EngAllocUserMem((3 * psurf_gbl->wHeight * psurf_gbl->wWidth) / 2, '1VYD');
        ...
        (dwCaps & DDSCAPS_ZBUFFER)))
  {
    ...
    if (_DD(ddAAModeRequested))
    {
...
#ifdef WINNT
        if ((PARTOFPRIMARYCHAIN(psurf->ddsCaps.dwCaps)) || (dwCaps & DDSCAPS_ZBUFFER))
#else
      ...
      if (_DD(ddSLIModeRequested))
      {
        ...
        {
          case 3:  _DD(ddAAPrimaryStart) = AdjustHwPtr(ppdev, _DS(secondaryThirdBuffer));
                   break;
          case 2:  _DD(ddAAPrimaryStart) = AdjustHwPtr(ppdev, _DS(secondaryZBuffer)) - SST_TILE_SIZE;
                   break;
          case 1:  _DD(ddAAPrimaryStart) = AdjustHwPtr(ppdev, _DS(secondaryBackBuffer));
                   break;
          case 0:  _DD(ddAAPrimaryStart) = AdjustHwPtr(ppdev, _DS(secondaryFrontBuffer));
                   break;
        }
        _DD(ddAAZbufferStart) = AdjustHwPtr(ppdev, _DS(secondaryZBuffer));
      }
      else
      {
...
// heap, there's really no point in using them
  ...
  if (NULL != surfaceData)
  {
    ...
    if ((DDSCAPS_OVERLAY & psurf->ddsCaps.dwCaps) &&
        ...
        (FOURCC_YV12 == psurf_gbl->ddpfSurface.dwFourCC))
    {
      EngFreeUserMem((LPVOID)psurf_gbl->fpVidMem);
      psurf_gbl->fpVidMem = surfaceData->lfbPtr;
    }
#endif
...
DdAddAttachedSurface( LPDDHAL_ADDATTACHEDSURFACEDATA pasd )
{
  ...
  if ((IS_NAPALM) && (dwCap2 & DDSCAPS_ZBUFFER))
  {
    ...
    // 16bpp render target, 32bpp zbuffer: do nothing, error will result
    ...
    // this seems like a very bad thing to do if we're in a 16bpp mode!
    // 16bpp zbuffers aren't over allocated in 16bpp modes so we're going to stomp
    // on the wrong video memory doing this promotion in 16bpp modes
#ifdef WINNT
    else if ((4 == GETPRIMARYBYTEDEPTH) && (pixelByteDepth == 4) && (zbufferDepth != 4))
#else
...
HRESULT FXGETBUSYSTATUS(NT9XDEVICEDATA *ppdev)
{
  ...
  if (_FF(doAgpCF) && CMDFIFOUNBUMPEDWORDS)
  {
    ...
    while (GET(ghwIO->status) & SST_PCIFIFO_BUSY)
      ;
