           ...
           DWORD            dstPixelFormat)
{
  ...
  if((_D3(lastContext)!= 0) && (_D3(lastContext)!= 0xADE))
  {
           ...
           LPDDHAL_BLTDATA  pbd)
{
  ...
  // Create memory for new surfaceData structure
  ...
  pbd->ddRVal = memMgr_allocSurface(
      ...
      (DWORD) height,                  // [IN] height in scanlines
