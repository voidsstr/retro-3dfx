/* $Header: ddflip.c, 34, 10/11/00 8:52:24 PM, Brent$ */
/*
...
#ifdef STEREO
#define STEREO_FLIP 2
#define MONO_FLIP   1
VOID StereoSLISync(BOOL, NT9XDEVICEDATA *);
#endif
...
void Convert32BitToTGAFormat(FxU32 width, FxU32 height , DWORD src, FxU32 strideInBytes , FxU8 * dst);
void Convert16BitToTGAFormat(FxU32 width, FxU32 height , DWORD src, FxU32 strideInBytes , FxU8 * dst);
#endif
#endif
DWORD __stdcall
...
// can't use DD_ENTRY_SETUP because CMDFIFO need pdev
...
#ifdef WINNT
   /* Since we never build NT anymore in this code (H5 Win9x) this is not required
   {
   int counter ;
  ...
  if ((NULL != HotKey_Data[ScreenShot_HOTKEY].pHotKeyData) && (1 == *HotKey_Data[ScreenShot_HOTKEY].pHotKeyData))
  {
  FxU8 *buffer;					// Pointer to our destination buffer (3 times bigger than the screen rez)
  FxU8  header[18];				// TGA Header, Version 1.0
  HANDLE  hTraceFile = INVALID_HANDLE_VALUE;	// File Handle, defaulted to invalid.
  DWORD	dwNumBytesWritten ;		// How much bytes actually written, we don`t worry about this..
  
  /* Allocate buffer to store frame into.. X times Y times 3 bytes per pixel */
  buffer = DXMALLOC(_FF(hres) * _FF(vres) * 3);
  if(buffer)
	{

    /* Depending on the back-buffers mode we Read the back buffer into our own buffer */
	if (_FF(bpp) == 16)
	   Convert16BitToTGAFormat(_FF(hres), _FF(vres), _DD(ddSurfaceFlippedTo), pfd->lpSurfTarg->lpGbl->lPitch, buffer) ; 
	  else
	   Convert32BitToTGAFormat(_FF(hres), _FF(vres), _DD(ddSurfaceFlippedTo), pfd->lpSurfTarg->lpGbl->lPitch, buffer) ;

	/* Generate TGA Header */
	memset (header, 0, 18);
	header[2] = 2;    /* Uncompressed Coloured Targa */
	header[12] = (FxU8)(_FF(hres) & 255);
	header[13] = (FxU8)(_FF(hres) >> 8);
	header[14] = (FxU8)(_FF(vres) & 255);
	header[15] = (FxU8)(_FF(vres) >> 8);
	header[16] = 24 ;  /* 24 bytes per pixel*/

	/* Create the output file */
	hTraceFile = CreateFile("c:\\output.tga",
                           GENERIC_WRITE,
                           0,
                           NULL,
                           CREATE_ALWAYS,
...
#if defined(CMDFIFO)
  {
	...
	if (_D3(useMaxPendingBuffers))
	{
		swapsQueued = _D3(maxPendingBuffers);
	}
	else
	{
		if (waitOnVsync)
		...
		else
		{
		   ...
		   swapsQueued = MAXPENDINGBUFFERS_TOAVOIDHANG;  // allow 7 swap buffers
		}
	}
#else
    if (waitOnVsync)
...
// this seems to break the windows simulator.  readd when it does not
  ...
  if (_DD(ddAAModeEnabled))
  {
   ...
   _FF(FlipCount)++;
   if (_DD(ddAAModeEnabled) || _DD(ddSLIModeEnabled) || (_DD(ddAANumberSamples) && (_FF(dwNumUnits) > 1)))
   {
...
#endif
#endif
#ifdef STEREO //jcochrane - stereo surface flipping
  	if( pfd->dwFlags & DDFLIP_STEREO )
	{


  			swapToAddrR = GET_HW_ADDR(pfd->lpSurfTargLeft) & ~SSTG_IS_TILED;
  			...
  			SETPH(hwPtr, CMDFIFO_BUILD_PK1(1, 0, rightOverlayBuf, 0xF ));
   			SETPD(hwPtr, ghw0->rightOverlayBuf, swapToAddrR);
			...
			StereoSLISync(STEREO_FLIP, ppdev); //handle stereo on/off toggle and SLI sync

	}
	else
	{

		StereoSLISync(MONO_FLIP, ppdev); //does nothing unless stereo software wrapper loaded
...
/*----------------------------------------------------------------------
Function name:  Convert16BitToTGAFormat

Description:    Converters a 16 Bit frame buffer to a 24-Bit TGA format
                compatible buffer.

                16 Bit buffer looks to be in format RGB 1555 (2 bytes)
				which is converted to BGR (3 bytes) for TGA.
...
void Convert16BitToTGAFormat(FxU32 width, FxU32 height , DWORD src, FxU32 strideInBytes , FxU8 * dst)
{
  ...
  /* Set-up some locals, Divide by 2 because at 16 bpps we use >2< bytes per pixel */
  strideInBytes = (strideInBytes / 2) - width ;
  s = (FxU16 *)src ;
  ...
  {    
	...
	/* Convert the 4 bytes to 3 bytes and store */
	for (x = 0 ; x < width; x ++) 
	{
		/* Read the 2 byte from memory, convert from ABRG (2 byte) to BGR (3 byte) format */
	    pix32 = *s++ ;
		
        r = (FxU8)((pix32 >> 11) & 0x1f);
        g = (FxU8)((pix32 >> 6) & 0x1f);
        b = (FxU8)(pix32 & 0x1f);

        *d++ = b * 8 ;	// * TODO : x8? is there a better way to do this? limits max values to 248, not 255?!?
        *d++ = g * 8 ;
        *d++ = r * 8 ;
	}
	/* Move our source point past the right edge, onto the next actual byte of bitmap*/
	s = s + strideInBytes ;
  } 
}

/*----------------------------------------------------------------------
Function name:  Convert32BitToTGAFormat

Description:    Converters a 32 Bit frame buffer to a 24-Bit TGA format
                compatible buffer.

                Based on an original routine in minihwc.c, modified to
				cater for D3D's world.                        
...
void Convert32BitToTGAFormat(FxU32 width, FxU32 height , DWORD src, FxU32 strideInBytes , FxU8 * dst)
{
  ...
  /* Set-up some locals */
  strideInBytes = (strideInBytes / 4) - width ;
  s = (FxU32 *)src ;
  ...
  {    
	...
	for (x = 0 ; x < width; x ++) 
	{
		/* Read the 4 byte from memory, convert from ARGB (4 byte) to BGR (3 byte) format */
	    pix32 = *s++ ;
	...
	/* Move our source point past the right edge, onto the next actual byte */
	s = s + strideInBytes ;
  }	
}
...
#ifdef STEREO
/*----------------------------------------------------------------------
Function name:  StereoSLISync

Description:    Stereo Mode Toggle and SLI sync

Return:        	void
----------------------------------------------------------------------*/
VOID StereoSLISync(BOOL mode, NT9XDEVICEDATA * ppdev)
{

	static DWORD bStereoFlipEnabled;
	...
	switch(mode)
	{
		case MONO_FLIP:
		{
			if(bStereoFlipEnabled)
       		{
				bStereoFlipEnabled = FALSE;
				...
				{
					//5 flip delay before reeanbling the slave (5 just seemed good at the time, odd is definately better than even)
   					retry = 5; 
					//get vidproccfg on the slave
   					vidProcCfg = ((SstIORegs *)ppdev->regBase[HWINFO_SST_MAX_NUM_CHIPS + HWINFO_SST_IOREGS_INDEX])->vidProcCfg;
					vidProcCfg &= ~SST_OVERLAY_STEREO_EN;
					vidProcCfg &= ~SST_VIDEO_PROCESSOR_EN;
					//temporarily disable it
					((SstIORegs *)ppdev->regBase[HWINFO_SST_MAX_NUM_CHIPS + HWINFO_SST_IOREGS_INDEX])->vidProcCfg=vidProcCfg;
           	
				}
		 	}
			//allow stereo sync'ing of left and right overlay buffers in SLI mode
			if(retry) 
			{	
 				retry--;
 				if(retry == 0)
 				{
 					vidProcCfg = ((SstIORegs *)ppdev->regBase[HWINFO_SST_MAX_NUM_CHIPS + HWINFO_SST_IOREGS_INDEX])->vidProcCfg;
 					vidProcCfg |= SST_VIDEO_PROCESSOR_EN;
					//reenable the slave after a few flips
					((SstIORegs *)ppdev->regBase[HWINFO_SST_MAX_NUM_CHIPS + HWINFO_SST_IOREGS_INDEX])->vidProcCfg=vidProcCfg;
				}
			}
		...
		case STEREO_FLIP:
		{
            if( !bStereoFlipEnabled )
            {
                bStereoFlipEnabled = TRUE;
            
				vidProcCfg = GET(ghwIO->vidProcCfg);
				vidProcCfg |= SST_OVERLAY_STEREO_EN;
				SETDW(ghwIO->vidProcCfg, vidProcCfg);

				//SLI stereo synching currently only works for 2 units
				if( _DD(ddSLIModeEnabled)  ) 
				{
   					vidProcCfg = ((SstIORegs *)ppdev->regBase[HWINFO_SST_MAX_NUM_CHIPS + HWINFO_SST_IOREGS_INDEX])->vidProcCfg;
					vidProcCfg |= SST_VIDEO_PROCESSOR_EN;
