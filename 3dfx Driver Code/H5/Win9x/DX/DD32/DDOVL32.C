/* $Header: ddovl32.c, 35, 10/11/00 8:52:32 PM, Brent$ */
/*
...
//#endif
             ...
             (_FF(ddVisibleOverlaySurf) == overlayBaseAddr))
    {
     ...
     if(!IS_NAPALM)     //NAPALM seems don't have this problem
	  ...
	  // Fix for PRS 12592, we dec the overlay height by 1
	  // rVidDst.bottom-1;  jjmccartney 09/06/00
      //Overlay screen bottom, right
#ifndef WINNT
	  ...
	  // are not the same height as software ones or on competitors cards
...
DWORD ShrinkOverlaySurface(NT9XDEVICEDATA * ppdev, LPDDRAWI_DDRAWSURFACE_LCL psurf)
{
  ...
  else
  {
     ...
     dstBottom = srcSurfaceData->shrinkHeight;	   // fix for PRS 12592; removed +1 addition to height jjmccartney 09/06/00
     // SHRINK
