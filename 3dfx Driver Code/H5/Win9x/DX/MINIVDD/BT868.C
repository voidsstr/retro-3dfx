/* -*-c++-*- */
/* $Header: bt868.c, 24, 10/16/00 3:47:08 PM, Geoff Bullard$ */
/*
...
**
** $Revision: 24$
** $Date: 10/16/00 3:47:08 PM$
**
** $Log: 
**  24   3dfx      1.19.1.1.1.110/16/00 Geoff Bullard   Implement
**       SET_ALTERNATE_DISPLAYS escape thru FunctionAPI.  This is a generic
**       interface to turn on/off TVOUT/DFP/CRT.
**  23   3dfx      1.19.1.1.1.010/11/00 Brent           Forced check in to enforce
**       branching.
**  22   3dfx      1.19.1.1    06/02/00 Dan O'Connel    Sync changes between Win9x
**       and Win2K versions.
**  21   3dfx      1.19.1.0    05/18/00 Dan O'Connel    Major clean up of DFP
**       support code.  Removes obsolete XLCD support.  Restructures code to
**       simplify interfaces.
**  20   3dfx      1.19        02/17/00 Dan O'Connel    Change Bt868 timings to
...
**  19   3dfx      1.18        02/07/00 Dan O'Connel    Fix boot to PAL TV in
**       WinNT4/Win2K and cleanup code.
**  18   3dfx      1.17        02/07/00 Dan O'Connel    Remove a lot of O/S
**       specific ifdefs from these files by changing the way IS_VOODOO3 and
**       IS_NAPALM macros are used.
**  17   3dfx      1.16        02/02/00 Dan O'Connel    Add code to access Bios
**       "Board Config" information to check for TvOut and DFP support.  Also sync
**       up with changes to WinNT/Win2K.
**  16   3dfx      1.15        01/28/00 Dan O'Connel    Add comments and sync up
**       with WinNt4/Win2k versions.
**  15   3dfx      1.14        01/27/00 Dan O'Connel    Changes necessary to
**       initially bring up TvOut on Napalm.  The deletions of the unnecessary
**       calls to tvStdToNVRAM fixes an unrelated problem the having to do with
**       correctly starting up in PAL mode.
**  14   3dfx      1.13        01/10/00 Dan O'Connel    Remove remnants of Banshee
**       code.
**  13   3dfx      1.12        01/05/00 Dan O'Connel    Fix DCT300 MacroVision test
**       to pass when card is not TvOut capable on Win2K.
**  12   3dfx      1.11        01/05/00 Dan O'Connel    Correct Win9x
...
**  11   3dfx      1.10        12/29/99 Dan O'Connel    Change to use macros which
...
**  10   3dfx      1.9         12/28/99 Dan O'Connel    Mode is derived using TV
**       Standard instead of being saved in a variable.  This is the way the WinNT
**       driver has worked for some time.
**  9    3dfx      1.8         12/28/99 Dan O'Connel    TV Standard was not getting
**       initialized from the value stored in NVRAM in some cases, so this modifies
**       the initialization method.
**  8    3dfx      1.7         12/21/99 Dan O'Connel    Cleanup Win9x code that
**       interface with BIOS via shared registers and NVRAM.  Also update code that
**       interfaces with BIOS to work with Napalm BIOS.
**  7    3dfx      1.6         12/13/99 Dan O'Connel    The wrong value was being
**       passed to BT868_CopyProtect resulting in MacroVision never being turned
**       off once it got turned on for WinNT and Win2K.
**  6    3dfx      1.5         12/07/99 Dan O'Connel    This is the initial cut at
**       producing a bt868.c module that is common across Win9x/WinNT/Win2K.  This
**       code is mainly based on the WinNT/Win2k code.
**  5    3dfx      1.4         12/06/99 Dan O'Connel    Extensive but minor name
...
**  4    3dfx      1.3         12/03/99 Dan O'Connel    Delete obsolete I2c code
**       that was previously commented out.
**  3    3dfx      1.2         09/22/99 Ryan Bissell    I2C-related updates, and a
**       bug fix
**  2    3dfx      1.1         09/21/99 Steve Rogers    Porting my fix from V3 OEM
**       tree: Fixed Compaq 369 board problem enumerating TV Out functionality.
...
static const GUID vpguid = { 0x2c62061, 0x1097, 0x11d1, { 0x92, 0xf, 0x0, 0xa0, 0x24, 0xdf, 0x15, 0x6e } };
...
int BT868_ProcessVideoParameters (BT868CONTEXT pContext, PVIDEO_REQUEST_PACKET RequestPacket)
{
    ...
    if ((!pContext->tvOutCapable) && (videoParametersIn.dwCommand != VP_COMMAND_GET))
        return (-1);  // don't handle set commands if no tvOut
    ...
    if (videoParametersIn.dwCommand == VP_COMMAND_GET)
    {
        ...
        {   // no tvout adapter, apparently
            return (0);  // handled with all results set to zero
        }
...
#ifdef WINNT
// The tvOutCapable flag has no analog in W9x currently but the function should
// work if we use the method for checking if the BT86x is there directly
    if (pContext->tvOutCapable)
#else
    if (BT868_GetStatus (pContext, 0) > 0)
#endif
