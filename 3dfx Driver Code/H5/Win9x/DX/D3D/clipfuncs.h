...
**
** $Revision: 4$
** $Date: 10/11/00 8:49:26 PM$
**
** $Log: 
**  4    3dfx      1.0.1.2     10/11/00 Brent           Forced check in to enforce
**       branching.
**  3    3dfx      1.0.1.1     09/23/00 Allen Hansen    general code cleanup -
**       deleted unused variables and long-unused functions
**  2    3dfx      1.0.1.0     07/24/00 Allen Hansen    whql bug fix
** 
...
DWORD __CLIP_NAME(RC *pRc, TLCLIPVTX **inv, TLCLIPVTX **outv, DWORD count)
{
    ...
    prev_inside = __CLIP_SIGN(prev->hw * pRc->tl.ViewData.__CLIP_GBCOEF - prev->__CLIP_COORD);
#else
#ifdef __CLIP_W
    ...
    {
#ifdef __CLIP_GUARDBAND
        curr_inside = __CLIP_SIGN(curr->hw * pRc->tl.ViewData.__CLIP_GBCOEF - curr->__CLIP_COORD);
#else
#ifdef __CLIP_W
            ...
            { // second point is inside
              // Find intersection and insert in into the output buffer
                ...
                Interpolate(pRc, outv[out_count],
                            curr, prev, 
                            (prev->clip_code & CLIPPED_ENABLE) | __CLIP_FLAG,
