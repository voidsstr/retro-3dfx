...
**
** $Revision: 112$
** $Date: 10/26/00 7:57:05 AM$
**
** $Log: 
**  112  3dfx      1.86.2.3.1.2010/26/00 Johnny Trainor  Changes due to modifation
**       of StateBlock structure.
**  111  3dfx      1.86.2.3.1.1910/23/00 Johnny Trainor  Removed the global
**       variable vertexType. We now use the dwVertexType element of the RC
**       structure.
**  110  3dfx      1.86.2.3.1.1810/20/00 Jonny Cochrane  Use C code for w scaling
**       instead of afan.asm
**  109  3dfx      1.86.2.3.1.1710/20/00 Johnny Trainor  Removed the clearing of
**       zEnable and zWriteEnable when there is no zBuffer. Added the case where if
**       a zBuffer is introduced the hardware states are set up correctly. Fixes
**       zBuffer problem with the DX8 water sample.
** 
**  108  3dfx      1.86.2.3.1.1610/19/00 Jonny Cochrane  fix chipmask for subsample
**       LOD dithering
**  107  3dfx      1.86.2.3.1.1510/18/00 Allen Hansen    Optimized when T&L
...
**       the dirty params.
**  106  3dfx      1.86.2.3.1.1410/12/00 Johnny Trainor  Added support for more DX8
**       DrawPrimitive2 tokens.
**  105  3dfx      1.86.2.3.1.1310/11/00 Brent           Forced check in to enforce
**       branching.
**  104  3dfx      1.86.2.3.1.1210/10/00 Johnny Trainor  Added support for the DX8
**       DP2 tokens D3DDP2OP_SETSTREAMSOURCE, D3DDP2OP_SETSTREAMSOURCEUM and
**       D3DDP2OP_SETINDICES.
**  103  3dfx      1.86.2.3.1.1109/29/00 James Hunter    D3D Screen Capture code
**       (first revision) and code to allow multiple hot key detection.
**  102  3dfx      1.86.2.3.1.1009/29/00 Johnny Trainor  Modified clear functions,
**       FastFill and FastFill_2pass, so they take fewer parameters. They now take
**       a pointer to a D3DHAL_DP2CLEAR structure rather than the individual
**       elements. 
**  101  3dfx      1.86.2.3.1.909/26/00 Johnny Trainor  #ifdefed some Dx7 specific
**       code. The file now compiles under Dx8.
**  100  3dfx      1.86.2.3.1.809/03/00 Allen Hansen    fixed bug setting up
**       D3DTSS_ADDRESS, D3DTSS_ADDRESSU and D3DTSS_ADDRESSV were being loaded but
**       not D3DTSS_ADDRESS, then for the D3DTSS_TEXTURETRANSFORMFLAGS case we were
**       loading the value into D3DTSS_ADDRESSU and D3DTSS_ADDRESSV, which was
**       wrong
**  99   3dfx      1.86.2.3.1.708/21/00 Allen Hansen    added stateblock support
**       for T&L, set the TLPV_DIRTY_VB flag whenever renderstate info changes, if
**       it's unchanged and the vertex buffer has been processed, we don't T&L it
**       (just render)
**  98   3dfx      1.86.2.3.1.608/10/00 Steve Rogers    Fixing PRS 15142: Shadows
...
**  97   3dfx      1.86.2.3.1.507/29/00 Allen Hansen    fixed bug in parsing of
**       T&L'd indexed fans and strips
**  96   3dfx      1.86.2.3.1.407/24/00 Allen Hansen    Rewrote cpu detection code,
**       cpuType is now defined in cpu.h, all new code should be protected by
**       #ifdef WINNT
**  95   3dfx      1.86.2.3.1.307/18/00 Jonny Cochrane  Fix for PRS 14997 and 14904
**       - Support for TEXTURETRANSFORMFLAGS 2d texture coords
**  94   3dfx      1.86.2.3.1.207/06/00 Bob Seitsinger  In setDX6state, modified
**       setup of texture LOD register to write chip-specific Lod Bias values.
** 
**  93   3dfx      1.86.2.3.1.107/05/00 Bob Seitsinger  Added some comments in
**       HandleSLIAA.
**  92   3dfx      1.86.2.3.1.006/26/00 Johnny Trainor  Fixed guard band clipping
**       problem which caused DCT 350TLVertex Clip test to fail. We shouldn't
**       inclue the bottom and right edges of the clipping rectangle.
**  91   3dfx      1.86.2.3    06/15/00 Sam Hanna       Added code to make sure
**       that the AA buffers exist before referencing them. In addition, added code
**       to disable AA and dither matrix rotation if AA mode is enabled but the AA
**       buffers are not allocated. This fixes PRS 14079.
**  90   3dfx      1.86.2.2    06/05/00 Andrew Sobczyk  New code for 4 units
**  89   3dfx      1.86.2.1    05/16/00 Bob Johnston    Consolodating VERT_BUFF and
**       TnL_HAL defines to just use TnL_HAL
**  88   3dfx      1.86.2.0    05/10/00 Bob Seitsinger  Set pRc->dwZeroJitter to
**       zero before entering ddiDrawPrimitives2 command processing for loop.
**       Augment a few debug statements.
...
DWORD __stdcall ddiDrawPrimitives2( LPD3DHAL_DRAWPRIMITIVES2DATA lpdp2d )
{
  ...
  if (IS_DX7_OR_EARLIER_APP(pRc))
  {
      ...
      pRc->tl.dwDirtyFlags |= TLPV_DIRTY_VB;	// new VB
#else
...

            {
              ...
              else
              {
                    ...
                    (_renderFuncs[renderState] != &dummy))
                {
                  ...
                  SET_STATEBLOCK_RENDERSTATE_FLAG(pRc->pCurrSB,renderState);
                }
              }
#endif
            }
...
#ifdef TnL_HAL
        pRc->tl.dwDirtyFlags |= TLPV_DIRTY_VB;	// changed a renderstate
#endif
...
//      lpPrim = (LPBYTE)(lpCmd + 1);
        ...
        if (_D3(ondrtState)) {
          ...
          else
          {
                ...
                (state <= MAX_TEXTURESTAGESTATES))
            {
              ...
              SET_STATEBLOCK_TEXTSTAGESTATE_FLAG(pRc->pCurrSB,stage,state);
            }
          }
#endif
        ...
        UPDATE_HW_STATE( SC_SOMETHING );
#ifdef TnL_HAL
        pRc->tl.dwDirtyFlags |= TLPV_DIRTY_VB;	// textures changed
#endif
...
//      lpPrim = (LPBYTE)(lpCmd + 1);
...
#if (DIRECT3D_VERSION >= 0x0700) && (DX >= 7)
        // if we're recording a state block just go save the viewport info
        if (FALSE == pRc->bSBRecMode)
        {
#endif
#ifdef  TnL_HAL
          // Update T&L viewport state IF it really changed
		  if( (pRc->tl.Viewport.dwX != pVpt->dwX) ||
          ...
          if (IS_NAPALM) { //cws gb
              DWORD dwX			= ((D3DHAL_DP2VIEWPORTINFO *)lpPrim)->dwX;
              DWORD dwY			= ((D3DHAL_DP2VIEWPORTINFO *)lpPrim)->dwY;
              DWORD dwWidth		= ((D3DHAL_DP2VIEWPORTINFO *)lpPrim)->dwWidth;
              DWORD dwHeight	= ((D3DHAL_DP2VIEWPORTINFO *)lpPrim)->dwHeight;
        ...
        else
        {
          if (NULL != pRc->pCurrSB)
          {
              ...
              DWORD dwWidth		= ((D3DHAL_DP2VIEWPORTINFO *)lpPrim)->dwWidth;
              DWORD dwHeight	= ((D3DHAL_DP2VIEWPORTINFO *)lpPrim)->dwHeight;
              pRc->pCurrSB->uc.ViewportInfoState.dwX		= dwX;
              pRc->pCurrSB->uc.ViewportInfoState.dwY		= dwY;
              pRc->pCurrSB->uc.ViewportInfoState.dwWidth	= dwWidth;
              pRc->pCurrSB->uc.ViewportInfoState.dwHeight	= dwHeight;
              D3DPRINT(D3DDBGLVL,"Storing VIEWPORTINFO %08x %08x %08x %08x",dwX,dwY,dwWidth,dwHeight );

              // set flag to mark this viewport state is stored in state block
              SET_STATEBLOCK_VIEWPORT_FLAG(pRc->pCurrSB);
          }
        }
#endif

#ifdef TnL_HAL
        pRc->tl.dwDirtyFlags |= TLPV_DIRTY_VB;	// viewport changed
#endif
...
// RAJ 8/2/99 Fix for WHQL DCT perspective correction test
   ...
   pRc->scaleW = 1.f / scale;
        ...
        D3DPRINT(D3DDBGLVL+1, "    minW=%lX, maxW=%lX, aW=%lX, bW=%lX",
                 *(ULONG *)&pRc->minW, *(ULONG *)&pRc->maxW,
    ...
    #endif
#ifdef TnL_HAL
        pRc->tl.dwDirtyFlags |= TLPV_DIRTY_VB;	// w changed
#endif
      ...
      {
        D3DPRINT( 32, "  Command TRIANGLELIST (%d, %d) (# PRims = %d)",
					lpCmd->bCommand, lpCmd->wStateCount, lpCmd->wPrimitiveCount );
...
#else
        if (pRc->state & FLAGS_BAD_EXECUTE_CASE)
#endif
        {
           if ( pRc->fillMode != D3DFILL_SOLID )
...
/*******************************************************************************/
        ...
        if ( (
           ...
           #endif
             ((CPUTYPE & CPU_FEATURE_SSE) && !(pRc->state & (FLAGS_FVF_IDX2 | STATE_REQUIRES_AA | STATE_NOT_SOLID_FILL)) && (pRc->zEnable)) ||
           #else
		#endif
		#ifdef K6_2
      ...
      {
        D3DPRINT( 32, "  Command TRIANGLESTRIP (%d, %d) (# Prims %d)",
					lpCmd->bCommand, lpCmd->wStateCount, lpCmd->wPrimitiveCount );
...
//      lpPrim = (LPBYTE)(lpCmd + 1);
        ...
        if (TRANSFORM_NEEDED)
        {
          ...
          if (ProcessPrimitive( pRc ) != D3D_OK)
          {
            ...
            // the primitive's vertices are clipped and we should be able
            // to trivially reject this primitive
        	lpCmd = (LPD3DHAL_DP2COMMAND)((LPBYTE)lpCmd + sizeof(D3DHAL_DP2COMMAND) +
        		sizeof(D3DHAL_DP2STARTVERTEX) + sizeof(WORD)*(lpCmd->wPrimitiveCount+2));
            hr = D3D_OK;
            break;
          }
      ...
      case D3DDP2OP_TRIANGLEFAN_IMM :
      {
        ...
        D3DPRINT( 255, "Edge Flag 0x%08lx", ((LPD3DHAL_DP2TRIANGLEFAN_IMM)lpPrim)->dwEdgeFlags );
      ...
      if (_D3(ondrtTri)) {
        ...
        // to call the ASM code.
		//jcochrane@3dfx.com - no support for w scale down this route either, uses DrawTriFanImmASm_P5 in afan.asm
        if (!(pRc->state & FLAGS_BAD_FAN_IMM_CASE) && !pRc->tl.clipUnion && (pRc->bW==0.0f))
#else
      ...
      {
        D3DPRINT( 32, "  Command TRIANGLEFAN (%d, %d) (# Prims %d)",
					lpCmd->bCommand, lpCmd->wStateCount, lpCmd->wPrimitiveCount );
...
//      lpPrim = (LPBYTE)(lpCmd + 1);
   ...
   D3DPRINT(1, "  TEXBLT - pSrcSurf=%8lXh (hSrc = %ld) pDstSurf=%8lXh (hDst=%ld)",
...
//      lpPrim = (LPBYTE)(lpCmd + 1);
        ...
        pStateSetOp = (D3DHAL_DP2STATESET *)lpPrim;		// operation
...
//      D3DHAL_DP2DRAWTRIPATCH *pDrawTriPatch;
     ...
     **-------------------------------------------------------------*/
      ...
      case D3DDP2OP_ZRANGE:
      {
        ...
        {
#endif
			if( ( (*(DWORD*)&pRc->tl.Viewport.dvMinZ) != (*(DWORD*)&pZRange->dvMinZ) ) ||	// only update this if it's dirty
				( (*(DWORD*)&pRc->tl.Viewport.dvMaxZ) != (*(DWORD*)&pZRange->dvMaxZ) ) )
			{
        ...
        else
        {
			if (NULL != pRc->pCurrSB)
			{
				D3DPRINT(D3DDBGLVL,"Storing new z-range  min=%08xh max=%08xh", *(DWORD*)&(pZRange->dvMinZ), *(DWORD*)&(pZRange->dvMaxZ) );
	    		pRc->pCurrSB->uc.ZRangeState.dvMinZ = pZRange->dvMinZ;
	    		pRc->pCurrSB->uc.ZRangeState.dvMaxZ = pZRange->dvMaxZ;
              	// set flag to mark this change is stored in the state block
				SET_STATEBLOCK_ZRANGE_FLAG(pRc->pCurrSB);
			}
        }
#endif
      ...
      case D3DDP2OP_SETMATERIAL:
	  {
        ...
        {
#endif
			// lets see if the material actually changed because it causes all lights to be dirty,
			// this happens a lot in 3dmark
			if (memcmp( &pRc->tl.lighting.Material, pSetMat, sizeof(D3DMATERIAL7)) != 0)
			{
				pRc->tl.lighting.Material = *(D3DMATERIAL7 *)pSetMat;	// copy the material
		        pRc->tl.dwDirtyFlags |= TLPV_DIRTY_MATERIAL;			// set the dirty flags
        		pRc->tl.dwDirtyFlags |= TLPV_DIRTY_VB;
			}
        ...
        else
        {
			if (NULL != pRc->pCurrSB)
			{
				D3DPRINT(D3DDBGLVL,"Storing material from %lXh", (DWORD)pSetMat );
				memcpy( &pRc->pCurrSB->uc.MaterialState, pSetMat, sizeof(D3DMATERIAL7) );
				SET_STATEBLOCK_MATERIAL_FLAG(pRc->pCurrSB);
			}
        }
#endif
        // Update the command buffer pointer
	  ...
	  // StateBlock recording is done in DP2TL_SetLight()
      ...
      case D3DDP2OP_SETCLIPPLANE:
      {
        ...
        if (FALSE == pRc->bSBRecMode)
        {
			...
			{
			  if (memcmp( &pRc->tl.UserClipPlanes[pSetClipPlane->dwIndex], pSetClipPlane->plane, sizeof(TLVECTOR4)) != 0)	// did they really change?
			  {
            	memcpy( &pRc->tl.UserClipPlanes[pSetClipPlane->dwIndex], pSetClipPlane->plane, sizeof(TLVECTOR4) );
        ...
        else
        {
		  if (NULL != pRc->pCurrSB)
		  {
            for (i = 0; i < (int) wNumClipPlanes; i++, pSetClipPlane++)
			{
				DWORD cpIndex = pSetClipPlane->dwIndex;
				D3DPRINT(D3DDBGLVL,"Storing user clip plane %d from %lXh", cpIndex, (DWORD)pSetClipPlane->plane );
				memcpy( &pRc->pCurrSB->uc.ClipPlaneStates[cpIndex], pSetClipPlane->plane, sizeof(TLVECTOR4) );
				SET_STATEBLOCK_CLIPPLANE_FLAG( pRc->pCurrSB, cpIndex );
			}
		  }
        }
#endif
            ...
            case 2: // 2 chip or 4 chip
              ...
              if (_FF(dwNumUnits) > 1)
                 ...
                 SETPH( cmdFifo, CMDFIFO_BUILD_PK1( 1, 0, aaCtrl, 0 ) );
  			      if ((pRc->dwZeroJitter) || ((NULL != HotKey_Data[AA_HOTKEY].pHotKeyData) && (1 == *HotKey_Data[AA_HOTKEY].pHotKeyData)))
  			         {
