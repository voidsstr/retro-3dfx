...
**
** $Revision: 18$
** $Date: 10/26/00 7:57:49 AM$
**
** $Log: 
**  18   3dfx      1.11.2.5    10/26/00 Johnny Trainor  Initialise new elements of
**       our RC structure.
**  17   3dfx      1.11.2.4    10/23/00 Johnny Trainor  Updated so we no longer use
**       surface local pointers.
**  16   3dfx      1.11.2.3    10/11/00 Brent           Forced check in to enforce
**       branching.
**  15   3dfx      1.11.2.2    10/10/00 Johnny Trainor  We now use a dwDXInterface
**       variable for setting up the DX interface version.
**  14   3dfx      1.11.2.1    09/29/00 Johnny Trainor  Initialize DX 8 context
**       values. Some minor text formating.
**  13   3dfx      1.11.2.0    09/22/00 Johnny Trainor  DX8 support. Initialize the
**       DX interface version. Set D3DRENDERSTATE_TEXTUREPERSPECTIVE to true
**       (always the case in Dx8 ).
** 
...
#ifdef WINNT
  newContext->lpDDS =               pccd->lpDDS;
  newContext->pid   =               pccd->dwPID;
  if (pccd->lpDDSZ)
    newContext->lpDDSZ =            pccd->lpDDSZ;
  else
...
#ifdef WINNT
    ASSERTDD((DWORD)newContext->lpDDS == newContext->pHndlList->ppTxtrHndlList[newContext->DDSHndl]->surfLcl, " render targ surfLcl's don't match!");
#endif
    if (pccd->lpDDSZLcl)
    {
...
#ifdef WINNT
      ASSERTDD((DWORD)newContext->lpDDSZ == newContext->pHndlList->ppTxtrHndlList[newContext->DDSZHndl]->surfLcl, " z buffer surfLcl's don't match!");
#endif
    }
    else
