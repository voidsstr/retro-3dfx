...
** 
** 6     6/01/98 12:21p Adrians
** Added aa to DrawPrimitives.
** 
** 5     5/07/98 6:50p Adrians
** Disable new triangle flavors until later.
** 
** 4     5/07/98 6:43p Adrians
** Added performance test code.
** Two new triangle Flavors (Not yet used).
** 
...
void __stdcall dpDrawTriangleAll(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices )
{
  SETUP_PPDEV(pRc)
...
void __stdcall dpDrawTriangleIZT(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices )
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
...
void __stdcall dpDrawTriangleIZ(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices )
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
  FxU32         setupFlag = pRc->sst.sSetupMode;
...
void __stdcall dpDrawTriangleIZTH(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices )
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
  float         lscaleS = pRc->sst.scaleS, lscaleT = pRc->sst.scaleT;
...
void __stdcall dpDrawTriangleIZTF(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices )
{
  D3DTLVERTEX   *pA, *pB, *pC;
  float         lscaleS = pRc->sst.scaleS, lscaleT = pRc->sst.scaleT;
  FxU32         pk3Hdr = CMDFIFO_BUILD_PK3( CMD_TRI, 3, pRc->sst.sSetupMode, 1 );
  float         area;
  int           sign;
  CMDFIFO_PROLOG(cmdFifo);
  ...
  {  
    ...
    if ((sign ^ pRc->cullMask) != 0x80000000)
    {
    ...
    #else    
      SET( cmdFifo, ghw0->sSetupMode, pRc->sst.sSetupMode);
    #endif    
      ...
      SETPD( cmdFifo, ghw0->sARGB, pA->color );
      SETFPD( cmdFifo, ghw0->sVz, ZSCALE( pA->sz ) );
      SETFPD( cmdFifo, ghw0->sOowfbi, (float)(255 - RGBA_GETALPHA(pA->specular)) );
      SETFPD( cmdFifo, ghw0->sOow0, pA->rhw );
      SETFPD( cmdFifo, ghw0->sSow0, ( ( pA->tu * lscaleS ) + TEXEL_SOFFSET ) * pA->rhw );
      SETFPD( cmdFifo, ghw0->sTow0, ( ( pA->tv * lscaleT ) + TEXEL_TOFFSET ) * pA->rhw );
    #ifndef CMDFIFO    
      ...
      SETPD( cmdFifo, ghw0->sARGB, pB->color );
      SETFPD( cmdFifo, ghw0->sVz, ZSCALE( pB->sz ) );
      SETFPD( cmdFifo, ghw0->sOowfbi, (float)(255 - RGBA_GETALPHA(pB->specular)) );
      SETFPD( cmdFifo, ghw0->sOow0, pB->rhw );
      SETFPD( cmdFifo, ghw0->sSow0, ( ( pB->tu * lscaleS ) + TEXEL_SOFFSET ) * pB->rhw );
      SETFPD( cmdFifo, ghw0->sTow0, ( ( pB->tv * lscaleT ) + TEXEL_TOFFSET ) * pB->rhw );
    #ifndef CMDFIFO    
      ...
      SETPD( cmdFifo, ghw0->sARGB, pC->color );
      SETFPD( cmdFifo, ghw0->sVz, ZSCALE( pC->sz ) );
      SETFPD( cmdFifo, ghw0->sOowfbi, (float)(255 - RGBA_GETALPHA(pC->specular)) );
      SETFPD( cmdFifo, ghw0->sOow0, pC->rhw );
      SETFPD( cmdFifo, ghw0->sSow0, ( ( pC->tu * lscaleS ) + TEXEL_SOFFSET ) * pC->rhw );
      SETFPD( cmdFifo, ghw0->sTow0, ( ( pC->tv * lscaleT ) + TEXEL_TOFFSET ) * pC->rhw );
    #ifndef CMDFIFO    
...
void __stdcall dpDrawTriangleIZTFS(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices )
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
  float         lscaleS = pRc->sst.scaleS, lscaleT = pRc->sst.scaleT;
  FxU32         pk3Hdr = CMDFIFO_BUILD_PK3( CMD_TRI, 3, pRc->sst.sSetupMode, 1 );
  FxU32         pk3HdrS = CMDFIFO_BUILD_PK3( CMD_START, 3, (pRc->sst.sSetupMode & ~(SST_SETUP_ST0 | SST_SETUP_W0 | SST_SETUP_Wfbi)), 1 );
  float         area;
  int           sign;
  CMDFIFO_PROLOG(cmdFifo);
  ...
  {  
    ...
    if ((sign ^ pRc->cullMask) != 0x80000000)
    {
      CMDFIFO_CHECKROOM( cmdFifo, PH3_SIZE + (3 * 8) + 24 );
    #ifdef CMDFIFO    
      SETPH( cmdFifo, pk3Hdr );
    #else    
      SET( cmdFifo, ghw0->sSetupMode, pRc->sst.sSetupMode);
