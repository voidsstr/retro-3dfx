...
//RWS       if (TXTRHNDL_PTR(hSurf) && (TXTRHNDL_PTR(hSurf)->surfLcl == (DWORD)surfLCL))
...
#define TDL_CF_ALIGN_MASK  ((TDL_CF_DW_ALIGN * 4) - 1)
            ...
            ((srcPitchAdjust + dstPitchAdjust) == 0))
         {
            ...
            while (nTextureDwordsR > 0)
            {
               ...
               // a pure linear copy
               //
               if ((nTextureDwords < TDL_LARGETEXTURE_DWORDS) || !_FF(doAgpCF) ||
#ifdef WINNT	// OLD CPUID CODE
                  !(_FF(cpuType) & P6_INTELCPU_WITH_KNI))
#else
                  !(_FF(cpuType) & CPU_FEATURE_SSE))
#endif
               {
                                ...
                                d3Global      *g)
{
   ...
   if (! buffer)
   {
      ...
      if (IS_NAPALM)
         g->buffer = buffer = (DWORD*)DXMALLOC(D3D_BUFFER_SIZE_TBIG);
      else
         g->buffer = buffer = (DWORD*)DXMALLOC(D3D_BUFFER_SIZE);
   }
   if (!buffer)
      return NULL;
