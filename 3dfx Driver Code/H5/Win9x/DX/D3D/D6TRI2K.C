...
** File name:   d6tri2k.c
**
** Description: Implements DrawPrimitive triangle drawing functions with
...
**
** $Revision: 10$
** $Date: 10/11/00 8:47:51 PM$
**
** $Log: 
**  10   3dfx      1.7.1.1     10/11/00 Brent           Forced check in to enforce
**       branching.
**  9    3dfx      1.7.1.0     08/21/00 Allen Hansen    replaced 3dnow macros with
...
**       them)
**  8    3dfx      1.7         01/18/00 Bob Seitsinger  Changes to correctly write
...
**  7    3dfx      1.6         01/13/00 Bob Seitsinger  Encase some variables in
**       #ifdef CMDFIFO. Also, fix some dereferncing syntax errors related to
**       structure element sOowfbi.
**  6    3dfx      1.5         01/10/00 Chris W. Shaw   Default to white diffuse in
**       the case that no diffuse is in FVF.  Fixes PRS 12055.
**  5    3dfx      1.4         12/14/99 Matt McClure    Fix for PRS 11794.  Added
...
** 
** 9     8/17/99 10:47a Msmith
** I added include statements for stbperf.inc so we can enable the
** C_AUTOSTRIP code.
** 
** 8     8/09/99 9:47a Msmith
** Some fixes for 3dnow code related to DCT test fix. From Bob J.
** 
** 7     8/06/99 3:13p Msmith
** oops. Forgot to merge new code in on the last checkin.
** More DCT and auto-strip changes from Bob J.
** 
** 6     8/04/99 2:13p Msmith
...
** Branch from H3
** 
** 8     5/24/99 5:07p Bseitsin
** Removal of Antialiasing code.
** 
** 7     4/09/99 12:35p Stb_bseitsin
** Added Napalm registers. Added ifdef H5.
** 
** 6     1/25/99 4:54p Peterm
** added unified header information
** 
** 5     12/16/98 7:20p Sreid
...
/*-------------------------------------------------------------------
Function Name:  dp2TriangleAll_K62O

Description:    This function handles drawing DrawPrimitive triangles
                in solid-filled mode.  This version is optimized for
                3DNow.
...
void dp2TriangleAll_K62O( RC *pRc, DWORD count, LPBYTE idx, LPDWORD vertices, DWORD vertexType )
{
  LPDWORD       pA, pB, pC;
#ifdef C_AUTOSTRIP
...
#ifdef DCT_FIX
      __asm{ femms }
      // RAJ 8/2/99  DCT fix for perpective correct DCT
...
//                movd    mm3, f_zscale
    ...
    if (ST0_W0_Z_RGB_A) {
           ...
           && (CI_MASKALPHA(pA[FVFO_SPECULAR] | pB[FVFO_SPECULAR] | pC[FVFO_SPECULAR]) != 0))
      {
        ...
        SETPD( cmdFifo, ghw0->sARGB, pA[FVFO_SPECULAR] );
        ...
        SETPD( cmdFifo, ghw0->sVx, *((int*)&tmp_pb  ));
        SETPD( cmdFifo, ghw0->sVy, *((int*)&tmp_pb+1));
        SETPD( cmdFifo, ghw0->sARGB, pB[FVFO_SPECULAR] );
        ...
        SETPD( cmdFifo, ghw0->sVx, *((int*)&tmp_pc  ));
        SETPD( cmdFifo, ghw0->sVy, *((int*)&tmp_pc+1));
        SETPD( cmdFifo, ghw0->sARGB, pC[FVFO_SPECULAR] );
