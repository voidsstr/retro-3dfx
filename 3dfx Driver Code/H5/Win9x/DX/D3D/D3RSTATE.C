...
#ifdef TnL_HAL
  pRc->tl.dwTLState &= ~TLPV_NOT_SOLID_FILL_MODE;
#endif
  if (pRc->fillMode != D3DFILL_SOLID)
  {
     pRc->state |= STATE_NOT_SOLID_FILL;
...
#ifdef TnL_HAL
    pRc->tl.dwTLState |= TLPV_DOSPECULAR;
#endif  //TnL_HAL
  }
  else
  {
...
#ifdef TnL_HAL
    // Every 16 frames read the current 
	// queued buffer count and allow
	// the T&L code to be updated
	// with the proper GB settings
    if ( !(++pRc->tl.dwFrameCounter % 16) )
    {
	   pRc->tl.dwFramesQueued = READSWAPCOUNT();
	   pRc->tl.dwDirtyFlags |= (TLPV_DIRTY_DYNGB | TLPV_DIRTY_VIEWRECT);
    }
#endif
  }
  else
...
//    pRc->sst.pixelOffset = 0.5f;
    pRc->sst.pixelOffset = 0.5f + 1/32.0f;
  else
  ...
  else
  {
    ...
    switch(pgsd->ddState.drstRenderStateType)
    {
      ...
      case D3DRENDERSTATE_RANGEFOGENABLE :			// 48
        pgsd->ddState.dwArg[0] = pRc->tl.lighting.fog_range_enable ;
        break;

      case D3DRENDERSTATE_CLIPPING :				// 136
        pgsd->ddState.dwArg[0] = pRc->tl.StateClipping ;
        break;

      case D3DRENDERSTATE_LIGHTING :				// 137
        pgsd->ddState.dwArg[0] = pRc->tl.StateLighting ;
        break;

      case D3DRENDERSTATE_AMBIENT :					// 139
        pgsd->ddState.dwArg[0] = pRc->tl.lighting.ambient_save ;
        break;

      case D3DRENDERSTATE_FOGVERTEXMODE :			// 140
        pgsd->ddState.dwArg[0] = pRc->tl.lighting.fog_mode ;
        break;

      case D3DRENDERSTATE_COLORVERTEX :				// 141
        pgsd->ddState.dwArg[0] = pRc->tl.StateColorVertex ;
        break;

      case D3DRENDERSTATE_LOCALVIEWER :				// 142
        pgsd->ddState.dwArg[0] = pRc->tl.StateLocalViewer ;
        break;

      case D3DRENDERSTATE_NORMALIZENORMALS :		// 143
        pgsd->ddState.dwArg[0] = pRc->tl.StateNormNormals ;
        break;

      case D3DRENDERSTATE_DIFFUSEMATERIALSOURCE :	// 145
        pgsd->ddState.dwArg[0] = pRc->tl.lighting.DfusMaterialSrc ;
        break;

      case D3DRENDERSTATE_SPECULARMATERIALSOURCE :	// 146
        pgsd->ddState.dwArg[0] = pRc->tl.lighting.SpecMaterialSrc ;
        break;

      case D3DRENDERSTATE_AMBIENTMATERIALSOURCE :	// 147
        pgsd->ddState.dwArg[0] = pRc->tl.lighting.AmbMaterialSrc ;
        break;

      case D3DRENDERSTATE_EMISSIVEMATERIALSOURCE :	// 148
        pgsd->ddState.dwArg[0] = pRc->tl.lighting.EmisMaterialSrc ;
        break;

      case D3DRENDERSTATE_VERTEXBLEND :				// 151
        pgsd->ddState.dwArg[0] = pRc->tl.StateVertexBlends ;
        break;

      case D3DRENDERSTATE_CLIPPLANEENABLE :			// 152
        pgsd->ddState.dwArg[0] = pRc->tl.clipPlaneEnable ;
        break;
#endif
