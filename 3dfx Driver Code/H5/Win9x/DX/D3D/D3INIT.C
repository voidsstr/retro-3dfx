...
**
** $Revision: 61$
** $Date: 10/26/00 7:59:48 AM$
**
**
** $Log: 
**  61   3dfx      1.32.1.6.1.2010/26/00 Johnny Trainor  Removed  D3DFVFCAPS_PSIZE
**       cap.
** 
**  60   3dfx      1.32.1.6.1.1910/23/00 Johnny Trainor  #ifdefed some DX5 only
**       stuff
**  59   3dfx      1.32.1.6.1.1810/18/00 Allen Hansen    Hacked around execute
**       buffer creation problem, we'll only create these now with the SSE parts. 
**       This can slow down non-T&L apps when T&L is enabled (DX issue).
**  58   3dfx      1.32.1.6.1.1710/11/00 Brent           Forced check in to enforce
**       branching.
**  57   3dfx      1.32.1.6.1.1610/10/00 Johnny Trainor  Modified
**       MaxPrimitiveCount, MaxVertexIndex and MaxStreamStride so that they match
**       the DX8 sample driver. These were all incorrectly initialized to zero.
**  56   3dfx      1.32.1.6.1.1510/09/00 Johnny Trainor  Added a few more texture
**       formats to the DX8 code including D3DFMT_X4R4G4B4 and D3DFMT_D16.
**  55   3dfx      1.32.1.6.1.1410/04/00 Johnny Trainor  Replaced IS_DX7() and
**       IS_DX6() macros. They have now been fixed.
**  54   3dfx      1.32.1.6.1.1310/04/00 Leo Galway      Fix for PRS 15633 - Caps
**       bits missing for 32bit, DXTn and FXT1 texture formats under DX7 Caps
**       Viewer on Win98. Removed IS_DX7 and IS_DX6 check macros in
**       BuildDX7TextureFormats(). Also corrected RGB Bit Mask settings for 8-bit
**       RBG 332 texture format.
**  53   3dfx      1.32.1.6.1.1210/02/00 Johnny Trainor  Changed the DX capability
**       reporting code. Added two new functions, BuildD3DCaps() and
**       BuildD3DCaps8().
**  52   3dfx      1.32.1.6.1.1109/27/00 Johnny Trainor  Changed the texture format
**       reporting code. Added two new functions,  BuildDX7TextureFormats() and
**       BuildDX8TextureFormats().
**  51   3dfx      1.32.1.6.1.1009/18/00 Andrew Sobczyk  Added code to add Scott
**       Optimization; Switch from Digital AA to Analog when in Config 7 or 8; and
**       to roll down to mode 6 when in 7 or 8 with pixel  clock doubling <all done
**       for the 6000>
**  50   3dfx      1.32.1.6.1.909/03/00 Allen Hansen    added DX7 texture caps (but
**       didn't turn them on)
**  49   3dfx      1.32.1.6.1.808/27/00 Allen Hansen    Only turn on T&L on SSE or
**       extended-mmx (Athlon's)
**  48   3dfx      1.32.1.6.1.707/24/00 Allen Hansen    Rewrote cpu detection code,
**       cpuType is now defined in cpu.h, all new code should be protected by
**       #ifdef WINNT
**  47   3dfx      1.32.1.6.1.607/06/00 Bob Seitsinger   Modified ifdef for code to
**       retrieve LodDither from registry to include ifdef LODBIASPERCHIP.
** 
**  46   3dfx      1.32.1.6.1.507/05/00 Allen Hansen    changed "TextureLighting"
**       to "TransformAndLighting"
**  45   3dfx      1.32.1.6.1.407/05/00 Reid Campbell   Updated to add check for
**       regisrty entry which enables T&L.
**  44   3dfx      1.32.1.6.1.307/03/00 Allen Hansen    turned off SW T&L on
**       non-SSE or 3DNow cpu's
**  43   3dfx      1.32.1.6.1.207/03/00 Bob Seitsinger  (1) Added code in
**       D3DHalCreateDriver to retrieve _D3(LodDither) from registry, if LODDITHER
**       is defined. (2) Moved sanity check code for max pending buffers to
**       D3DHalCreateDriver from ddFlip.
** 
**  42   3dfx      1.32.1.6.1.106/30/00 Bob Seitsinger  (a)  Included
**       D3DPTEXTURECAPS_TEXREPEATNOTSCALEDBYSIZE in caps bits for lines and
**       triangles, if TEXTURE_REPEAT_NOT_SCALED is defined. (b) Changed default
**       value for _D3(columnBandControl) to 0, if COLUMNBANDCONTROL_DEFAULT is
**       defined. (c) Added code in D3DHalCreateDriver to retrieve
**       _D3(maxPendingBuffers) from registry, if MAXPENDINGBUFFERS is defined.
** 
**  41   3dfx      1.32.1.6.1.006/25/00 Allen Hansen    added tnl support for
**       non-kni cpu's, if-def'd out the old 3dnow now-tnl rendering functions from
**       the tnl path. The reason is they don't support clipping, they're really
**       not used anymore (the asm code is used instead), and there's still the
**       fallback C path.  This path is used by tnl for the fallback path (cases
**       like vertex blends or color vertex).
**  40   3dfx      1.32.1.6    06/14/00 James Hunter    Disabled
**       D3DPRASTERCAPS_ANTIALIASSORTDEPENDENT capability on the Voodoo 3.
** 
** 
**  39   3dfx      1.32.1.5    06/05/00 Andrew Sobczyk  New code and variables for
**       4 units AA modes
**  38   3dfx      1.32.1.4    05/30/00 Allen Hansen    Fixed
**       D3DDEVCAPS_HWTRANSFORMANDLIGHT enable so it won't get set on non-sse cpu's
**  37   3dfx      1.32.1.3    05/19/00 Bob Seitsinger  When retrieving registry
**       lod bias, do NOT shift. Just leave as a normal integer.
**  36   3dfx      1.32.1.2    05/18/00 Bob Johnston    final removal of VERT_BUFF
**  35   3dfx      1.32.1.1    05/17/00 Bob Seitsinger  Clamp LOD Bias to -32 to
**       +31 in D3DHalCreateDriver.
**  34   3dfx      1.32.1.0    05/08/00 Bob Johnston    Change to not allow non SSE
**       enabled CPU's to export DX7 HW Trasformation and light when compiling
**       under TnL_HAL
...
#else
   if (! (CPUTYPE & CPU_FEATURE_3DNOW)) return FALSE;
#endif
...
// AHansen 6/4/00
// I hacked these out because they don't handle clipped tri's.  We (I) should probably add clipping 
// to these but on the other hand, these get executed rarely; most of the time the asm code handles 
// the tri's.  I ran the this using the d3d runtime for tnl on an Athlon 950 with and without this 
// enabled and the differences in the scores were in the noise range so this is probably okay.
// AHansen 6/17/00 added the strips and fans
#ifndef TnL_HAL
...
#ifdef WINNT	// OLD CPUID CODE
   FxU32 cpu = cpu_detect_asm();	// really "detect_intel_cpu_asm"
#endif
...
//////////////////////////
...
#ifdef WINNT	// OLD CPUID CODE
   if (cpu!=6)	// not Intel or family < 6
      _cpu_type=STATE_BAD_CPU;	// UNUSED!
#endif
   ...
   if (cpu==6)
#else
   if ((CPUTYPE & CPU_INTELP6) && (CPUTYPE & CPU_FEATURE_CMOV))
#endif
   {
	  // These all require CMOV instructions
...
//        PrefetchTriIdx2_6T_Asm=PrefetchTriIdx2_6T_Asm_KNI;
    ...
    // Read registry to check for the enabling of TnL
    if( NULL == GETENV(TRANSFORM_AND_LIGHTING) )
    { 
       _D3(TransformAndLighting) = TnL_DISABLED;
    }
    else
    {
       _D3(TransformAndLighting) = ATOI(GETENV(TRANSFORM_AND_LIGHTING));
    }


    
	// Only the SSE path uses optimized VBs
    if(CPUTYPE & CPU_FEATURE_SSE) 
	{
	...
	{
      pExebufCallbacks->dwSize                = sizeof(DDHAL_DDEXEBUFCALLBACKS);
      pExebufCallbacks->CanCreateExecuteBuffer= 0;
      pExebufCallbacks->CreateExecuteBuffer   = 0;
      pExebufCallbacks->DestroyExecuteBuffer  = 0;
      pExebufCallbacks->LockExecuteBuffer     = 0;
      pExebufCallbacks->UnlockExecuteBuffer   = 0;
      pExebufCallbacks->dwFlags               = 0;
    }            
#endif //TnL_HAL
  ...
  if (IS_NAPALM)
  {
     ...
     _D3( TwoPpcLog2BandHeight ) = TwoPpcInit(ppdev);
...
#endif
    if (IS_NAPALM)
      (DWORD *)_D3(buffer) = (DWORD*)DXMALLOC(D3D_BUFFER_SIZE_TBIG);
    else
      (DWORD *)_D3(buffer) = (DWORD*)DXMALLOC(D3D_BUFFER_SIZE);
