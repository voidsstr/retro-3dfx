...
** $
** 
** 4     8/19/99 3:23p Msmith
** changed an fp conversion to use a lookup table
** 
...
** 
** 8     8/23/98 12:49a Adrians
** Added support for AA SuperSampling.
** 
** 7     7/24/98 1:37p Hohn
** 
** 6     6/01/98 12:20p Adrians
** Added aa to DrawPrimitives.
** 
** 5     5/07/98 6:43p Adrians
** Added performance test code.
** 
** 1     4/29/98 6:31p Adrians
** Created
** 
** 4     4/17/98 8:13p Miriam
** typo in macro, file wouldn't compile with direct writes.
** 
...
void __stdcall dpDrawFanAll(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices )
{
  SETUP_PPDEV(pRc)
...
//  faster here to use a lookup vs. letting fpu do conversion -mls
	  fogA = f_255_reverse_lookup[RGBA_GETALPHA(pA->specular)];
	  fogB = f_255_reverse_lookup[RGBA_GETALPHA(pB->specular)];
	  fogC = f_255_reverse_lookup[RGBA_GETALPHA(pC->specular)];
#else
...
void __stdcall dpDrawFanIZ(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices)
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA,  *pB,   *pC;
  FxU32         setupFlag = pRc->sst.sSetupMode | SST_SETUP_FAN;
  CMDFIFO_PROLOG(cmdFifo);
  ...
  for( --count; count > 0; --count )
  {
    ...
    //Send additonal vertex for next triangle    
    SETFPD( cmdFifo, ghw0->sVx, pC->sx + PIXEL_OFFSET );
    SETFPD( cmdFifo, ghw0->sVy, pC->sy + PIXEL_OFFSET );
    SETPD( cmdFifo, ghw0->sARGB, pC->color );
    SETFPD( cmdFifo, ghw0->sVz, ZSCALE( pC->sz ) );
...
//-------------------------------------------------------------------
// Iterated color, ZBuffering and Texturing.
void __stdcall dpDrawFanIZT(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices)
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
...
//-------------------------------------------------------------------
// Iterated Color, Texturing and Hardware Table Fog.
void __stdcall dpDrawFanITH(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices)
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
  float         lscaleS = pRc->sst.scaleS, lscaleT = pRc->sst.scaleT;
  FxU32         setupFlag = (pRc->sst.sSetupMode | SST_SETUP_FAN) & ~SST_SETUP_W0;
  CMDFIFO_PROLOG(cmdFifo);
  ...
  CMDFIFO_CHECKROOM( cmdFifo, PH3_SIZE + (3 * 6) );
#ifdef CMDFIFO    
  SETPH( cmdFifo, CMDFIFO_BUILD_PK3( CMD_START, 3, setupFlag, 1 ) );
#else    
  SET( cmdFifo, ghw0->sSetupMode, setupFlag);
  ...
  for( --count; count > 0; --count )
  {
    ...
    CMDFIFO_CHECKROOM( cmdFifo, PH3_SIZE + 6 );
  #ifdef CMDFIFO    
    SETPH( cmdFifo, CMDFIFO_BUILD_PK3( CMD_CONT, 1, setupFlag, 1 ) );
  #else    
    SET( cmdFifo, ghw0->sSetupMode, setupFlag);
...
//-------------------------------------------------------------------
// Iterated Color and Hardware Table Fog.
void __stdcall dpDrawFanIH(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices)
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
  FxU32         setupFlag = pRc->sst.sSetupMode | SST_SETUP_FAN;
...
//-------------------------------------------------------------------
// Iterated Color and Hardware Table Fog.
void __stdcall dpDrawFanI(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices)
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
  FxU32         setupFlag = pRc->sst.sSetupMode | SST_SETUP_FAN;
  ...
  CMDFIFO_CHECKROOM( cmdFifo, PH3_SIZE + (3 * 3) );
#ifdef CMDFIFO    
  SETPH( cmdFifo, CMDFIFO_BUILD_PK3( CMD_START, 3, setupFlag, 1 ) );
#else    
  SET( cmdFifo, ghw0->sSetupMode, setupFlag);
  ...
  for( --count; count > 0; --count )
  {
    ...
    CMDFIFO_CHECKROOM( cmdFifo, PH3_SIZE + 3 );
  #ifdef CMDFIFO    
    SETPH( cmdFifo, CMDFIFO_BUILD_PK3( CMD_CONT, 1, setupFlag, 1 ) );
  #else    
    SET( cmdFifo, ghw0->sSetupMode, setupFlag);
...
//-------------------------------------------------------------------
// Iterated Color and Texturing.
void __stdcall dpDrawFanIT(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices)
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
  float         lscaleS = pRc->sst.scaleS, lscaleT = pRc->sst.scaleT;
...
//-------------------------------------------------------------------
// Iterated color, ZBuffering and Texturing.
void __stdcall dpDrawFanIZTH(RC *pRc, DWORD count, WORD *triIndex, LPD3DTLVERTEX vertices)
{
  SETUP_PPDEV(pRc)
  D3DTLVERTEX   *pA, *pB, *pC;
  float         lscaleS = pRc->sst.scaleS, lscaleT = pRc->sst.scaleT;
