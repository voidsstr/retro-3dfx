...
** File name:   d6fank.c
**
** Description: Implementes DrawPrimitive fan drawing functions with
...
**
** $Revision: 11$
** $Date: 10/11/00 8:47:48 PM$
**
** $Log: 
**  11   3dfx      1.7.1.2     10/11/00 Brent           Forced check in to enforce
**       branching.
**  10   3dfx      1.7.1.1     08/21/00 Allen Hansen    replaced 3dnow macros with
...
**       them)
**  9    3dfx      1.7.1.0     07/11/00 Steve Rogers    Fixing PRS 8371: Flat
...
** $
** 
** 8     9/02/99 11:15a Cwilcox
** Added 24bpp zbuffer support to Direct3D.
** 
** 7     8/09/99 10:09a Msmith
** Added some femms around 3dnow code for DCT fix. Bob J.
** 
** 6     8/06/99 2:51p Msmith
** More DCT and auto-strip changes from Bob J.
** 
** 5     8/04/99 2:06p Msmith
** fixes for DCT failure from Bob J.
** 
** 4     7/28/99 11:59p Bseitsin
...
Return:         void
-------------------------------------------------------------------*/
void dp2FanAll_K62O( RC *pRc, DWORD count, LPWORD idx, LPDWORD vertices, DWORD vertexType )
{
  LPDWORD       pA, pB, pC;
    ...
    if (ST0_W0_Z_RGB_A) {
           ...
           && (CI_MASKALPHA(pA[FVFO_SPECULAR] | pB[FVFO_SPECULAR] | pC[FVFO_SPECULAR]) != 0))
      {
        ...
        __asm{
                ...
                movq    tmp_pa, mm0
                punpckldq mm1, [ecx+edx*4]             //pA[FVFO_SY]|pA[FVFO_SX]
        ...
        if(setup & SST_SETUP_Wfbi) 
           ...
           SETPD( cmdFifo, ghw0->sOowfbi, *((int*)&wb1) );
#else
           SETPD( cmdFifo, ghw0->sOowfbi, *((int*)&w2) );  //RAJ w2?  Why??
#endif
