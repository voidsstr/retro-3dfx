...
**
** $Revision: 10$
** $Date: 10/11/00 8:48:55 PM$
**
** $Log: 
**  10   3dfx      1.5.2.0.1.2 10/11/00 Brent           Forced check in to enforce
**       branching.
**  9    3dfx      1.5.2.0.1.1 08/25/00 Steve Rogers    Fix for WHQL tests when TNL
**       is enabled.  This changes the line width to start at -.5 and goto +.5
**       rather than go from -1.0 to 0.
**  8    3dfx      1.5.2.0.1.0 08/21/00 Allen Hansen    look for "SPECULARFIX" -
...
**  7    3dfx      1.5.2.0     05/19/00 Jonny Cochrane  Fix an ifdef typo,
**       NUMTEXTURES was used instead of NUMTEXTUREUNITS causing a fifo stall
** 
** 
** 
** 
...
#define LINE_WIDTH_POS 0.5f
#define LINE_WIDTH_NEG 0.5f
...
//    WRAPST(s, t, pRc->wrapU, pRc->wrapV);
  ...
  if(xMajor)
  {
    ...
    SETFPD( cmdFifo, ghw0->sVy, ay + LINE_WIDTH_POS + PIXEL_OFFSET );
#endif
...
#else
    SETFPD( cmdFifo, ghw0->sVx, ax + PIXEL_OFFSET );
    SETFPD( cmdFifo, ghw0->sVy, ay - LINE_WIDTH_NEG + PIXEL_OFFSET );
#endif
    if(setupFlag & (SST_SETUP_RGB | SST_SETUP_A))
      SETPD( cmdFifo, ghw0->sARGB, aColor );
    if(setupFlag & SST_SETUP_Z)
    ...
    SETFPD( cmdFifo, ghw0->sVy, by + LINE_WIDTH_POS + PIXEL_OFFSET );
#endif
    if(setupFlag & (SST_SETUP_RGB | SST_SETUP_A))
...
#else
    SETFPD( cmdFifo, ghw0->sVx, bx + PIXEL_OFFSET );
    SETFPD( cmdFifo, ghw0->sVy, by - LINE_WIDTH_NEG + PIXEL_OFFSET );
#endif
    if(setupFlag & (SST_SETUP_RGB | SST_SETUP_A))
      SETPD( cmdFifo, ghw0->sARGB, bColor );
    if(setupFlag & SST_SETUP_Z)
  ...
  else
  {
    ...
    SETFPD( cmdFifo, ghw0->sVy, ay +fYAdj1 + PIXEL_OFFSET );
#else
    SETFPD( cmdFifo, ghw0->sVx, ax - LINE_WIDTH_NEG + PIXEL_OFFSET );
    ...
    SETFPD( cmdFifo, ghw0->sVy, ay +fYAdj2 + PIXEL_OFFSET );
#else
    SETFPD( cmdFifo, ghw0->sVx, ax + LINE_WIDTH_POS + PIXEL_OFFSET );
    SETFPD( cmdFifo, ghw0->sVy, ay + PIXEL_OFFSET );
#endif
    if(setupFlag & (SST_SETUP_RGB | SST_SETUP_A))
      SETPD( cmdFifo, ghw0->sARGB, aColor );
    ...
    SETFPD( cmdFifo, ghw0->sVy, by +fYAdj1 + PIXEL_OFFSET );
#else
    SETFPD( cmdFifo, ghw0->sVx, bx - LINE_WIDTH_NEG + PIXEL_OFFSET );
    ...
    SETFPD( cmdFifo, ghw0->sVy, by +fYAdj2 + PIXEL_OFFSET );
#else
    SETFPD( cmdFifo, ghw0->sVx, bx + LINE_WIDTH_POS + PIXEL_OFFSET );
    SETFPD( cmdFifo, ghw0->sVy, by + PIXEL_OFFSET );
#endif
    if(setupFlag & (SST_SETUP_RGB | SST_SETUP_A))
      SETPD( cmdFifo, ghw0->sARGB, bColor );
