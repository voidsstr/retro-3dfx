...
**
** File name:   d6stripi.c
**
** Description: Implements DrawPrimitive strip drawing functions.
**
...
**
** $Revision: 20$
** $Date: 10/11/00 8:43:29 PM$
**
** $Log: 
**  20   3dfx      1.16.3.2    10/11/00 Brent           Forced check in to enforce
**       branching.
**  19   3dfx      1.16.3.1    08/11/00 Russ Lind       merge of various w9x
**       changes into w2k
**       including
**         LODBIASPERCHIP
**         LODDITHER
...
**  18   3dfx      1.16.3.0    07/26/00 Daoxiang Gong   Fixed PRS 14903. Enable
**       specular only when the specular color entry is within FVFO Size.
...
**  16   3dfx      1.15        01/28/00 Scott Kephart   Big T&L Merge: FVF handling
**       changes
**  15   3dfx      1.14        01/13/00 Bob Seitsinger  Encase some variables in
**       #ifdef CMDFIFO. Also, fix some dereferncing syntax errors related to
**       structure element sOowfbi.
**  14   3dfx      1.13        01/10/00 Chris W. Shaw   Default to white diffuse in
**       the case that no diffuse is in FVF.  Fixes PRS 12055.
**  13   3dfx      1.12        01/06/00 Tim Little      Removed the
**       IGX_SPECULAR_FIX2 code.  Fixes PRS #12053
...
**       restore use of pF in dp2StripAll when shadeMode=FLAT (fixes Test 7)
...
** $
** 
** 10    8/19/99 4:48p Msmith
** adding opt for converting int to float with lookup table
** 
** 9     8/17/99 10:45a Msmith
** I added include statements for stbperf.inc so we can enable the
** C_AUTOSTRIP code.
** 
** 8     8/06/99 3:02p Msmith
** More auto-strip and DCT changes from Bob J.
** 
** 7     8/05/99 2:01p Msmith
** adding auto-strip code from Bob J.
** 
** 6     8/03/99 2:35p Msmith
...
** Add ifdef CMDFIFO's around DumpCmdFifoEntries
**
** 22    4/09/99 12:35p Stb_bseitsin
** Added Napalm registers. Added ifdef H5.
**
** 21    3/27/99 2:10p Russ
** additional debug output
**
** 20    2/26/99 8:29a Cshaw
** Fix for PRS 4049.  Insert a nopCMD in dp2StripAll for the
** 2-pass-specular case.  Also add 2 to the CMDFIFO_CHECKROOM macro.
**
** 19    2/18/99 8:28a Russ
** added some debug output in dp2StripAll (only affects NT5 debug builds)
**
...
** super-sampling AA.
**
** 16    11/22/98 9:13p Andrew
** Changes to support multi-monitor
**
** 15    10/15/98 6:35p Artg
** changed ifdef h3 to account for h4
** ifdef h3  --> if defined(h3) || defined(h4)
**
** 14    10/15/98 11:09a Artg
** fixed typo ghw-SOowfbi to ghw->SOowfbi.
** appears only in direct write cases.
**
...
Return:         void
-------------------------------------------------------------------*/
void dp2StripAllFill( RC *pRc, DWORD count, LPWORD idx, LPDWORD vertices, DWORD vertexType )
{
  LPDWORD       pA, pB, pC, pF;
#ifdef TnL_HAL
...
void dp2StripAll( RC *pRc, DWORD count, LPWORD idx, LPDWORD vertices, DWORD vertexType )
{
  ...
  for( ; count > 0; count-- )
  {
    ...
    {
#ifdef CMDFIFO
      DumpCmdFifoEntries(MakeString(Stringize,dp2StripAll), ppdev, cmdFifo);
#endif
      ...
      #endif //NEW_CCU
    #else
      CMDFIFO_CHECKROOM( cmdFifo, (PH3_SIZE * 2) + 24 + 24  + (PH1_SIZE * 7) + 8 + 2);
    #endif
#else
    #if (NUMTEXTUREUNITS > 1)
      CMDFIFO_CHECKROOM( cmdFifo, (PH3_SIZE * 2) + 30 + 15  + (PH1_SIZE * 7) + 8 );
    #else
      CMDFIFO_CHECKROOM( cmdFifo, (PH3_SIZE * 2) + 24 + 15  + (PH1_SIZE * 7) + 8 + 2);
    #endif
#endif
...
#pragma message("note - NOT compiling with performance opt for int to float conversion")
           ...
           && (CI_MASKALPHA(pA[FVFO_SPECULAR] | pB[FVFO_SPECULAR] | pC[FVFO_SPECULAR]) != 0))
      {
        ...
        #if defined(H3) //This fixes sparkles for 1-pixel-specular triangles on banshee -cws.
