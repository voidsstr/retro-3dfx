...
MIX       mix)          // Always a copy mix -- 0x0d0d
{
    ...
    if (_FF(ddMultiChipConfig))
    {
      ...
      bRet = EngTextOut(pso, pstro, pfo, pco, prclExtra, prclOpaque,
...
#if USE_NT5_DDMEMMGR
          psoPunt->pvScan0 = pdsurf->pvScan0;
#else
...
#if USE_NT5_DDMEMMGR
    if ((GLYPH_CACHE_HEIGHT / ppdev->cjPelSize) < (ppdev->cyMemory - ppdev->cyScreen))
#else
...
#if USE_NT5_DDMEMMGR
        ppdev->cyText = GLYPH_CACHE_HEIGHT / ppdev->cjPelSize;

        ppdev->ulGlyphStart
            = (ppdev->cyScreen * ppdev->cxMemory * cFactor) + ppdev->ulScreenOffset;
#else
        ppdev->ulGlyphStart
...
#if USE_NT5_DDMEMMGR
        ppdev->ulGlyphEnd
            = (((ppdev->cyScreen + ppdev->cyText) * ppdev->cxMemory) * cFactor) + ppdev->ulScreenOffset;
#else
        ppdev->ulGlyphEnd
...
#if USE_NT5_DDMEMMGR
        DISPDBG((1, "Text cache gets %lXh x %lXh surface at (%lXh, %lXh)",
                 ppdev->cxMemory, ppdev->cyText, 0 , ppdev->cyScreen));
#endif
    }
