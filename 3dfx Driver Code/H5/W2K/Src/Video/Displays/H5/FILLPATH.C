...
FLONG       flOptions)
{
	...
	// DWF(dfowler@stb.com)2/23/99 Fixes PRS#4425
	// there appears to be problem with hardware polygons, such that this function fails
	// WHQL, in 1920x1200x32bpp
	// to work around this problem, let GDI handle all of the drawing in this mode.
	// there is a software drawing routine in this driver; however, I'm not that familiar
	// with it and would rather let GDI handle the routine.
	// DanO 7/22/99 The above problem resurfaced in 1920x1200x16bpp and 1920x1200x8bpp for Velocity 100 WHQL
	// testing.  So we are now punting at all pixel depths in both Velocity and Voodoo3 to avoid the problem.
    ppdev = (PDEV*)pso->dhpdev;
    if( ( ppdev->cxScreen == 1920 ) && ( ppdev->cyScreen == 1200 ) )
    {
    	// return false out of this routine, causing GDI to handle the drawing
    	goto ReturnFalse;
    }
#endif
    ...
    if (_FF(ddMultiChipConfig))
    {
      ...
      bRetVal = EngFillPath(pso, ppo, pco, pbo, pptlBrush, mix, flOptions);
        ...
        ((((rop4 >> 4) ^ (rop4)) & 0xf0f) != 0))  // Only do if we need a pattern
    {
        ...
        // Prevent transparent from calling vMmFastPatRealize
        // because Banshee doesn't support transparent patterns with rops.
        //
        // Might be able support transparent for non pattern rop4's.
        // For example rop4=0xaa00 , no pattern is required but we get one
        // with the mask in it? So, by realizing the pattern here, if the
        // pattern is mono we can use it as source data to determine
        // transparency in fastfill.c.

        if (bRealizeTransparent)
            return FALSE;
...
not_rectangle:
    ...
    if (ppo->cCurves > MAX_EDGES) {
        ...
        //
        pFreeEdges = ENGALLOCMEM(0, ppo->cCurves * sizeof(EDGE), ALLOC_TAG, &pFreeEdges);
