...
DUMP_SURFACEDESC( NT9XDEVICEDATA *ppdev, int Level, LPDDSURFACEDESC lpSD )
{
  ...
  if (DDSD_PITCH & lpSD->dwFlags)
    DISPDBG((NORMAL_DBG_LEVEL, "    pitch  = %8lXh", lpSD->lPitch));
  if (DDSD_LINEARSIZE & lpSD->dwFlags)
    DISPDBG((NORMAL_DBG_LEVEL, "    linearSize = %8lXh", lpSD->dwLinearSize));
