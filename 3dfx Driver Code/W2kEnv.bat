@echo off
REM -----------------------------------------------------------------------------
REM Copyright (c) 2000, 3Dfx Interactive, Inc.
...
REM 
REM SETENV.BAT --- Setup build environment for Napalm W2K display driver.
REM -----------------------------------------------------------------------------

REM
REM setup drive letters
REM
:EnvChk0
REM Checking Environment tools
if not X%DEVTOOLS%==X   goto EnvChk1
echo.
echo The DEVTOOLS environment variable is not set correctly.
echo Currently DEVTOOLS = "%DEVTOOLS%"
echo.
echo Please set this variable and rerun this batch file.
goto EnvError
...
if X%OS%==X goto w95path
REM
REM setting Win2K start path
REM
...
set MSDevDir=%DEVTOOLS%\msvc6_0\common\msdev98
set MSVCDir=%DEVTOOLS%\msvc6_0\vc98
...
set path=%DEVTOOLS%\masm614\bin;%path%
set include=%DEVTOOLS%\masm614\inc;%include%
set lib=%DEVTOOLS%\masm614\lib;%lib%

REM
REM move include files
REM

REM copy %SRCDRV%\3dfx\devel\h5\incsrc\h3gdefs.h %SRCDRV%\3dfx\devel\h5\include
REM copy %SRCDRV%\3dfx\devel\h5\incsrc\h3defs.h  %SRCDRV%\3dfx\devel\h5\include
REM copy %SRCDRV%\3dfx\devel\h5\incsrc\h3regs.h  %SRCDRV%\3dfx\devel\h5\include

REM
REM miscellaneous stuff
REM 

if X%TMP%==X  set tmp=%WINDIR%\temp
